...
> **Current adoption model.** The current adoption and default direction is the **shared / global stable runtime ToolRoot** — channel 3, the global stable install at `%USERPROFILE%\.claude\ai-harness-toolset\current`, resolved per invocation (see `docs/contracts/global-invocation/SHARED_GLOBAL_INVOCATION_CONTRACT.md` and `docs/systems/install-update/GLOBAL_INSTALL_UPDATE_MODEL.md`). The **legacy project-local copy mode** (channel 5) — the `.ai-harness/` payload covered in its own subsection below — is still supported for backward compatibility, but is not the recommended adoption shape for new projects. Source-repo dogfooding resolves the ToolRoot to the repo root (channel 4), but channel 4 is only reached when no channel 3 global stable install is present; on a machine that has one, pass an explicit `-ToolRoot <repo-root>` (channel 1) so channel 3 does not shadow it. Full mode boundaries: `docs/user_guide/OPERATOR_GUIDE_KR.md` §2.
...
Day-to-day, the entrypoint is the Claude Code natural-language UX (`docs/user_guide/OPERATOR_GUIDE_KR.md` §7); the raw PowerShell commands in the sections below are the fallback / reference shape. Materializing and updating the channel 3 install follows `docs/systems/install-update/GLOBAL_INSTALL_UPDATE_MODEL.md`.
...
- No automatic or system-wide install. No system-wide CLI, no PATH mutation. (The channel 3 global stable runtime ToolRoot lives under `%USERPROFILE%\.claude\ai-harness-toolset\current`, but it is a deliberate, user-requested materialization — not an auto-install, not a PATH change, and not a system-wide CLI; see `docs/systems/install-update/GLOBAL_INSTALL_UPDATE_MODEL.md` and `docs/user_guide/OPERATOR_GUIDE_KR.md` §15.)
...
|---|---|---|
| `docs/current/SOURCE_OF_TRUTH.md` | active operational | Question → read-first document (Primary / Secondary / Implementation / Historical / Do-not-use). The only file under `docs/current/`. |
...
Note: as part of the access-pattern restructure, the former `docs/roadmap/` design/model/decision/record docs moved to their scope homes — `docs/decisions/POST_MVP_PLAN.md`, `docs/decisions/GLOBAL_ADOPTION_DECISION.md`, `docs/systems/install-update/GLOBAL_INSTALL_UPDATE_MODEL.md`, `docs/systems/install-update/STEP3_INSTALL_UPDATE_DECISION_GUIDE.md`, `docs/contracts/global-invocation/SHARED_GLOBAL_INVOCATION_CONTRACT.md`, `docs/policies/REVIEW_EFFORT_GUIDE.md`, and `docs/user_guide/GLOBAL_ADOPTION_PROCEDURE.md`. `docs/roadmap/` now holds **milestone routing only** (`INDEX.md`, `CURRENT_MILESTONES.md`); the relocated design docs each carry a top routing banner to their system STATUS, and current status lives in the system STATUS docs, not in the design docs.

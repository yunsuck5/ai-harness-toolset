...
- `-Mode inspect` / `-Mode verify` 는 **read-only** (mutation / 승인 없음). `-Mode update-source` 가 **유일한 mutation mode** 이며, payload delta 가 실제로 있을 때 §13.8 의 two-choice (Yes/No) 승인을 받은 뒤에만 payload 를 다시 쓴다.
...
| `update_aborted_no_approval` | update-source | yes | payload delta 가 있어 mutation 이 필요하나 two-choice 승인이 Yes 가 아니었음 (No / Esc / noninteractive). mutation 미수행. |
...
- **mutation 은 two-choice 승인 없이는 불가** → 승인이 Yes 가 아니면 `update_aborted_no_approval` (mutation 미수행). noninteractive context 는 자동 Yes 가 되지 않는다 (§13.8).
...
### 13.8 Mutation approval — exact two-choice (Yes/No) selector

`update-source` 가 실제 payload mutation 을 수행하기 전 (payload delta 가 있을 때), entrypoint 는 **정확히 두 선택지 (Yes / No)** 의 terminal selector 로 명시 승인을 받는다.

- 선택지는 **정확히 Yes 와 No 둘뿐** — `Other` / `Custom` / `Type something` / `Chat about this` 같은 제3 선택지를 노출하지 않는다.
- **default highlight = Yes**, 그러나 확정에는 **Enter 가 반드시 필요**하다 (default 가 자동 confirm 되지 않는다). Up/Down 이 highlight 를 이동하고, Enter 가 highlight 된 선택지를 confirm 하며, Esc 는 No 로 resolve 한다 (Ctrl+C 는 host default 의 abort — mutation 이전이므로 안전한 no-op abort).
- **timeout 으로 Yes 가 자동 선택되지 않는다.**
- **noninteractive / redirected stdin / CI context 는 자동 Yes 가 될 수 없다** — interactive terminal 이 아니면 승인은 No 로 resolve 되고 (`update_aborted_no_approval`), 어떤 환경 변수도 이를 auto-Yes 로 뒤집지 못한다.
- 본 selector 는 **mutation 승인 전용** 이다 — read-only `inspect` / `verify` 는 selector 를 호출하지 않는다.

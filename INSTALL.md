...
9. **activation_pending 보고.** update-source / verify 가 `activation_pending` (또는 activation-only `verify_failed`) 를 내면 — payload 는 정합이고 activation surface 만 drift 한 상태일 수 있다 (§13.2). payload update 성공과 activation follow-up 을 **구분해서** 보고하고, activation 의 실제 apply 는 별도 explicit 단계 (§2A phase 2 / §10) 임을 명시한다 — update-source 는 activation 을 자동 apply 하지 않는다.
...
- operator-created bootstrap clone 은 **성공 후 자동 삭제** 한다 — 삭제 여부를 불필요하게 사용자에게 다시 묻지 않는다.
- 실패 / 추가 조사가 필요해 보존해야 하면 그 **leftover path 와 이유를 보고** 한다 (자동 재삭제하지 않고 사용자 결정에 맡긴다). preserved debug evidence 를 남기는 경우에도 동일하게 path + 이유를 명시한다.
...
| **Activation refresh / apply** (§2A phase 2, §10) | no | yes — managed block / skill mirror write | **별도 explicit** user-approved 단계 (command-implied 적용 안 됨) | `scripts/activate-global.ps1` + `scripts/apply-managed-block.ps1` | activation surface 적용 + verify |
...
- **materialization (deterministic, managed).** fresh install 과 update-source **둘 다** canonical pipeline (`Invoke-InstallPipelineDispatch`) 이 materialization 직후 in-payload template 에서 `<InstallArea>/README.md` 로 deterministic overwrite 한다. 따라서 update-source 마다 latest template 으로 갱신되며 old / orphan 상태로 남지 않는다. **payload 가 이미 current 라 update-source 가 payload 재작성을 건너뛰는 경우 (`noop_already_current` / activation-only `activation_pending`) 에도, drift / missing 된 root README 는 (이미 검증된 current `current/` template 기준으로) in-place 로 healing 된다** — 따라서 "re-run update-source" 가 README-only drift 도 실제로 복구한다 (re-clone 없이). README repair 자체는 status / exit-code 를 바꾸지 않고 `reasons` 에 보고된다.
- **manifest / verify coverage (managed artifact).** template 자체는 manifest-covered 이고, root `README.md` 는 canonical verify (`Invoke-InstallPipelineVerify`) 가 in-payload template 과의 **byte-identity (SHA-256)** 로 검사한다 — manifest schema 변경 없이 transitive integrity 를 얻는 별도 managed-root-artifact verification path 다. **missing / stale / corrupted root README 는 verify failure** (`verify_failed`) 로 드러난다. payload manifest / marker / cross-binding invariant 와 `installedHead` / `lastUpdatedHead` semantics 는 바뀌지 않는다.
...
- **command-implied / activation 범위 불변.** 본 절은 새 artifact 의 materialize/verify 만 추가하며 status / exit-code vocabulary, command-implied approval 범위, activation apply (별도 explicit step) 를 바꾸지 않는다.
...
|---|---|---|---|
| `inspect_clean` | inspect | yes | source HEAD == `install.json.lastUpdatedHead` AND `payload-manifest.json.head` == `payload-marker.json.head` == `install.json.lastUpdatedHead` (cross-binding ok) AND 3 activation surface (Claude user-global managed block, Codex user-global managed block, installed skill mirror) 모두 destination 에 존재 + byte-identical 한 상태. 셋 중 하나라도 fail 이면 본 status 가 아니다. |
| `inspect_payload_drift` | inspect | yes | manifest / marker / install.json 의 cross-binding 불일치 또는 `current/` payload root 부재. |
...
| `verify_pass` | verify | yes | 14-field schema ok + manifest digest ok + cross-binding ok + 3 activation surface 모두 byte-identical. 어느 하나라도 fail 이면 `verify_failed`. |
| `verify_failed` | verify | yes | cross-binding 또는 manifest digest 또는 activation byte-identity 어느 하나라도 fail. reasons array 에 구체 reason 명시. |
...
| `activation_applied_verify_failed` | future | yes | activation apply 후 verify 실패 (rollback 후 final status). activation apply mutation 도입 batch 의 vocabulary; update-source 는 activation 을 apply 하지 않으므로 emit 하지 않는다. |

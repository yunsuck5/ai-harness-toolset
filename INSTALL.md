...
- **install 실행 중 repo `docs/` 를 열지 않는다.** Claude Code 는 install / update / operational install 을 완료하기 위해 repo `docs/` 문서를 읽을 필요가 없으며, 읽어서 install 동작을 결정해서도 안 된다. 사용자가 명시적으로 design / background review 를 요청한 경우에만 그 문서들을 연다 — 그것은 install 실행이 아니라 별도의 review 작업이다.
...
- **PowerShell.** install / update script의 검증·지원 기준은 Windows 기본 제공 **Windows PowerShell 5.1**이다. 다른 PowerShell host에 대한 지원 또는 비지원은 이 계약이 주장하지 않는다.
- **`%USERPROFILE%\ai-harness-toolset\` (install area) 와 `%USERPROFILE%\.claude\` / `%USERPROFILE%\.codex\` (activation home) 의 사용자 read / write 권한.** materialized runtime ToolRoot 는 install area (`%USERPROFILE%\ai-harness-toolset\current\`) 아래에 만들어지고, activation surface 는 vendor 별 activation home 아래에 만들어진다.
...
install 의 결과는 **global install layer 안의 deterministic runtime payload** 와 그것에 정합 binding 된 metadata / integrity artifact 들이다. 벤더-중립 install area (`%USERPROFILE%\ai-harness-toolset\`) 아래에 다음 항목이 생성 / 갱신된다.

- **runtime payload directory** — `%USERPROFILE%\ai-harness-toolset\current\` 아래에 `config/` / `scripts/` / `snippets/` / `templates/` 의 deterministic overwrite materialization.
...
1. **base payload install** — `%USERPROFILE%\ai-harness-toolset\` 아래에 §2 의 네 항목 (`current/` + `install.json` + `payload-manifest.json` + `payload-marker.json`) 을 materialize 한다 (§4–§9 의 install model 본체).
2. **activation apply** — 실제 운용에 필요한 **global / user integration surface** 를 적용한다. operational install 의 activation 은 §2 footprint 규칙 (target project 의 persistent footprint = `<ProjectRoot>/log/` only) 과 정합하도록 **global / user 영역의 surface 만** 대상으로 한다. default plan 은 아래 surface 들을 **모두** 포함하며 (skill adoption 은 각 source skill을 Claude/Codex vendor home에 하나씩 mirror — 현재 ship 되는 것은 `ai-harness-review` · `ai-harness-brief` · `ai-harness-consultation` · `ai-harness-blind-advisory` 넷 — 모두 같은 source에서 vendor별 mirror·verify 대상), phase 1 / 3 / 4 / 5 와 함께 하나의 full operational install plan 으로 묶여 단일 yes/no 로 승인된다 (아래 "Default install UX"). operator 는 사용자에게 어느 surface 를 설치할지 고르게 묻지 않는다.
   ...
   - **vendor skill adoption** — 각 source skill `snippets/claude-skills/<name>/SKILL.md` 를 user-global `%USERPROFILE%\.claude\skills\<name>\SKILL.md`와 Codex home의 `skills\<name>\SKILL.md`로 각각 install / update한다. Codex home은 `%CODEX_HOME%`이 설정되면 그 값을, 아니면 `%USERPROFILE%\.codex`를 쓴다(현재 ship: `ai-harness-review` + `ai-harness-brief` + `ai-harness-consultation` + `ai-harness-blind-advisory` 넷; §10 의 skill adoption 규칙).
...
plain "설치해줘" (또는 동등한 install 의도) 에 대한 default 는 위 5 phase 전체 (base payload + Claude managed-block + Codex managed-block + 각 source skill의 vendor별 mirror + activation verify + operational smoke + cleanup) 를 수행하는 **full operational install** 이다. 보통 사용자에게 노출되는 install UX 는 다음과 같다.

1. operator 는 발생할 **global / user mutation 전체 목록**을 inspect 해서 사용자에게 한 번에 설명한다 — 최소한 다음을 묶어서 보여준다.
   - base payload → `%USERPROFILE%\ai-harness-toolset\` (`current/` + `install.json` + `payload-manifest.json` + `payload-marker.json`).
   ...
   - 각 source skill mirror → user-global `%USERPROFILE%\.claude\skills\<name>\SKILL.md` + Codex home의 `skills\<name>\SKILL.md` (현재 ship: `ai-harness-review` + `ai-harness-brief` + `ai-harness-consultation` + `ai-harness-blind-advisory` 넷).
...
- **global install layer** — operational install 의 **mutation target.** 두 영역으로 나뉜다: (a) 벤더-중립 **install area** `%USERPROFILE%\ai-harness-toolset\` (base payload — `current/` + metadata / integrity artifact + root README), (b) 벤더별 **activation home** `%USERPROFILE%\.claude\` / (Codex integration 의 경우) `%USERPROFILE%\.codex\` (activation surface — managed block / skill mirror). base payload 는 (a) 에, activation surface 는 (b) 에 만들어지며, 모든 write 는 §10 의 explicit user-approved global / user filesystem mutation scope 다.
...
- **Destination.** runtime payload 의 자리는 `%USERPROFILE%\ai-harness-toolset\current\` (sibling `install.json` 포함) 다. 어느 source 도 다른 destination 으로 가지 않는다.
...
1. **Inspect.** operator 는 현재 호스트 상태를 확인한다 — Claude Code / git / PowerShell 가용 여부, `%USERPROFILE%\ai-harness-toolset\current\` 의 존재 여부, sibling `install.json` 의 존재 / schema / 내용, source repo (URL clone 위치 또는 local clone path) 의 유효성, source 의 현재 HEAD SHA.
...
**install-pipeline 의 CLI surface 는 fixture / test harness 다 (production global installer 가 아니다).** install / update / restore deterministic core contract — resolver tuple → materialization → install metadata / `payload-manifest.json` / `payload-marker.json` write → verify — 의 회귀를 보호하는 **fixture-only deterministic core validation entry** 는 `tests/support/install-pipeline-fixture.ps1` 에 위치한다 (구버전의 `scripts/install-pipeline.ps1` 에서 동일 코드가 이동했고, path 와 이름이 fixture 역할을 self-document 한다). 본 entry 의 `Assert-NotForbiddenInstallArea` guard 가 `-InstallArea` 가 activation home (`%USERPROFILE%\.claude` / `%USERPROFILE%\.codex`) 또는 그 descendants, **또는** 벤더-중립 global stable install area (`%USERPROFILE%\ai-harness-toolset\`) 또는 그 아래와 일치할 때 fail-fast 거부하므로 — 본 CLI 로는 actual global / user filesystem apply 를 수행할 수 없다 (구조적 boundary). (install area 는 activation home 의 descendant 가 아니라 별도의 forbidden root 로 취급된다.) 실제 global / user filesystem apply — `%USERPROFILE%\ai-harness-toolset\current\` materialize / refresh, sibling install metadata / `payload-manifest.json` / `payload-marker.json` write, managed-block apply, vendor별 skill install / update / removal — 는 §10 의 explicit user-approved scope 위에서 **AI-guided global apply** 로 수행되며, 본 CLI 가 그 apply 를 묶음으로 자동 수행하지 않는다 (§11 out-of-scope, productized installer / one-liner wrapper 미도입). 이 apply 의 operative 규칙은 본 INSTALL.md 안에 self-contained 하다 — 예컨대 managed-block 은 §10 의 Managed-block apply 규칙, mutation class 구분은 §9.1, name-based update 의 적용 단계는 §7.1 quickstart, 그리고 그 규칙을 구동하는 `scripts/apply-managed-block.ps1` / `scripts/activate-global.ps1` 가 그 home 이며, 도메인 spec `docs/install-update/install-update_spec.md` 와 git history 에 보존된 설계 기록 (구 STEP3 결정 가이드 §19.4) 은 그 설계 배경 / 결정 기록 (background / record) 일 뿐 operative authority 가 아니다 (상단 anti-coupling 절 / §12 와 정합). production 영역에서 verify 호출은 `scripts/lib/install-pipeline-core.ps1` 의 `Invoke-InstallPipelineVerify` 함수 (canonical library helper) 를 직접 사용한다 — 아래 첫 bullet 이 그 verify helper 의 우선 사용을 가리킨다.
...
3. operator 는 §5 propose 를 수행한다. propose 에는 다음이 포함된다.
   ...
   - 생성될 destination path (`%USERPROFILE%\ai-harness-toolset\current\`).
...
본 단계의 어떤 부분도 자동 PATH 변경, managed-block apply (`%USERPROFILE%\.claude\CLAUDE.md` 등), vendor skill mirror의 자동 설치, target project 의 자동 변경을 포함하지 않는다 — 단, 위 base payload + activation / smoke / cleanup 은 §2A 의 단일 yes/no 로 승인된 plan 안에서 결정적 진입점이 수행한다 (아래 §6.1).
...
- 기본 `-InstallArea` 는 `%USERPROFILE%\ai-harness-toolset` (install ROOT — `current/` 의 부모). source 는 `-SourcePath` (local-clone) **또는** `-RepoUrl` (git-url) 중 **정확히 하나** 다.
- **이미 install 이 있으면 fail-fast.** `-InstallArea` 에 `install.json` 이 이미 있으면 (혹은 비어 있지 않으면) install-global 은 **overwrite 하지 않고 거부** 하며 `install.json.installMode`를 먼저 읽도록 안내한다. git-url이면 `scripts/update-global.ps1` 또는 §7.1.1의 compat 경로에 exact advertised 40-hex branch-tip `-Ref`와 non-empty recorded `-Branch` 중 정확히 하나를 명시한다(branch가 empty이면 `-Ref`만 사용). local-clone이면 두 selector를 모두 넘기지 않는다. clean-reinstall / overwrite 옵션은 본 진입점에 두지 않는다 (별도 future 결정).
...
1. **install.json을 읽고 target을 명시적으로 고른다.** installed `install.json` 의 `installMode` / `repoUrl` (또는 `sourcePath`) / `branch` / `remote` / `lastUpdatedHead` 를 읽는다. git-url에서 recorded branch가 non-empty이면 그 full `refs/heads/<branch>`의 advertised HEAD를 exact 40-hex SHA로 고정하고, one-shot `-Ref` 또는 같은 recorded `-Branch`를 선택한다. branch가 empty이면 remote symbolic/default HEAD의 SHA가 현재 `refs/heads/*` 중 하나로 광고되는지 확인해 exact `-Ref`만 사용하며 branch 이름을 추정·persist하지 않는다. install.json 이 부재하거나 §4의 14-field schema와 mismatch이면 본 path가 적용되지 않는다 — operator는 §6 fresh install 절차로 분기하고 상황을 보고한다.
2. **temporary work area에서 source를 획득하고 exact target에 결박한다.** `installMode == git-url` 이면 §2 temporary work area에 clone한 뒤 `git -C <bootstrap-clone> checkout --detach <resolved SHA>`를 수행하고 `git -C <bootstrap-clone> rev-parse HEAD`가 그 SHA와 정확히 같은지 확인한다. 이 equality가 확인되기 전에는 clone 안의 `INSTALL.md`나 script를 읽거나 실행하지 않는다. `installMode == local-clone` 은 사용자 기존 clone path를 source로 사용한다 — §7 본문의 standing rule이 그대로 적용된다.
3. **검증된 target의 INSTALL.md만 operative contract로 재채택한다.** 위에서 HEAD equality를 확인한 exact checkout의 `INSTALL.md`를 update의 operative contract로 **재채택**한다. 본 step 이전까지 payload materialize / activation apply / verification / smoke / cleanup 어느 phase도 시작하지 않는다. moving `-Branch`를 택했고 inspect가 bootstrap SHA와 다른 SHA를 resolve하면 stale bootstrap으로 중단하고 새 target을 acquire·verify·재채택한다.
4. **Propose phase self-check (single line).** operator 는 사용자에게 한 줄을 surface 한다 — "INSTALL.md @ `<resolved-head>` read; operative contract 적용." 본 surface 는 사용자가 operator 의 contract re-adoption 을 확인할 수 있는 minimal evidence 다. 사용자는 본 보고와 propose 의 전체 mutation 목록 위에서 yes/no 를 결정한다 — name-based update 의 이 승인은 §7.1.1 / §13.8 의 **command-implied approval (payload-only)** 이고, 그 전제는 operator 의 **read-only inspect / plan-report** (mutation 이전의 prev HEAD → resolved HEAD / 변경 surface / destination 보고) 다. fresh / full install 의 §2A "Default install UX" single yes/no 가 payload + activation 을 **하나의 승인으로 번들** 하는 것과 달리, name-based update 에서 activation surface 의 실제 apply 는 이 승인에 포함되지 않고 §2A phase 2 / §10 의 **별도 explicit approval 단계** 다 (step 5 / §7.2 matrix).
5. **같은 explicit target으로 진행한다(activation scope는 §7.2).** 사용자가 yes 면 inspect와 update에 같은 git-url `-Ref <resolved 40-hex SHA>`(권장) 또는 같은 explicit recorded `-Branch`를 전달하고 §2 / §4 / §5 / §7 의 deterministic overwrite materialization (payload) → §5 verify → operational smoke → cleanup 순으로 진행한다. payload 단계의 승인은 §7.1.1 / §13.8 의 command-implied approval (update-source) 이며 **payload-only** 다. activation surface 는 update-source 가 byte-identity verify 만 하고, drift 가 있으면 `activation_pending` 으로 보고한다 — 그 activation 의 실제 apply 는 §2A phase 2 / §10 의 **별도 explicit 단계** 다 (fresh / full install 의 single yes/no activation bundling 과 구분된다; §7.2 matrix). 즉 name-based update 는 fresh install 처럼 activation 을 같은 승인으로 자동 apply 하지 않는다 — payload update 성공과 activation follow-up 을 구분해서 보고한다.

#### Name-based update quickstart (self-contained)

사용자가 자연어로 update 를 요청할 때 — 예: `ai-harness-toolset 최신버전으로 업데이트해줘` — operator 는 다음 체크리스트를 따른다. 본 quickstart 는 위 5-step bootstrap 의 실행 순서이며 self-contained 다 (외부 문서 참조 없이 본 절 + §7.1.1 + §7.2 + §13 으로 닫힌다). git-url target selector는 `-Ref` 또는 `-Branch` 중 정확히 하나다.
...
2. **remote/source HEAD를 확인하고 target을 고정한다.** `git-url`에서 recorded branch가 non-empty이면 그 exact branch ref의 advertised HEAD SHA를 확인해 이후 inspect/update의 `-Ref`로 고정한다. branch가 empty이면 remote symbolic/default HEAD의 SHA가 현재 advertised branch tip인지 확인해 `-Ref`만 사용하고 branch를 새로 기록하지 않는다. branch tracking을 의도하면 non-empty recorded branch와 같은 값을 `-Branch`로 명시할 수 있다. `local-clone`이면 사용자 clone의 HEAD SHA를 `lastUpdatedHead`와 비교한다.
3. **selected source를 clone하고 exact checkout을 검증한다.** `git-url`이면 §2 temporary work area에 clone하고 `git -C <bootstrap-clone> checkout --detach <selected SHA>`를 실행한 뒤 `git -C <bootstrap-clone> rev-parse HEAD` 출력이 selected SHA와 같은지 확인한다. equality 확인 전에는 그 clone의 contract/script를 소비하지 않는다. `local-clone`이면 사용자 기존 clone을 source로 쓴다.
4. **검증된 target의 INSTALL.md를 재채택한다.** exact HEAD equality가 확인된 source의 `INSTALL.md`만 operative contract로 채택한다 (§7.1 step 3). moving `-Branch` preflight가 다른 SHA를 resolve하면 새 target으로 step 2–4를 다시 수행하며, 그 전에는 mutation phase를 시작하지 않는다.
5. **inspect (read-only).** **clone 한** source 의 `scripts/install-update.ps1 -Mode inspect -InstallArea <global install area> -Ref <resolved 40-hex SHA>` 로 prev HEAD → resolved HEAD / payload·activation drift 를 chat 에 보고한다. branch tracking을 택했다면 `-Ref` 대신 `-Branch <recorded branch>`를 쓰며 둘을 함께 쓰지 않는다.
6. **update (payload mutation) — operator-facing 진입점은 `scripts/update-global.ps1` 다.** **clone 한** source 의 `scripts/update-global.ps1 -InstallArea <global install area> -Ref <the same resolved 40-hex SHA>` 를 호출한다 — 이 호출 자체가 command-implied approval 이다 (§7.1.1 / §13.8). branch tracking을 택했다면 inspect와 같은 `-Branch <recorded branch>`를 전달한다. selector를 생략하거나 둘을 함께 지정하지 않는다. `update-global.ps1` 은 thin wrapper 로, 기존 valid install 이 아니면 fail-fast 하여 `install-global.ps1` (§6.1) 로 안내하고, 맞으면 underlying `install-update.ps1 -Mode update-source` 로 delegate 한다 (payload — `current/` + 세 sibling 파일 — 만 갱신, activation surface 는 byte-identity verify only; logic 동일, 이름만 operator-facing). (compat / 직접 경로: 같은 selector와 함께 `scripts/install-update.ps1 -Mode update-source -InstallArea <global install area>` 를 호출해도 된다 — 그것이 update-global 이 감싸는 canonical implementation 이다.)
...
9. **activation_pending 보고.** update-source / verify 가 `activation_pending` (또는 activation-only `verify_failed`) 를 내면 — payload 는 정합이고 activation surface 만 drift 한 상태일 수 있다 (§13.2). 이것은 **payload 실패가 아니라 follow-up** 이다: human 보고는 `payload=ok` / `activation=pending` / `result=INCOMPLETE (payload OK; activation follow-up required)` 로 hard failure 처럼 보이지 않게 하고 (human label 도 `FAIL` 이 아니라 `INCOMPLETE`; machine status·exitCode 는 그대로 — §13.2), activation 의 실제 apply 는 별도 explicit 단계 (§2A phase 2 / §10) 임을 명시한다 — update-source 는 activation 을 자동 apply 하지 않는다. update-source 는 activation drift 시 복붙 가능한 exact next command 를 출력한다 — installed context 기준:
   ...
   - apply 는 global/user instruction file 을 mutate 한다 — managed-block surface (Claude / Codex) 는 `.amb-backup` rollback backup 을 만들고, skill mirror (source skill당 vendor별 하나) 는 backup 없는 whole-file canonical-overwrite 다 (§10). dry-run 은 managed-block 에 대해 기본적으로 **compact change summary** 를 내며, 전체 before/after 가 필요하면 `-ShowFullDiff` 를 붙인다.

> **`-InstallArea` 는 install ROOT 이지 `current/` 가 아니다.** 위 모든 `-InstallArea <global install area>` 인자는 `current/` + `install.json` + `payload-manifest.json` + `payload-marker.json` 을 **포함하는** 디렉터리 (예: `%USERPROFILE%\ai-harness-toolset`) 다. `current/` 를 넘기면 `install.json` 을 찾지 못해 `inspect_mode_unknown` 으로 분류되며, "did you mean its parent" hint 가 reasons 에 붙는다. update-source 는 넘긴 InstallArea 를 mutate 하므로 default 추론을 두지 않는다 (잘못된 default 가 실제 global install 을 mutate 하는 것을 막기 위함).
...
- **git-url target selector는 explicit XOR다.** `inspect`와 `update-source`는 `-Ref <advertised 40-hex branch-tip commit>` 또는 `-Branch <recorded branch>` 중 정확히 하나를 요구한다. selector 생략·동시 지정, symbolic/short `-Ref`, git-url `-SourcePath`, inspect/preflight 시점에 remote branch tip으로 광고되지 않는 exact SHA는 source resolve failure로 닫고 persistent install을 바꾸지 않는다. apply는 그 preflight SHA가 fresh clone에서 available하고 동일한지만 확인하며 apply 시점의 tip 재광고는 요구하지 않는다. `-Ref`는 one-shot payload target으로 `install.json.branch`/`remote`를 빈 값까지 보존한다. recorded branch가 empty이면 remote default HEAD의 advertised tip을 exact `-Ref`로만 선택하며 branch를 추정·persist하지 않는다. 다른 `-Branch`/`-Remote`는 기존 source-cut이다.
...
- source-root D3 marker 검증과 payload archive는 clone의 default checkout이 아니라 같은 selected `resolvedRefSha` Git tree를 본다. marker 3개 중 하나라도 그 tree에서 blob이 아니면 canonical output mutation 전에 실패하고, 통과하면 D3를 검사한 바로 그 exact SHA를 `git archive` materialization input으로 쓴다.
...
- **apply coverage == verify coverage (Phase 4a).** `update-source` / `verify` 가 검사하는 **모든 activation surface** (Claude managed block, Codex managed block, source skill당 Claude/Codex mirror) 를 `scripts/activate-global.ps1` 가 모두 **apply** 한다 (이전에는 managed block 2 개만 apply 했고 skill mirror 는 수동이었다). apply 와 verify 는 동일한 surface resolver (`scripts/lib/activation-surface.ps1`) 를 공유하므로 destination 해석 (특히 Codex `AGENTS.override.md` 우선순위) 이 어긋나지 않는다. activate-global 은 여전히 **별도 explicit step** 이며 update-source 가 자동 호출하지 않는다.
...
- **Vendor skill activation surface (canonical-overwrite class)** — 각 source skill의 destination은 `%USERPROFILE%\.claude\skills\<name>\SKILL.md`와 Codex home의 `skills\<name>\SKILL.md`다. 이것은 managed-block file 이 **아니라** canonical-overwrite activation surface 다. 적용은 `scripts/activate-global.ps1` 의 canonical-overwrite path 가 source `snippets/claude-skills/<name>/SKILL.md` → 두 destination 으로 **whole-file byte overwrite + post-write SHA-256 byte/hash verify** 하며, 사용자가 destination 을 수정해 두었다면 overwrite 로 그 수정이 사라진다는 사실을 **사전 고지** 한다 (dry-run preview 가 source/destination hash 와 `create | overwrite | unchanged` action + overwrite 고지를 보여준다). skill 은 marker-bounded 부분 치환이 아니고 **pre-write backup / rollback / backup sidecar 의 대상이 아니다** — read-only dry-run preview 는 있으나 backup / rollback / sidecar 는 두지 않는다. post-write verify 가 실패하면 rollback 하지 않고 fail-fast + report + reinstall guidance 로 닫는다 (canonical source 가 곧 회복원).
...
- **Global / user filesystem mutation** — 실제 `%USERPROFILE%\ai-harness-toolset\current\` materialize / refresh, `install.json` write, `%USERPROFILE%\.claude\` / `%USERPROFILE%\.codex\` 어느 경로의 write 도 **explicit user-approved global / user filesystem mutation scope** 다. 본 문서를 읽은 사실, operator 가 propose 한 사실, 어떤 자동화 trigger 의 fact 어느 것도 그 승인을 대체하지 않는다.
...
- **Vendor skill adoption** — `snippets/claude-skills/<name>/SKILL.md` 의 두 사용자 글로벌 vendor destination으로의 install / update / removal 은 install-pipeline automation 본체 **밖** 의 explicit user-approved 동작이지만, §2A operational install 의 **activation phase (phase 2)** 다. default install 에서 이 변경은 §2A 의 단일 yes/no 에 포함되어 함께 승인되며 (surface 별 재질문 없음), base payload verify 가 자동 승인하지 않는다.
...
- **source / destinations.** source = install artifact `snippets/claude-skills/<name>/SKILL.md`. operational install 의 destinations = Claude user-global `%USERPROFILE%\.claude\skills\<name>\SKILL.md`와 Codex home의 `skills\<name>\SKILL.md`다. Codex home은 `%CODEX_HOME%`이 설정되면 그 값을, 아니면 `%USERPROFILE%\.codex`를 쓴다. project-local `<ProjectRoot>/.claude/skills/<name>/SKILL.md` 또는 `<ProjectRoot>/.codex/skills/<name>/SKILL.md`는 project-specific adoption 으로 operational install phase 가 아니다. 두 vendor 모두 폴더 이름 `<name>`(예: `ai-harness-review`)을 그대로 유지한다.
- **first install** (vendor destination dir 부재): 각 vendor의 `skills` 부모 dir 부재 시 그 생성도 skill activation 의 일부로 propose 목록에 포함한다(default operational install 에서는 §2A 단일 yes/no 가 cover 하며 별도 prompt 를 추가하지 않는다) → 승인된 plan 적용 시 각 `<name>/` dir 생성 → 같은 source `SKILL.md` 복사(frontmatter 임의 수정 금지) → 각 destination content 가 source 와 일치하는지 verify.
- **update** (vendor destination 존재): 각 destination 을 source 와 독립적으로 hash 비교한다. 동일한 destination 은 변경 없음을 보고하고, 다른 destination 만 diff propose → 승인 시 `SKILL.md` 전체 교체(partial merge 금지) → hash 일치 verify. 사용자가 destination 을 수정해 두었다면 교체로 그 수정이 사라짐을 사전 고지.
- **removal**: 양 vendor에서 toolset이 소유한 삭제 대상 `<name>/` dir와 그 안의 파일 목록을 propose → 승인 시 그 owned dir만 삭제(dir 밖 파일과 동명 아닌 sibling skill 미삭제) → 각 destination 부재 verify. source repo의 `snippets/claude-skills/<name>/`는 영향받지 않는다.

### Activation apply orchestration 규칙 (self-contained; Phase 4a)

`scripts/activate-global.ps1` 는 `update-source` / `verify` 가 검사하는 **activation surface 전부** (두 managed block + source skill당 vendor별 mirror) 를 적용하는 단일 explicit orchestrator 다 (apply coverage == verify coverage). apply 와 verify 는 동일한 surface resolver (`scripts/lib/activation-surface.ps1`) 를 공유하므로 destination 해석이 어긋나지 않는다.
...
- generated payload 에 대한 transaction log / rollback framework / tamper detection / partial-state reconciliation. generated payload 의 materialization atomicity / partial state 는 §9 / §9.1 의 reinstall-first 로 닫힌다. (managed-block instruction file 의 dry-run / pre-write backup / rollback / verification 은 §9.1 / §10 의 managed-block tooling 영역, canonical-overwrite surface 인 vendor skill mirror 의 whole-file overwrite + hash verification + read-only dry-run preview 는 §9.1 / §10 의 activation apply orchestration 영역으로 각각 별개이며, 본 out-of-scope 항목과 혼동하지 않는다. canonical-overwrite class (skill mirror 등) 에는 read-only dry-run preview 외의 pre-write backup / rollback / sidecar 를 두지 않으며, activation apply 도 cross-surface transaction / rollback framework 로 묶지 않는다.)
...
본 INSTALL.md 는 install / update / reinstall / operational install 을 수행하기 위한 self-contained operative contract 다 (상단 anti-coupling 절 참조). repo 의 `docs/` 트리는 model / decision 의 history / design / background material 일 뿐 **install-time input 이 아니다.** 본 절은 "install 중 읽어야 할 reference 목록" 이 아니다.
...
| `verify_pass` | verify | yes | 14-field schema ok + manifest digest ok + cross-binding ok + managed root README (§7.3) byte-identity ok (template-conditional — payload 에 template 이 있을 때만 검사) + 모든 activation surface (두 managed block + source skill당 vendor별 mirror) byte-identical. 어느 하나라도 fail 이면 `verify_failed`. |
...
- **"payload updated 이나 activation 미적용" 은 `complete` 로 보고 금지** → `activation_pending`. update-source 가 mechanical 하게 enforce 한다 (activation surface — 두 managed block + source skill당 vendor별 mirror — 중 하나라도 byte-mismatch 면 `activation_pending`).
...
- **`update-source` mode 는 InstallArea payload (`current/` + `install.json` + `payload-manifest.json` + `payload-marker.json`) 를 write 한다.** 그 InstallArea 가 global install layer (`%USERPROFILE%\ai-harness-toolset\`) 일 때 이 write 는 §10 의 explicit user-approved global / user filesystem mutation scope 이며, 그 explicit approval surface 는 §13.8 의 **command-implied approval** (operator 의 명시 update-source 호출 + 사용자 명시 update 지시 + plan-report; optional `-ConfirmInteractive` 시 two-choice selector) 다. update-source 는 activation surface (managed block / skill mirror) 와 run evidence file 은 write 하지 않는다.
- run evidence file 의 future write destination (`%USERPROFILE%\ai-harness-toolset\log\install-update\<runId>\`) 의 도입은 별도 future scoped batch 이며, 그 actual write 도입 시 §10 explicit user-approved scope 가 별도 동작한다. run.json 의 contract 정의 자체가 §10 mutation list 를 자동 확장하지 않는다.

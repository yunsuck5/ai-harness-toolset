...
5. **Proceed with §2A full operational install flow.** 사용자가 yes 면 §2 / §4 / §5 / §7 의 deterministic overwrite materialization → §5 verify → §2A activation apply → activation verify → operational smoke → cleanup 순으로 진행한다. surface 별 재질문 없음 — §2A standing rule.

본 절은 §2A "Default install UX" 의 single yes/no 와 정합한다. name-based update entry 도 surface 별로 쪼개 묻지 않는다. cleanup 의무 / strict yes/no / activation apply 규칙은 §2A / §5 / §10 / §11 standing rule 그대로 적용된다.
...
- read-only inspect / verify mode 를 제공하는 source-controlled install/update entrypoint (`scripts/install-update.ps1`). default 동작은 어떤 mutation 도 수행하지 않는다 — no payload write, no managed-block write, no skill mirror write, no run evidence global write.
- fixed-vocabulary final status 의 stdout emit. status enumeration 과 그 의미는 §13 본문에 codify 된다.
- explicit-flag-gated future mutation path. 본 entrypoint 에 mutation flag (`-ApplyActivation`, `-UpdateSource`, `-ApplyPayload`, `-RefreshSkill` 등) 가 도입될 때 production guard 가 함께 도입되며, default 는 inspect / verify 로 유지된다. mutation flag 의 발현은 §10 Approval boundaries 와 정합한다 — 각 mutation 은 explicit user-approved scope 다.
- run evidence contract (§13). evidence shape / status vocabulary 의 docs codification 은 본 contract 안에 포함되며, 실제 file write 의 도입은 future scoped batch 영역이다.

### Self-imposed boundary 의 invariant

(b) class 의 entrypoint 는 (a) class 의 framework 로 자라지 않는다. 본 entrypoint 에 (a) 의 wizard / rollback / doctor / package-manager 동작이 추가 도입되면 그 도입은 본 §11 의 narrow scope 를 침범하므로 별도 scoped approval + 본 §11 자체의 재정의가 prerequisite 다. 본 boundary 의 침식을 막기 위해 entrypoint 의 production guard (mutation flag 의 default-off + 명시 거부 helper) 와 fixed-vocabulary status (§13) 가 source-side mechanism 으로 함께 동작한다.
...
| `noop_already_current` | future (update-source mode) | yes | source HEAD == `lastUpdatedHead`, payload 정합, activation 정합. mutation 불필요. 본 contract 에서는 vocabulary 만 정의; mutation path 도입 batch 에서 emit 가능. |
| `complete` | future (full update / install mode) | yes | payload update + activation apply + verify + smoke + cleanup 모두 ok. 본 contract 에서는 vocabulary 만 정의. |
| `activation_pending` | future | yes | payload updated 이나 activation 미적용 / 미승인. `complete` 로 보고 금지 invariant 의 직접 표현. |
| `activation_applied_verify_failed` | future | yes | activation apply 후 verify 실패 (rollback 후 final status). |
| `smoke_failed` | future | yes | operational smoke 실패. |
| `cleanup_failed_with_leftover` | future | yes | cleanup 실패 (leftover paths 존재). user approval prompt 로 전환 금지 — 본 status 로 보고. |
| `failed` | any | yes | 위 어느 분류에도 해당 안 되는 일반 실패. |

### 13.2 핵심 invariants

- **"payload updated 이나 activation 미적용" 은 `complete` 로 보고 금지** → `activation_pending`. 본 contract 에서는 docs codify; mutation path 도입 batch 에서 mechanical enforce.
- **cleanup 실패는 user approval prompt 로 전환 금지** → `cleanup_failed_with_leftover`. `leftoverPaths` 에 정리 미완료 path 열거.
- **status 는 단일 terminal value** — 한 run 에 두 status 를 emit 하지 않는다.
...
| `verify_failed`, `activation_pending`, `activation_applied_verify_failed`, `smoke_failed`, `cleanup_failed_with_leftover`, `failed` | 1 |

### 13.4 Run evidence contract — `run.json` schema

본 contract 에서는 entrypoint 가 어떤 file 도 write 하지 않는다. **stdout JSON body 는 future `run.json` 의 strict field-name subset** 이다 — stdout 이 emit 하는 field 이름은 모두 아래 `run.json` schema 에 존재하며, `run.json` 은 그 위에 (a) lifecycle field (`runId` / `startedAt` / `finishedAt`) 와 (b) mutation-outcome field (`payloadUpdated` / `activationRequired` / `activationApproved` / `activationApplied` / `managedBlockVerified` / `skillMirrorVerified` / `smoke` / `cleanup` / `leftoverPaths`) 를 추가한다. inspect / verify mode 는 (a)(b) 를 emit 하지 않으므로 stdout 은 진부분집합 (proper subset) 이다. future write destination 의 path 는 글로벌 install layer 안의 `%USERPROFILE%\.claude\ai-harness-toolset\log\install-update\<runId>\run.json` 이며, 실제 디렉터리는 본 contract 단계에서 생성되지 않는다.
...
- **run.json-only group** (`runId`, `startedAt`, `finishedAt`, `branch`, `remote`, `payloadUpdated`, `activationRequired`, `activationApproved`, `activationApplied`, `managedBlockVerified`, `skillMirrorVerified`, `smoke`, `cleanup`, `leftoverPaths`): future write 시점의 lifecycle + mutation-outcome. inspect / verify mode 의 stdout 은 이 group 을 emit 하지 않는다.
...
- run evidence file 의 future write destination (`%USERPROFILE%\.claude\ai-harness-toolset\log\install-update\<runId>\`) 의 도입은 §10 Approval boundaries 와 정합한다 — global / user filesystem mutation scope 다. 본 path 에 대한 actual write 는 별도 explicit user-approved scope.
- 본 contract 의 entrypoint 는 어떤 path 에도 file 을 write 하지 않으므로 본 boundary 와 충돌하지 않는다.
- run.json 의 도입이 §10 mutation list 를 자동으로 확장하지 않는다 — 실제 write 시점에 §10 의 explicit user-approved scope 가 별도 동작.

### 13.7 Pester 회귀 lock

본 절의 enumeration / shape / invariant 는 source repo 의 Pester suite 가 회귀 lock 으로 보호한다.

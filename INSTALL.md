...
- **install 실행 중 repo `docs/` 를 열지 않는다.** Claude Code 는 install / update / operational install 을 완료하기 위해 `docs/roadmap/**` 또는 그 밖의 repo 문서를 읽을 필요가 없으며, 읽어서 install 동작을 결정해서도 안 된다. 사용자가 명시적으로 design / background review 를 요청한 경우에만 그 문서들을 연다 — 그것은 install 실행이 아니라 별도의 review 작업이다.
...
본 INSTALL.md 는 install / update / reinstall / operational install 을 수행하기 위한 self-contained operative contract 다 (상단 anti-coupling 절 참조). repo 의 `docs/` 트리 — `docs/roadmap/**` 등 — 는 model / decision 의 history / design / background material 일 뿐 **install-time input 이 아니다.** 본 절은 "install 중 읽어야 할 reference 목록" 이 아니다.

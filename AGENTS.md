...
| **Source / docs** | **active rule — read before placing / changing docs / closing out: `rules/docs-working-model/docs-working-model.md`** (document artifact classes, Design/Plan/Spec lifecycle + Spec identity, end-state placement + transition, Spec↔implementation 1:1 sync + proportionality rule, on-demand briefing, reduced closeout gate); orientation only — `docs/README.md` (placement map + question routing) | 내용 변경이 모두 반영된 working tree의 Codex review; DWM-qualified retirement-only closeout은 reviewable content가 없다 | review 뒤 review-bound source-managed artifact set이 달라지거나 그 artifact의 source-managed bytes/content, path, Git mode, target-relevant meaning이 바뀌면 stale이므로 re-run; exact retirement-only closeout은 예외 |
| **Rule authority** (authoring / revising a rule, or handling a higher-rule conflict) | `rules/rule-authority.md` — authority classification and disposition; **do not load for ordinary work** | classify the exact clause × scope × enforcement path | disposition requires user decision; do not add an automatic downstream workaround |
| **Project terminology** (looking up an adopted meaning; introducing a project term; changing adopted meaning/classification (including de-adoption); resolving an observed name collision; preventing a rejected-term revival; or making a meaning-preserving glossary correction) | `rules/terminology-glossary.md` — minimal single home of adopted / rejected project-term meaning | read-only lookup is always available; only the four decision triggers mutate classification/meaning, while meaning-preserving correction follows proportionality; candidate-local labels stay with their owner | glossary grants no mutation / commit / push approval |
...
- **A repo surface that claims the structured native-capture contract must preserve stdout / stderr / exit code separately and expose exactly the case-insensitive PowerShell property set `{ ExitCode; Stdout; Stderr }`; `Invoke-NativeProcess` is the preferred default, not the sole conforming realization.** Simple stdout-only capture and admitted Step F raw-merged sites are weaker forms whose retention or migration is separate maintenance work. The active rule's Step F lint is a limited physical-line lexical guardrail over a narrower literal raw-merged shape in `tests/**/*.ps1`; its reason-bearing comment is a physical-line exception, and a Step F pass is neither AST proof nor broader compliance evidence. Under Windows PowerShell 5.1 with `$ErrorActionPreference = 'Stop'`, raw merged capture can abort before `$LASTEXITCODE` can be read.

...
- Directly inspect the declared target, required paths, and relevant repository context with available read-only tools; for a Git delta, inspect live status and the applicable working-tree and stated-base diffs. For a change that renames, moves, deletes, or otherwise changes a name, path, identifier, or structure, use read-only inspection to check residual references across filename/path, bare token/ID, folder-as-bucket, and semantic-phrasing classes; for deletions, include case-insensitive searches, spelling/case variants, and bare-section references. Do NOT silently repair packet defects, mutate, or redefine the intended target. Do NOT substitute a limitation for an available material read-only check.
- Every `no` must be supported by at least one self-proving blocking finding. Each blocker must identify (1) a concrete failure mode or path, or a direct contract or acceptance breach, (2) the affected consumer or decision, and (3) the failed function, outcome, or contract. Unknown, missing, ambiguous, or inaccessible material alone is not a blocker unless that absence itself breaches an explicit required contract.
...
- Read any load-bearing declared target directly. Do not proxy, inline, stage, or copy it into the workspace. For each load-bearing target, one rejected read/list is inconclusive; try one simpler direct read/list. If that target remains inaccessible, identify the required-inspection or target-completeness acceptance breach as a blocker and issue "no"; do not substitute "yes with risk" for the unavailable check.
"@
    }
    $reviewerPreamble += @'

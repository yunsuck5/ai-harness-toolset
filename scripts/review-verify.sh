#!/usr/bin/env bash
# Thin Bash invocation adapter for scripts/review-verify.ps1.
#
...
# as do input.md and result.md shape verification.
#
# Forwards PowerShell-style parameters unchanged (e.g. -ReviewTaskId, -Pass,
# -RequireResult). Performs minimal path normalization only for -ProjectRoot
# and -ToolRoot values so that CWD-relative or POSIX-rooted Bash inputs reach
# the .ps1 as absolute Windows-native paths.
...
target_ps1="$script_dir/review-verify.ps1"

if [ ! -f "$target_ps1" ]; then
    printf 'review-verify.sh: FAIL canonical script not found: %s\n' "$target_ps1" 1>&2
    exit 1
fi

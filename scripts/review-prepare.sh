#!/usr/bin/env bash
# Thin Bash invocation adapter for scripts/review-prepare.ps1.
#
...
#
# Forwards PowerShell-style parameters unchanged (e.g. -ReviewTaskId,
# -Pass, -Stage, -Purpose). Performs minimal path normalization only for
...
target_ps1="$script_dir/review-prepare.ps1"

if [ ! -f "$target_ps1" ]; then
    printf 'review-prepare.sh: FAIL canonical script not found: %s\n' "$target_ps1" 1>&2
    exit 1
fi

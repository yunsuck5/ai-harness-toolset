...
| **PowerShell / script** | `rules/powershell-and-file-encoding.md` (active rule — `.ps1` encoding / native-exe output separation / file-IO discipline); host prerequisites in `INSTALL.md` §1. (Rationale / derivation history: `docs/policies/POWERSHELL_POLICY.md` + `docs/policies/CLI_ENVIRONMENT_ASSUMPTIONS.md`.) | affected Pester + full suite; `scripts/verify-ps1.ps1` | see *Repo execution conventions* below; controlled IO via `scripts/lib/encoding.ps1` |
| **Repo-local instruction surface** (editing `CLAUDE.md` / `AGENTS.md`) | this file's *Mirror-edit rule* + *Public-safe boundary* sections (active owner) and `tests/repo-local-instruction-parity.Tests.ps1` (parity contract). (Design / audit history: `docs/architecture/instruction-surface/REPO_LOCAL_INSTRUCTION_SURFACE_PLAN.md` + `GLOBAL_SNIPPET_RELOCATION_AUDIT.md`, git history.) | Codex review; `tests/repo-local-instruction-parity.Tests.ps1` | edit the shared body of **both** files symmetrically (*Mirror-edit rule*); tracked = public-safe only |

...
- **Where to read first** for any subsystem — the orientation / routing map — is `docs/**` and `docs/contracts/**`; this file only says **when to read which doc**, never duplicating doc bodies. These are read-first routing / context pointers, **not** active-behavior authority (per the *Final hard rule*). Start question→doc routing at `docs/current/SOURCE_OF_TRUTH.md`; placement at `docs/README.md`.
...
| **Source / docs** | **active rule — read before changing docs / closing out: `rules/docs-working-model.md`** (top-down flow, single-home, STATUS/BACKLOG shape, on-demand briefing, two-level closeout gate); orientation/rationale only — `docs/README.md` (placement), `docs/current/SOURCE_OF_TRUTH.md` (routing), `docs/policies/DOCS_OPERATING_MODEL.md` (why/record + layer map) | Codex review on the **corrected** working tree | a source/doc edit **after** a review makes that review stale — re-run |

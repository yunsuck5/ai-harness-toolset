...
| **Snippet** (global payload) | **Controlling migration authority:** `docs/architecture/instruction-surface/GLOBAL_SNIPPET_FIRST_MIGRATION_DESIGN.md` + `GLOBAL_SNIPPET_FIRST_MIGRATION_PLAN.md` (keep-by-proof default; existing docs are evidence, not blocking). **Evidence (per-section record until a migration batch lands):** `INSTRUCTION_SURFACE_PLAN.md` + `GLOBAL_SNIPPET_RELOCATION_AUDIT.md`; `docs/systems/skills/FUNCTION_LEVEL_SKILL_ARCHITECTURE_PLAN.md` + `STATUS.md` | full Pester (`Invoke-Pester -Path .\tests`); Codex local-correctness + system-coherence | re-judge each retained sentence under the migration keep-by-proof + direct owner-surface absorption; **no snippet minimization without an approved migration batch (spec)**; **no** skill-routing pointers; keep CLAUDE/AGENTS snippet symmetry |

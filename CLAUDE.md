...
These files are **tracked** (public to any contributor), so they carry only public-safe repo guidance. **Never** put in them: secrets / tokens / credentials; personal or machine-specific paths or state; local model endpoints; session-restore state or `log/brief/BRIEF.md` contents; private handoff or user decision history; runtime evidence or `log/**` payloads. Durable pointers resolve only to git-tracked files or git history — never to `log/**`, `polishing/**`, or user/global files (durable-pointer rule: `rules/docs-working-model/docs-working-model.md`, *Per-system STATUS.md shape / altitude*).

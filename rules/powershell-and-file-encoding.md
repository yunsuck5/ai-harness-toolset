...
- **Partial Step F guardrail:** Step F is a limited physical-line lexical guardrail over `tests/**/*.ps1`. It reports a line matching the literal single-line `& <native> ... 2>&1` shape unless that physical line carries the lint's explicit, reason-bearing comment pragma or a recognized non-capture discard form. It is not an AST or capture-site proof: a Step F pass means only that the lexical matcher reported no violation and is not evidence that other native-capture forms satisfy the structured contract above.

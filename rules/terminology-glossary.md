...
- **`global-invocation single-home`** — candidate (the single home of the global-invocation contract). Owner = an **install-update follow-on batch**; close = that batch. *(Facet note: pending sub-question facet; the broad-domain facet is rejected — see Rejected terms.)*

...
- **`Plan`** — decomposes a Design into batch order / scope / hard boundaries / validation expectations / review focus / Work Packet declarations — **approval-target decisions only**, never a work memo (execution detail belongs in a Work Packet).
...
- **`stable filename rule`** — domain documents use domain-prefixed role filenames (`<domain>_design.md` / `_plan.md` / `_spec.md` / `_backlog.md`), reused after deletion; topic-named files and filename-evading subfolder splitting are forbidden. Operative home: `rules/docs-working-model/docs-working-model.md`.
- **`Work Packet`** — a round-scoped, non-approval-target execution aid (application order, investigation results, candidate-file worklists, reviewer questions) living under `<ProjectRoot>/log/work/<topic>/`; never committed to the repo, absorbed into the Spec / owner surface / closeout report at closeout, then deleted. Operative home: `rules/docs-working-model/docs-working-model.md`.
...
- **`global-invocation single-home`** — candidate (the single home of the global-invocation contract). Owner = a **review / install-update follow-on batch**; close = that batch. *(Facet note: pending sub-question facet; the broad-domain facet is rejected — see Rejected terms.)*
- **`evidence contract absorption`** — candidate (folding the evidence contract into its owner). Owner = the **review pilot**; close = the review pilot Design / Spec. *(Facet note: pending sub-question facet; the broad-umbrella facet is rejected — see Rejected terms.)*
...
A broad standalone "global-invocation" domain. Rejected for the same broad-bucket reason. Do not revive as an independent domain. (Narrow pending facet: `global-invocation single-home`.)

...
- **`Plan`** — decomposes a Design into batches / scope / hard boundaries / validation / review gate.
- **`Spec`** — the implementation boundary: allowed vs forbidden active-surface changes, validation + review criteria, reconstructibility; live after closeout.
...
- **`stable filename rule`** — Design / Plan / Spec for a domain use domain-prefixed role filenames (`<domain>_design.md` / `_plan.md` / `_spec.md`), reused after deletion; topic-named files and filename-evading subfolder splitting are forbidden. Operative home: `rules/docs-working-model/docs-working-model.md`.

...
규범적 rule을 새로 저작하거나 기존 rule의 의미를 개정하려는 저작자는, 저작을 시작하기 전에 현재 작업 안에서 식별된 관련 정책급 전제, 영향을 받는 foreign owner, 이미 알려진 반례가 있는지 최소 한 번 대조한다. 오탈자·stale pointer·그 밖의 meaning-preserving correction에는 이 대조를 별도 절차로 요구하지 않는다.

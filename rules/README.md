...
- [docs-working-model.md](docs-working-model.md) — the docs change/closeout working model: top-down flow, single-home-plus-pointers, per-system `STATUS.md` shape / altitude, `BACKLOG.md` closed-row tombstone, the durable-pointer prohibition, the on-demand status-briefing model, and the two-level closeout reconciliation gate. Triggered before a docs change/closeout by the root `CLAUDE.md` / `AGENTS.md` *Docs trigger map* (`Source / docs` row). Rationale/record: `docs/policies/DOCS_OPERATING_MODEL.md`.

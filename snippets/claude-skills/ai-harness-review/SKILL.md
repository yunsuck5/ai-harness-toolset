...
When the review input references off-repo or sibling material (for example, a planning anchor under `polishing/`, a handoff doc, a snapshot or manifest, or user-provided text), do not promote it to source-of-truth. Reference it as advisory / planning / user-provided context and disclose its role explicitly in `## Context` (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md` §5a.2). Because the reviewer's read-only sandbox may not be able to read repo-outside paths, when the off-repo material is itself review-relevant — for example, the body of an anchor doc that the verdict depends on — inline its body verbatim inside `## Context` (or a dedicated subsection such as `## Anchor draft body`) so the reviewer can read it in-sandbox. Citing only the off-repo path while leaving its body unreadable inside the sandbox produces verdicts on the wrong base.

The reviewer is also instructed (per the H1 preamble in `scripts/review-run.ps1`) to articulate the strongest case **against** its own conclusion in `## Notes` before issuing the final verdict, especially when the verdict is `yes` or `yes with risk`. Treat that section as a substantive pressure-test surface when you read the result — it is not a courtesy aside. This is light wording-level pressure (no new required heading, no parser gate) — the H1 preamble's wording is unchanged. On top of that light P3 wording, the review-result contract now also codifies a separate `## Counter-argument` H2 as a strongly-recommended optional disclosure section dedicated to the strongest case AGAINST the verdict (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md` §3c). It is **not** parser-required — `scripts/review-verify.ps1 -RequireResult` still enforces only the four V2 H2s — but operators should instruct the reviewer to write it (see the `## Counter-argument` paragraph below). Heavier mechanisms beyond §3c — a devil's-advocate pre-pass or multi-reviewer consensus — remain idea-only (not implementation backlog).

...
When you must honestly cite a template placeholder token (e.g., the `AI_TO_FILL_*` form) in the prose, avoid the bare two-brace identifier form because the input-verify regex `\{\{[A-Za-z_][A-Za-z0-9_]*\}\}` does not distinguish honest citation from an unfilled placeholder. Use one of these operator conventions: (a) brace-less identifier only (`AI_TO_FILL_VALIDATION_EVIDENCE`), (b) wildcard form (`AI_TO_FILL_*`), or (c) backslash-escaped literal form (`\{\{TOKEN\}\}`). All three are mechanically safe by the same mechanism — the regex's leading `\{\{` requires two adjacent `{` characters in the raw file bytes, and none of the three forms contains such a substring. (a) and (b) carry no brace at all. (c)'s raw file bytes are 13 chars (`\`, `{`, `\`, `{`, `T`, `O`, `K`, `E`, `N`, `\`, `}`, `\`, `}`) with `\` between every `{` and `}`, so the regex's leading `\{\{` cannot start a match. The regex itself is not being changed in this batch.
...
- Read `## Findings`, `## Blocking findings`, `## Non-blocking concerns`, `## Review limitations`, `## Assumptions relied on`, `## Risks`, and `## Notes` (each if present) to understand reviewer reasoning.

The verdict has a narrowed meaning — `yes` = no blocking finding (not commit/push approval); `no` = blocking finding exists (corrective needed); `yes with risk` = no blocking finding but the risks under `## Risks` (and any flagged limitations / assumptions) require explicit supervisor or user acceptance before commit / push. `yes with risk` is not the automatic equivalent of `yes` for the next step. When reporting back to the user, report the verdict verbatim plus the four disclosure sections so the user has the same surface the reviewer surfaced.

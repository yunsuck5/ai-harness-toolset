...
8. **Validation evidence** — the validation evidence used and its limitations.
...
- **model / model-source** — the runner resolves the model but does not yet emit a model / model-source run-fact (that wiring is Batch D2). Report it as configured / known, not as an observed run-fact, until the wiring lands. Never bake a concrete model version into the report — cite the runtime-resolved value; `config/reviewer.json` is the source of truth.
- **reviewer-safe posture** — `--sandbox read-only` / `--ask-for-approval never` / `--ignore-user-config`; verified for the tested vectors (create / modify-tracked / modify-existing) only, not a blanket guarantee. Not yet emitted as a run-fact (available when emitted, Batch D2); until then report the known posture with the tested-vectors-only caveat, not as a runner-observed run-fact.
- **review engine identity** — engine ToolRoot / ProjectRoot / tool-root-source. For review-subsystem self-modification the engine must be the global stable ToolRoot (the review-engine independence rule, step 1); report the engine you invoked from your own invocation knowledge (runner run-fact emission is available when wired, Batch D2).

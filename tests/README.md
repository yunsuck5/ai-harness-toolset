...
- Test files (current canonical review topology — three-level `<review-task-id>/<perspective>/pass-NN/` with `<perspective>` **required**; spec-of-record: `docs/review/review_spec.md`):
  - `tests/review-prepare.Tests.ps1` — `review-prepare` allocating the canonical three-level `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/` and creating `input.md` (default seeds the full `templates/review-input.md` body; `-NoSeed` creates an empty `input.md`): per-perspective pass allocation, the `-Perspective`-required failure path (omitting it fails fast — no two-level fallback), and invalid-perspective rejection.
  - `tests/review-run.Tests.ps1` — `review-run` Codex invocation surface driven by controlled stubs, plus PASS/provenance 전 complete successful-result shape 검증. It covers strict UTF-8 raw-byte stdin (Korean/CJK, BOM 부재, caller tail 보존), standard npm `codex.ps1` → adjacent `node.exe` entrypoint resolution과 production argv의 reviewer-safe posture, unsupported script-launcher fail-fast, and invocation/result-unavailable paths that emit no consumable verdict. It also covers effort/model resolution and run-facts, including the U9 config-backed `categoryPolicy` lookup (`-EffortCategory`): matched category applies `{model,effort}`, a genuinely absent (missed) category soft-falls-back to the scalar config, explicit `-Effort` / `-Model` win per axis, a matched-but-malformed entry (out-of-enum / missing / null `reasoningEffort`) fails fast **unconditionally** (even under an explicit `-Effort` override), and the shipped config keeps every category at the `xhigh` safety floor. A distinct-value fixture ToolRoot (`New-CategoryToolRoot`, which copies the real `scripts/` tree so the explicit-ToolRoot script resolution succeeds) proves the category values flow into the Codex argv. All stubs are generated under Pester's `$TestDrive` physical path and never invoke the real Codex CLI.

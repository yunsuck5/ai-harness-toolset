...
- Test files (current canonical review topology — task `<review-task-id>/pass-NN/` with an optional `<perspective>` segment giving the three-level `<review-task-id>/<perspective>/pass-NN/`; `docs/contracts/review/REVIEW_RESULT_CONTRACT.md`):
  - `tests/review-prepare.Tests.ps1` — `review-prepare` allocating `<ProjectRoot>/log/review/<review-task-id>/pass-NN/` and seeding `input.md` from `templates/review-input.md`, plus the optional `-Perspective` three-level layout (`<review-task-id>/<perspective>/pass-NN/`): per-perspective pass allocation, old/new coexistence, and invalid-perspective rejection.
  ...
  - `tests/review-verify.Tests.ps1` — `review-verify` default and `-RequireResult` paths against the canonical `input.md` + `result.md` pair, including the optional `-Perspective` three-level layout and the explicit old/new layout resolution (omitting `-Perspective` resolves the two-level path; no inference).
...
- Pester test fixtures live under Pester's `$TestDrive` physical path, not under the repo's `log/`. Real review runtime artifacts live under `<ProjectRoot>/log/review/<review-task-id>/pass-NN/` (or `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/` when a perspective is used) when the scripts are invoked outside tests.

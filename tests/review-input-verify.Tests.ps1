...
Describe 'templates/review-input.md compact authoring reference' {
    It 'AC-IV-OC1: template keeps the input skeleton without duplicating the result runtime shape' {
        ...
        $content | Should -Match 'exact provenance pointer'
        $content | Should -Match '확인하지 못한 claim은 `unverified`로 표시한다'
        ...
        $content | Should -Not -Match 'review unavailable'
        $content | Should -Not -Match 'verbatim inline|inline fallback'
        $content | Should -Not -Match '(?m)^## (Framing self-check|Reference sweep|Blocking findings|Non-blocking concerns|Review limitations|Assumptions relied on|Findings|Risks)$'
    }
}
...
Describe 'source ai-harness-review skill compact judgment core' {
    It 'AC-IV-OC2: skill retains compact judgment core and two canonical point-of-use contract lines' {
        ...
        $content | Should -Not -Match 'original path and section and confirm the exact text'
        $content | Should -Not -Match 'Recalculate mutable counts against current state immediately before authoring'
        ...
        $content | Should -Not -Match 'consume no verdict and report the review as unavailable'
        $content | Should -Match 'Canonical review를 호출할 때 caller timeout을 명시적으로 1000년으로 설정한다\.'
        $content | Should -Match 'Timeout으로 종료된 실행은 canonical reviewer verdict `no`가 아니다\.'
        $content | Should -Not -Match 'verbatim inline|inline fallback'

...
BeforeAll {
    ...
    function script:Invoke-ReviewPrepare {
        ...
        if ($null -ne $TargetFiles -and $TargetFiles.Count -gt 0) {
            $listPath = Join-Path $script:FixtureRoot ('pester-prepare-targets-' + ([guid]::NewGuid().ToString('N')) + '.list')
            $listContent = ($TargetFiles -join "`n") + "`n"
            $enc = New-Object System.Text.UTF8Encoding($false)
            $listParent = Split-Path -LiteralPath $listPath
            if (-not (Test-Path -LiteralPath $listParent -PathType Container)) {
                $null = New-Item -ItemType Directory -Path $listParent -Force
            }

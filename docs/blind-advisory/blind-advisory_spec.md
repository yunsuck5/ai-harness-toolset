...
- durable end-state는 새 최소 계약과 source skill의 1:1 정렬, 구계약 전용 결박 제거, lifecycle state `live`다.
...
- 한 invocation의 reviewer 실행은 single-shot이며 실패 뒤 retry하지 않는다.
...
- 이후 의미 변경은 Design→Plan lifecycle을 다시 열고, Design·Plan·Work Packet은 그 revision 동안만 존재한 뒤 closeout에서 retire한다. Backlog는 아직 시작하지 않은 future work와 next-ID floor를 소유한다.
...
- read-only와 금지 행위는 reviewer prompt와 host/caller가 관측할 수 있는 결과로 통제하며 hard sandbox 증명이 아니다.
- 관측되지 않은 위반을 사후 탐지한다고 주장하지 않는다. 이 한계를 숨은 상태·추적기·별도 결과 artifact로 보완하지 않고, 실제 관측된 위반만 `unavailable(<짧은 실제 사유>)`로 전달한다.

## Cross-domain interface

- canonical review는 Blind 결과와 별개의 권위·절차를 가진다. Blind 결과는 verdict가 아니며 correctness·coverage·canonical pass 가능성을 주장하지 않는다.
...
- source 또는 native mirror가 설치·호출 가능한 runtime 상태는 이 Spec의 lifecycle 상태를 대신 결정하지 않는다. lifecycle은 docs-working-model closeout이 소유한다.

## Validation expectation

- Spec과 SKILL의 문장 대조로 최소 계약이 1:1인지 확인한다.
...
- actual native invocation에서 current repo direct review, no mutation, no tests, no toolset recursion, child no-fan-out과 reviewer final 무가공 반환을 확인한다.
- source·ToolRoot·공식 vendor native mirror parity와 activation preview는 install-update owner의 정규 표면으로 확인한다.
...
- skill이 canonical급 비용 또는 기존 authority/status/transport machinery를 도입해 경량 prefilter를 넘어섰는가?
...
- **live** — 새 최소 계약과 source skill이 closeout reconciliation을 거쳤으며, 이후 의미 변경은 새 Design→Plan lifecycle을 사용한다.

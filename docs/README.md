...
| `docs/systems/` | **retired layer** — the per-subsystem boards (skills was the last) have been retired; the layer now holds no current docs (history in git). **Current per-subsystem state lives in the migrated domains' `docs/<domain>/` spec/backlog**; the former install-update system docs migrated to `docs/install-update/`. (Structural removal of the now-empty layer is pending Batch P.) | current subsystem state → `docs/<domain>/` |

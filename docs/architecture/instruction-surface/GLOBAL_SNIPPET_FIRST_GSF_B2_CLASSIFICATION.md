...
- **why stale:** `REPO_READING_GUIDE.md` Q1 + Q4 already flag these as **"Do not use"** (superseded, isolated by the top reconciliation note); current truth lives in `docs/systems/install-update/STATUS.md` + `BRIEF_CONTRACT.md` (3rd reconciliation).
...
- **Artifact/protocol contracts (owner surfaces) — remain.** `docs/contracts/review/REVIEW_RESULT_CONTRACT.md`, `brief/BRIEF_CONTRACT.md`, `evidence/EVIDENCE_CONTRACT.md`, `global-invocation/SHARED_GLOBAL_INVOCATION_CONTRACT.md`. Each keeps artifact authority (design §4 note). `BRIEF_CONTRACT.md` (BF Level table) and `brief/DEFERRED.md` (BR-D-01/03; BR-D-02 retired) are exactly the **owner surfaces GSF-B1 / Batch 3 migrated facts into** — owner-migration **complete**; reconfirm, do not re-migrate. Operator-stance / AI-responsibility prose in `REVIEW_RESULT_CONTRACT.md` §5/§5a/§6a is the contract single-home the snippet + skill point to (correct direction, not a finding).

...
**Basis.** `docs/**` as of `HEAD == 2400bd0` (post Batch 3 / Track F; snippet at 8 H2). Inspection method: priority surfaces read line-by-line (`docs/policies/**`, the main `docs/contracts/**`, `docs/architecture/instruction-surface/**`, the then-extant skills docs, 당시 brief 시스템 docs(현 `docs/brief/`), `docs/current/REPO_READING_GUIDE.md`, `docs/user_guide/OPERATOR_GUIDE_KR.md`); cross-cutting public-safe / durable-pointer patterns swept repo-wide by grep (machine-specific paths, `log/**` durable pointers, username leak, `polishing/` / `repo_snapshot/`); the remaining surfaces (decision records, roadmap/backlog routing, per-system STATUS, review-system plan bodies) category-classified with grep-confirmed line items. The audit is **keep-by-default** for `docs/**` (design §2: existing docs are evidence, not automatically blocking source-of-truth; their *role* narrows, their *body* survives) — only the findings below deviate, and none is edited in this batch.
...
- **why stale:** `REPO_READING_GUIDE.md` Q1 + Q4 already flag these as **"Do not use"** (superseded, isolated by the top reconciliation note); current truth lives in `docs/systems/install-update/STATUS.md` + the brief spec (현 `docs/brief/brief_spec.md`).
- **classification:** `remain` (decision/model record; stale-but-isolated) + `compress candidate` for GSF-B4.
- **proposed owner:** current state → `install-update/STATUS.md`; brief framing → 당시 BRIEF_CONTRACT(현 `docs/brief/brief_spec.md`). The model doc remains the design/model authority for live install behavior.
...
- **why stale:** the doc's own supersede note + `REPO_READING_GUIDE.md` Q2 "Do not use" mark `review-cycle.ps1` / `meta.json` / `result.json` / `<run-id>` flat layout as removed-legacy.
...
- **source:** the instruction-surface family (`INSTRUCTION_SURFACE_PLAN.md`, the retired skill-subsystem plan — git history) + the GSF family (`..._MIGRATION_DESIGN/PLAN/SPEC_GSF_B1/GSF_B1_CLASSIFICATION.md`) — inline "(Landed — SK-0x)", "(since removed by Batch 3)", past-tense reconciliation annotations.
...
- **classification:** `remain` (decision-record / rationale) + weak `compress candidate` — the annotations are legitimate status pointers (they route to the skills status ledger rows, now in git history), so this is a low-severity altitude note, not a removal.
- **proposed owner:** current state → the skills status ledger (retired in batch S; git history); design docs keep the *decision*, pointing to it for status.
- **owner-migration status:** owner existed (the skills status ledger, since retired in batch S; git history); design docs over-annotate but route correctly.
...
- **Artifact/protocol contracts (owner surfaces) — remain.** `docs/contracts/review/REVIEW_RESULT_CONTRACT.md`, 당시 `brief/BRIEF_CONTRACT.md`(현 `docs/brief/brief_spec.md`), `evidence/EVIDENCE_CONTRACT.md`, `global-invocation/SHARED_GLOBAL_INVOCATION_CONTRACT.md`. Each keeps artifact authority (design §4 note). 당시 BRIEF_CONTRACT (BF Level table) 와 당시 brief DEFERRED (BR-D-01/03; BR-D-02 retired — 현 `docs/brief/brief_backlog.md`) are exactly the **owner surfaces GSF-B1 / Batch 3 migrated facts into** — owner-migration **complete**; reconfirm, do not re-migrate. Operator-stance / AI-responsibility prose in `REVIEW_RESULT_CONTRACT.md` §5/§5a/§6a is the contract single-home the snippet + skill point to (correct direction, not a finding).
...
- **Skill-subsystem plan/status — remain.** the skill-subsystem plan + status (since retired in batch S; git history). STATUS was the correct single-home for "since landed" status.
...
- **Decision records / project identity / routing — remain.** `docs/decisions/**` (POST_MVP_PLAN, DECISIONS, GLOBAL_ADOPTION_DECISION — settled decision records; GLOBAL_ADOPTION_DECISION carries the B2-F02 example paths), `docs/project/**` (scope/philosophy), `docs/roadmap/**` + `docs/backlog/**` (routing-only per `DOCS_OPERATING_MODEL.md` §3), per-system `STATUS.md` / `BACKLOG.md` / `DEFERRED.md`, `docs/current/REPO_READING_GUIDE.md` (per-question routing; its "Do not use" rows are the mechanism that already isolates B2-F08/F09 stale framing).
...
- **Deployed-surface (`snippets/` / `config/` / `scripts/`) public-readiness pointers** — the off-repo `polishing/` example in the deployed `SKILL.md` and deployed→non-deployed provenance pointers in `config/` / `scripts/`, already recorded in the retired skill-subsystem plan §3/§9 (git history) as public-readiness friction. **Out of GSF-B2's `docs/**` scope** — route to a deployed-surface cleanup batch, not GSF-B2 / GSF-B4 (which is docs-only).
...
- `docs/current/REPO_READING_GUIDE.md` — **updated** (Q11): registers this classification record as landed (classification-only) and reconciles the Q11 Implementation line so GSF-B2 is no longer listed among the *remaining* implementing tracks (GSF-B3/B4 remain).
...
- the (now-retired) skills status ledger — *checked at the time: no change required* (GSF-B2 changed no snippet / skill / deployed surface; the ledger is preserved in git history).
- existing `INSTRUCTION_SURFACE_PLAN.md` / `GLOBAL_SNIPPET_RELOCATION_AUDIT.md` / `REPO_LOCAL_INSTRUCTION_SURFACE_PLAN.md` / the GSF design / spec / GSF-B1 classification — *checked: intentionally not edited* (preserved as prior evidence; this record routes to them).

...
**Audit basis.** Snippet content as of repo `HEAD == ea61048`. Both snippets carry **11 H2 sections**: Adoption destination · Adoption rules · Role neutrality · Project layout · Review flow · Result verdict vocabulary · Operator stance · Brief · Chatlog · Forbidden in this toolset · Other rules.
...
- **Duplicated?** Yes — `## Project layout` (record path), `REVIEW_RESULT_CONTRACT.md` Q3 / `SOURCE_OF_TRUTH.md` Q3 (no-sidecar). Single-home pressure favors folding.
...
- **Duplicated?** Authority = `REVIEW_RESULT_CONTRACT.md` (non-deployed) + `ai-harness-review` `SKILL.md` + `SOURCE_OF_TRUTH.md` Q3. Snippet keeps only the quick-reference.
...
### 3.9 `## Chatlog`
- **Text (summary):** Chatlog ≠ Brief; Chatlog area `…/log/chatlog/`; current restore source = Brief; Chatlog is reconstruction-evidence only, never promoted to Brief's seat.
- **Disposition:** **Batch 3 defer** (whole section delete, owned by Batch 3).
- **Trigger-tier rationale:** Chatlog is not a current implemented capability; the plan removes the whole section in Batch 3 (no boundary-line residue). Authority = `docs/contracts/chatlog/CHATLOG_CONTRACT.md` (non-deployed).
- **Duplicated?** Yes — `CHATLOG_CONTRACT.md`. Note the embedded line "current restore source is **Brief**" is a Brief fact (owned by `ai-harness-brief` `SKILL.md` + `BRIEF_CONTRACT.md`), bundled here; **Batch 3 must decide whether that line needs to survive elsewhere** when the section is removed.
- **Prerequisite to move?** None — delete owned by Batch 3. **Symmetry:** byte-identical; remove from both. **Risk if removed early:** low-medium (loses the Chatlog≠Brief boundary + restore-source pointer); handled inside Batch 3 with the reconciliation note above. **Next action:** **Batch 3 defer**; flag the embedded restore-source line + the `## Forbidden` "no Brief↔Chatlog mirror" reference (3.10) for Batch 3 reconciliation.

### 3.10 `## Forbidden in this toolset`
Seven bullets — **six are global keep** (core safety hard boundaries); the seventh (Brief↔Chatlog mirror) is **Batch 3 defer** because it is a boundary note referencing a Batch-3-removed concept. Per-bullet notes:
...
- *No automatic Brief↔Chatlog mirror* — **Batch 3 defer (not unconditional global-keep).** It forbids a real automation, but it is a **boundary note that references Chatlog**, a Batch 3 removal target, and skill plan §8 keeps "no Chatlog … boundary note" in the end snippet — so this audit must not pre-classify it as global-keep. Its *substance* (a no-auto-mirror automation guardrail) is a candidate to survive only in a **generic, non-Chatlog form**, and that decision is **Batch 3's** (drop if already covered by the no-daemon/no-auto rules, or keep a generic Brief-mirror guardrail) — owned by skill plan §8, not pre-empted here.
...
- **Prerequisite to move?** N/A for the six global-keep bullets. **Symmetry:** byte-identical; keep/defer symmetric across both snippets. **Risk if removed early:** **very high** for the six safety boundaries (keep); for the mirror bullet, low — but its disposition belongs to Batch 3, not this audit. **Next action:** keep the six; **defer the Brief↔Chatlog-mirror bullet to Batch 3** (reconcile to a generic non-Chatlog guardrail or drop); Track D dedup the `.claude\AGENTS.md` overlap with `## Adoption destination`.
...
| **`docs/contracts` candidate (move)** | none to *move* — the contract authorities already exist (`SHARED_GLOBAL_INVOCATION_CONTRACT.md`, `REVIEW_RESULT_CONTRACT.md`, `CHATLOG_CONTRACT.md`, `BRIEF_CONTRACT.md`); the snippet keeps only *operative residue* because `docs/**` is non-deployed. (Listed as authority pointers, not relocations.) |
...
| **Batch 3 defer** | Brief — BF-lv3 non-claim (3.8); Chatlog — whole section (3.9); Forbidden — the "no Brief↔Chatlog mirror" bullet (a boundary note referencing a Batch-3-removed concept; substance may survive only in generic non-Chatlog form, Batch 3's call) (3.10). |
...
**Headline finding.** The only *content* that should leave the global snippet for a *new* home is the two PowerShell-authoring rules in `## Other rules` → **repo-local instruction (Track C)**. Everything else is either a genuine global invariant (keep), a Batch-3-owned non-current-item removal (Brief/Chatlog), or a Track-D in-snippet cleanup (fold/dedup/symmetry). No section is a skill-`description` / `SKILL.md` / contract / install-docs *relocation* target — those surfaces already own their procedures, and the snippet's overlaps are deployed operative residue of non-deployed authorities, not duplications to move.
...
- **Track F = skill-plan Batch 3:** delete `## Brief` (BF-lv3 non-claim) and `## Chatlog` (3.8/3.9), **plus decide the `## Forbidden` "no Brief↔Chatlog mirror" bullet** (3.10 classifies it Batch 3 defer, not global-keep). **Reconciliation dependencies inside Batch 3:** preserve the "current restore source = Brief" pointer elsewhere if still wanted (owned by `ai-harness-brief` / `BRIEF_CONTRACT.md`); and for the mirror bullet, either **drop** it (if the no-daemon/no-auto rules already cover it) or **keep a generic, non-Chatlog Brief-mirror guardrail** so the boundary survives without referencing a removed Chatlog section. This bullet's disposition is owned by skill plan §8, not pre-empted by this audit.
...
**Sequencing note.** Track D and Batch 3 both edit the snippet and overlap (Batch 3 deletes Brief/Chatlog; Track D folds/dedups around them); the safe order is Batch 3 first (remove non-current sections) then Track D (cleanup of what remains), or run them as one combined snippet-edit batch with a single Codex review. Track C is independent of the symmetry/fold work but must precede the PowerShell-rule removal. None of these is executed or approved here.

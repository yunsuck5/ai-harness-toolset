...
### 9. `## Chatlog`
- **disposition:** **defer → skill-plan Batch 3 / Track F** · **owner-surface:** n/a · **loading-class:** n/a
- **keep-by-proof:** non-current-capability section (a Batch-3 `delete` target). The embedded "current restore source = Brief" line + the Chatlog≠Brief boundary are Batch-3 reconciliation items (preserve elsewhere if still wanted) — Batch 3's call, not GSF-B1's.
- **invariant-impact:** none (unchanged). **rationale:** authority `docs/contracts/chatlog/CHATLOG_CONTRACT.md` (non-deployed). Not touched here.
...
- **Brief↔Chatlog-mirror bullet:** **disposition: defer → skill-plan Batch 3 / Track F** (boundary note referencing a Batch-3-removed concept; spec §8; audit §3.10). owner-surface: n/a. **rationale:** its substance may survive only as a generic non-Chatlog guardrail — Batch 3 decides; GSF-B1 leaves it.
...
6. **Every removed item has an owner-surface or rationale** — ✓ the only removal is the `## Review record` *section header* (its content folded into Project layout, no-sidecar invariant + contract pointer preserved). No content deleted. Brief/Chatlog/mirror = defer (untouched).
...
**Deferred to skill-plan Batch 3 / Track F (not executed, not re-owned, not renumbered):** `## Brief` BF-lv3 non-claim, `## Chatlog` section, `## Forbidden` Brief↔Chatlog-mirror bullet.

...
**promote 시 무엇이 되는가.** review-date 판정에서 **promote** 되면 `docs/consultation/` 가 새 domain 의 home 이 되고, promotion transition(같은 changeset 의 원자적 `_incubation.md` → `consultation_design.md` swap)이 이 문서의 current-bearing 내용을 E4 로 **entry promoted artifact 인 `consultation_design.md`** 에 흡수한 뒤(제거의 전제조건) 이 `_incubation.md` 를 삭제하며, 이후 정규 Design → Plan → Spec lifecycle 이 이어진다. 미해결 open question 은 그 시점에 resolve 되거나 명시 deferral 된다(`consultation_backlog.md`, 그것이 생기기 전이면 entry `_design` 의 Deferred Questions 절).
...
- **기존 domain/rule 이 못 덮는 구체 문제.** 현 모델에는 최종 verdict 를 내리는 review(canonical verdict gate — verdict vocabulary 는 review 소유)와, changeset 결함 pre-filter 인 blind advisory 가 있다. 그러나 operator 가 변경·승인·판정을 하기 *전에* 복수 AI 의 관점·반론·조사 의견을 read-only 로 수집하고 이를 **비권위 advisory 입력으로 어떻게 다룰지**에 대한 단일 규칙 home 이 없다. 그 결과 이 활동이 review 처럼 오인되어 승인 효과를 갖거나, 반대로 임의의 sub-agent / research 관행으로 흩어져 권한 경계와 산출물 지위가 불명확해진다. 이 gap 은 review·blind·brief·install-update·docs-working-model 어디에도 흡수하기 어렵다.
- **판단을 바꾼 증거 유형. [실측 검증 + 설계 원료]** (특정 회차의 검증 결과 기록이 아니라 E4 가 요구하는 *증거 유형* 서술이다 — 결과 기록 자체는 runtime 영역 소관.) 증거 유형 ① canonical gate 가 통과시킨 대상에서 독립 advisory 층이 결함성 지적을 내는 관측 유형 — "판정 *전* 의 비권위 advisory 계층" 이 기존 review gate 로 환원되지 않음을 보여준 축이다(blind advisory 의 semantics 는 그 candidate 소유; 여기서는 관측 유형만). 증거 유형 ② framing 축 두 operation(`독립 의견`/`재조율`)의 분화 근거 — pre-focus 독립 호출과 stance-공유 반론 호출이 각각 다른 종류의 기여(질문 밖 새 축 발굴 vs 입장의 실질 반박·정밀화)를 내는 반복 관측 유형 + 사용자 교정.
- **candidate shape.**
  ...
  - success-absorption artifact(promote): promotion transition 의 entry `consultation_design.md`(E4 흡수의 단일 목적지) — 이후 `consultation_plan.md` / `consultation_spec.md` 의 정규 lifecycle 로 이어진다.
...
- **promote-readiness 기준(review-date 에서 promote 를 정당화하는 positive evidence — 기계 gate 아님, operator 판단 입력; pilot 에서 누적 측정).** 위 discard 기준의 거울이며, 이 항목들이 곧 pilot 측정의 누적 대상이다.
  1. read-only advisory consultation 이 review · blind · 기존 orchestration 으로 환원되지 않는 독립 가치를 반복 입증한다(`독립 의견` / `재조율` 이 실제로 operator 종합에 기여).
  2. 산출물이 advisory 경계를 지킨다 — verdict / approval / merge·push·release 근거로 오용된 사례가 없다(경계 견고).
  3. scope 가 좁은 single-home 으로 유지된다(broad-bucket 확장 압력 부재).
  4. consultation 과 blind advisory 의 구분이 사례 수준에서 명확히 유지된다.
  5. domain-local closure 성립 — consultation 의 의미가 review 의 semantics 없이 설명된다.
...
- **read-only 기본값** — consultant 는 inspection 만, mutation 없음. consultation 진행 중 mutation 이나 별도 승인 필요가 드러나면 그 지점에서 진행을 멈추고 operator 가 escalate 한다(read-only 경계의 절차화). [설계 원료]
...
- **consultant output 은 truth oracle 이 아님** — 외부 AI 주장의 진위는 consultation 이 보증하지 않는다. 특히 시점·절차·문면 사실은 consultant 의 표현을 산출물로 옮기기 전에 operator 가 원문 정독으로 재검증한다 — 인용이 전부 정확해도 개념-표현은 부정확할 수 있다. [실측 검증]
...
- **status vocabulary** — consultation 자체에는 canonical verdict 를 쓰지 않는다: `synthesized` / `needs-follow-up` / `conflicting-opinions` / `insufficient-context`. 이는 review 의 verdict vocabulary 와도, blind advisory 의 status set 과도 분리된다(각 token 집합은 그 owner 가 소유 — 여기 재나열하지 않는다). (기각 대안: 초기 후보에 있던 `not-run`(미실행) 상태는 이 realigning changeset 에서 명시 기각 — no-file conversational 런타임에는 "미실행 기록" 이 붙을 산출 자리가 없다. [stale — 대체됨])
- **packaging mode(consultant topology 축)** — `single-consultant` / `parallel-consultation` / `role-split-consultation` / `counterpoint`(반론 생성). `roundtable`(1차 응답 종합 후 2차 반론)은 비용·contamination 위험이 커 후속 후보. `council` 은 domain 명이 아니라 pending packaging alias 다. `counterpoint` 는 canonical review 결과 문서의 `Counter-argument` 절과 무관한 별개 용어다(packaging mode 값이지 review 산출 절이 아니다).
- **명명 금지(기각 coinage). [설계 원료 — 기각 대안 기록]** advisory 층을 review/승인처럼 들리게 하는 명명은 후보 형성기에 검토·기각됐고 되살리지 않는다: `consultation verdict` · `consultation pass` · `multi-reviewer consensus` · `consensus gate` · `consultant consensus` · `AI review council` · `AI approval` · `AI sign-off` · `advisory verdict` · `non-blocking verdict` · `soft review` · `review-lite` · `lightweight review` · `pre-review approval` · `recommendation gate` · `council/panel decision` 류. 기각 이유 = 전부 verdict/approval 함의를 advisory 층에 이식해 status↔verdict 경계를 침식한다. glossary 에는 등재하지 않는다(naming proposal dropped — no-entry 종결); 이 목록의 do-not-revive 부담은 이 문서가 domain-local 로 소유하고 promote 시 E4 로 승계한다.
...
- **`독립 의견` (operation; id `independent`; = seed 의 mode A).** pre-focus 독립 자문. 입력 = 질문 + 배경만(operator 의 stance / 결론 유도 미포함). **한 run 은 one-shot, 항상 terminal** — run 내부에 round 루프가 없다. (operator 가 이후 다시 호출하면 그것은 *이전 run 의 연장/루프가 아니라 새 독립 run* 이다 — §권장 운용 의 "재호출"이 이것이다.) `독립 의견` run 결과들이 서로 충돌해도 `독립 의견` 자체는 루프로 들어가지 않는다 — 충돌을 다투려면 operator 가 *별도의 새 `재조율` run* 을 시작한다(§충돌 시 디폴트). 목적 = operator 포커스가 고정되기 전 앵커링 방지 독립 견해. **질문은 양방향으로 구성한다** — 성립·불성립 양쪽이 열린 형태("X 가 충분한가/부족한가"); 전제-내장 질문("부족한 축을 나열하라")은 pre-focus 독립성을 훼손하고, 양방향 구성이 consultant 의 잔여-압력 자기신고도 가능하게 한다. [실측 검증]
...
- **run 지속성은 purpose-bound. [실측 검증]** multi-round `재조율` arc 는 세션-지속으로 도는 것이 기본이다 — 라운드마다 operator 가 누적 맥락을 재패키징하면 그 재패키징에 operator 편향이 실린다(지속이 그 편향 표면을 제거). 반면 `독립 의견` 등 독립 검사는 fresh(앵커링 0)로 돈다. 지속성은 우열이 아니라 operation 목적에 묶인 파라미터다(도구-측 세부는 운용 도구 소관 — 이 문서는 결정만 소유).
...
- **권장 운용.** 방향-설정 지시 수신 즉시 → `독립 의견` 먼저(다른 진행과 *병행 가능* = non-blocking 호출 순서를 뜻하며, daemon · watcher · hidden background task 가 아니다) → operator 독립 정리 → 합쳐 방향 논의 → 진행 → 진행 중 모호/외부관점 필요 지점에서 재호출(`독립 의견` 또는 `재조율`).
...
- **충돌 시 디폴트 = `재조율` 루프(reconciliation loop).** consultant 응답이 서로 충돌하거나 (`재조율` 에서) operator 초안을 반박할 때, 디폴트는 *재조율 루프*(재-relay · 재질의)이지 fiat 종결이 아니다. 멈춤은 *양쪽 AI 가 모두 답을 못 내는 크리티컬 미결* 일 때뿐이며, operator 가 외부 circuit-breaker 다. 수렴은 advisory 일 뿐 어떤 행동도 승인하지 않는다(수렴 ≠ commit / promote 승인). (이 디폴트는 운용 도구 쪽에는 반영돼 있었으나 이 문서엔 빠져 있던 gap 의 fold-in 이다.) 그리고 **빠른 무저항 수렴은 종결 신호가 아니라 경계 신호다** — 재조율 없이 닫힌 합의가 결함 방향을 통과시킨 실측이 있다. [실측 검증]
- **request 가 inspection scope 를 소유한다. [실측 검증]** consultation request 의 허용 inspection scope 는 추론-only(경량 토론)부터 consultant 가 대상 파일을 직접 읽는 full-scope 조사(research delegation)까지 스펙트럼이며, 어느 지점인지는 도구 환경이 아니라 request 가 명시로 결정한다.
- **사실-frame 제공 ≠ 결론 주입. [실측 검증]** 대상의 계층·altitude·역할 같은 사실-frame 을 request 에 제공하는 것은 framing 오염이 아니라 판정 정확도의 결정변수다 — frame 부재의 fresh 검사가 대상을 잘못된 엄격도로 재단해 거짓 결함을 양산한 실측이 있다. 금지 대상은 결론·선호의 주입이지 사실-frame 이 아니다(`독립 의견` 의 stance-미포함 원칙과 양립 — stance 와 사실-frame 은 다른 것이다).
...
- review 로 넘기는 정보는 operator 가 중립화한 것만: 확정 우려 → `Known concerns`, 열린 반론/의심 → `Review questions`(`verify whether...` 형태), 배경 → `Context`(결론 유도 표현 제거) — 이 필드명들은 review 소유의 preflight-input interface 를 *참조*한 것이며(§consultation 자기 경계 downstream 항과 동일 경계), 그 형식이 바뀌면 review 쪽 정의가 우선한다.
- 금지: consultation 결과를 review verdict 로 변환 · consultation 을 review invocation / pass / coverage count 에 합산 · consultation transcript 를 `log/review/**` 에 저장 · consultation concern 을 `Blocking findings` 의 source-of-truth 로 승격 · canonical reviewer 에게 consultation 결론을 신뢰하라고 지시 · consultation preflight 를 조용한 기본값으로 자동 삽입(canonical-only 요청에 consultation 을 강제하지 않는다). [설계 원료]
...
> 이 절은 consultation **자신의** 경계만 선언한다 — no-file 런타임 · 자기 입력 경계 · 자기 advisory-항목 shape. close-the-loop / evidence-schema 처리는 **이름붙인 외부 계약**을 참조만 하고 그 semantics 를 재서술하지 않는다(candidate promote / discard / continue lifecycle 자체는 외부 계약이 아니라 이 문서 §Candidate lifecycle state 가 소유한다). 세부 schema·naming 은 promote 시 design/spec 에서 확정한다.

- **no-file 런타임.** consultation 이라는 **기능 자체는 per-run input/output 파일을 만들지 않는다** — 실행은 conversational synthesis 로만 둔다(canonical 리뷰의 `input.md` / `result.md` 와 정반대). 아래 §Open 의 "runtime artifact 저장 여부"는 이로써 *저장 안 함* 으로 닫힌다. no-file ↔ 측정의 긴장은 이렇게 풀린다: **기능은 no-file 이고, 실험의 pilot 측정은 gitignored runtime scaffolding 에 사는데 그것은 기능 파일이 아니다**(graduation 시 폐기되는 별개 실험 영역). (자기 작업을 codex review 로 돌릴 때 생기는 산출물도 *리뷰 도메인* 의 runtime footprint 로 별개이며 consultation 자신의 산출이 아니다.) (기각 대안: 초기 설계의 전용 per-run runtime 저장 영역·per-consultant 저장·그 정리 정책은 이 no-file 결정이 기각했다 — 저장·형식의 무게가 advisory 를 canonical artifact 파이프라인 쪽으로 끌기 때문. [stale — 대체됨])
...
- **consultation 자기 advisory-항목 shape.** consultation 의 각 advisory 항목은 자기 출력 형태로 **confidence** 와 (근거가 된) **assumption** 을 함께 단다. consultation 의 status vocabulary(`synthesized` / `needs-follow-up` / `conflicting-opinions` / `insufficient-context`)는 이 도메인 자신의 어휘로 분리 유지한다(vocab 분리 불변식 — 이 절의 옛 heading 이 쓰던 `finding` 토큰도 canonical review 어휘라 advisory-항목으로 개칭했다). 항목이 이 shape 를 갖는 이유는, operator 가 이를 **`subagent-work-orchestration` close-the-loop validation contract** 를 통해 다른 advisory 산출과 reconcile 할 수 있게 하기 위해서다 — 그 reconcile 절차·evidence semantics 자체는 그 계약이 소유하며 이 문서는 이름으로만 참조한다. 필드 naming·표현 세부는 promote design/spec 에서 후보정한다.
...
- 외부 상용 AI 전달의 세부 — 전송 자체에 사용자 승인이 필요한지, 원문 full paste vs 경로+발췌 요약의 기본값(위 redaction 항목과 별개 축). [설계 원료]
- sub-agent 만 지원할지, 외부 CLI / API adapter 도 지원할지.
- output vocabulary 세부를 어디까지 고정할지(핵심 필드 confidence / assumption 은 §consultation 자기 경계 의 advisory-항목 shape 에서 고정; 세부 naming·표현은 잔여).
...
- **presentation 3단 제시 형식**(operator 입장 / consultant counterpoint / operator synthesis 라벨) — promote 시 skill 표면이 소유할 출력 형식(이 문서는 결정만 소유, 실행 절차 prose 비소유). [미확정 잔존물]
- **ownership map / seam 구조물**(owned/imported 용어 마킹 장치) — promote 시 design/spec 배치 결정 대상. [미확정 잔존물]
- **자연어 UX 일체**(trigger 문구·preset·routing 축·"코덱스 리뷰해줘" 기본값 선택지) — promote design/skill 몫(이 문서는 UX 를 의도적으로 비소유). [설계 원료]
- **full preflight pipeline 조합 순서**(consultation→blind→canonical 체인) — orchestration close-the-loop 계약 소유 + review 통합은 로드맵 최후 단계. [설계 원료]
- **advisory preflight 의 chain-owner 문장**("소유 = consuming chain") — §Operating model 의 operation-intrinsic vs validation-workflow 경계가 같은 내용을 이미 표현; 별도 문장 채택 여부는 promote design 에서. [미확정 잔존물]
- **`operator synthesis` 의 별칭(advisory working note)** — 미채택; 별칭 고정은 promote 시 vocabulary 확정에서. [미확정 잔존물]
- **`synergy` 계열 측정 어휘** — 측정 scaffolding 소관(정규 기능 비포함)이라 용어 승격 비대상; 재평가는 promote 판정 입력에서. [미확정 잔존물]
- **초기 packaging 지원 범위**(counterpoint 포함 여부 등 최초 구현 폭) — promote design 확정 대상. [설계 원료]
- **(잔여 risk) TERM-RESERVE durable-pointer 회귀 테스트** — 이 realigning changeset 의 검사 코드는 `log`/`polishing` 의 슬래시·백슬래시 변형 포인터를 잡도록 확장됐으나, 그 변형들을 직접 실패시키는 테스트 케이스는 아직 없다(`_incubation.md` 포인터 케이스만 직접 커버). 닫힘 시점 = 다음 후보의 realigning changeset 이 이 검사의 bound 목록을 확장하는 때(테스트 동반 추가). [미확정 잔존물]
...
- **promote 경로**: promotion transition(원자적 `_incubation.md` → `consultation_design.md` swap)에서 본문 current-bearing 내용을 E4 로 entry `consultation_design.md` 에 흡수(제거의 전제조건) 후 `_incubation.md` 삭제; 이후 정규 Design → Plan → Spec lifecycle. 미해결 open question 은 resolve 또는 명시 deferral(`consultation_backlog.md`, 부재 시 entry `_design` 의 Deferred Questions 절).
...
- **E5** — 이 문서는 rule 자신의 incubation-tier bootstrap 이 **아니다**. incubation tier 의 첫 정규 **domain** candidate dogfood 이며(rule candidate `subagent-work-orchestration` 은 별 tier-axis 라 "첫"은 domain candidate 기준), E1~E4 가 정상 적용된다(E5 의 one-time bootstrap 면제 대상 아님).

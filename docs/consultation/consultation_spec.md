...
- **blind advisory** — 별개 레이어다. consultation 은 blind 의 산출을 입력으로 받지 않으며 두 레이어는 operator synthesis 에서만 결합한다(입력-독립). `재조율` 은 framing 을 제공하고 blind 는 framing 을 제거하므로 정반대 동작원리이며, 그 framing-제거 semantics 는 blind 가 소유한다.

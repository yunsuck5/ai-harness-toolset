...
- 한 번의 review pass 는 두 단계 entry script 호출로 닫힌다: `review-prepare.ps1 -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>]` 가 canonical pass directory 를 발급하고 `input.md` 를 seed → AI 가 `input.md` 본문을 직접 작성 → `review-run.ps1 -ReviewTaskId <id> -Perspective <viewpoint> -Pass <pass-NN>` 가 `review-input-verify.ps1` 로 heading shape 를 검증한 뒤 active reviewer adapter (현재 codex CLI) 를 1 회 실행해 같은 pass directory 에 `result.md` 의 verdict/disclosure body 를 작성하고, 그 뒤에 runner 가 `## Reviewer run provenance` 블록을 append 한다 (result.md 는 dual-authored; `docs/contracts/review/REVIEW_RESULT_CONTRACT.md` §3).
- 결과 record 는 `<project-root>/log/review/<review-task-id>/<perspective>/pass-NN/` 의 canonical pair (`input.md`, `result.md`) 두 파일로 닫힌다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md`). `<review-task-id>` 는 하나의 `/goal` 작업 또는 하나의 review gate 단위이며 Claude Code chat / session id 가 아니다. `<perspective>` 는 review viewpoint 를 별도 path segment 로 분리하는 operator-named 값이며 canonical three-level layout 의 **필수** 요소다 (미지정 / 빈 값이면 fail-fast; two-level fallback 없음). `pass-NN` 는 같은 task·perspective 의 corrective loop 안에서의 각 Codex review attempt 다.
...
└── review/    # review record (docs/contracts/review/REVIEW_RESULT_CONTRACT.md)
    └── <review-task-id>/
        └── <perspective>/            # required (-Perspective <viewpoint>); canonical three-level layout
            ├── pass-01/              #   pass-NN is per-perspective here
...
- `<perspective>` (**required**) 는 review viewpoint (예: `local-correctness` / `system-coherence`) 다. `-Perspective <viewpoint>` 로 명시하며 — `<review-task-id>` 와 `pass-NN` 사이의 별도 path segment 로 들어가 canonical three-level layout 을 이룬다. 미지정 / 빈 값이면 fail-fast (two-level fallback 없음). operator 가 명시하며 자동 추론하지 않고, single path segment (`..` / `/` / `\` / `pass-NN` 형태 금지, safe charset/length) 여야 한다.
- `pass-NN` 은 corrective loop attempt 다. 첫 attempt `pass-01`, 다음 `pass-02`, ... 로 증가한다. `review-prepare.ps1` 가 같은 `<task>/<perspective>/` directory 의 기존 pass 를 스캔해 다음 번호를 자동 할당하거나, `-Pass <pass-NN>` 로 명시한다 — pass 번호는 per-perspective 다.
- review record 의 retention 단위는 `<review-task-id>/` 디렉터리 전체 또는 그 안의 개별 `pass-NN/` 디렉터리다. 사용자가 손으로 지운다. 자동 prune / rotate 는 없다.

`log/` 하위 디렉터리(`log/brief/`, `log/evidence/`, `log/review/`)는 거기에 쓰는 primitive 가 필요 시 자동 생성한다 (`brief-init.ps1` → `log/brief/`, `review-prepare.ps1` → `log/review/<...>/`); 별도 log tree 초기화 단계는 필요 없다.
...
flowchart TD
    ...
    B -- yes --> P[scripts/review-prepare.ps1<br/>-ReviewTaskId &lt;id&gt; -Perspective &lt;viewpoint&gt; [-Pass &lt;pass-NN&gt;]<br/>allocate canonical pass directory<br/>&lt;review-task-id&gt;/&lt;perspective&gt;/pass-NN,<br/>seed input.md from templates/review-input.md]
    P --> C[Claude Code overwrites<br/>pass input.md<br/>with the actual review request]
    C --> R[scripts/review-run.ps1<br/>-ReviewTaskId &lt;id&gt; -Perspective &lt;viewpoint&gt; -Pass &lt;pass-NN&gt;]
    R --> F[scripts/review-input-verify.ps1<br/>required headings + no leftover placeholders]
    F -- fail --> X1[FAIL: write-once — allocate a new<br/>pass-NN under the same review-task-id/perspective<br/>(rerun review-prepare)]
    ...
    L -- no --> M3[user fixes and allocates new pass-NN<br/>under the same review-task-id/perspective]
...
- canonical review artifact 는 `log/review/<review-task-id>/<perspective>/pass-NN/input.md` 와 `log/review/<review-task-id>/<perspective>/pass-NN/result.md` 두 파일뿐이다 (`<perspective>` 는 required; canonical layout 은 three-level). 다른 sidecar 파일은 contract 의 일부가 아니다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md`).
...
- parsing 실패한 pass 는 디스크에 그대로 남는다. 사람이 같은 pass directory 안의 파일을 손으로 보정하지 않고, 같은 `<review-task-id>/<perspective>/` 아래에 새 `pass-NN/` 를 만든다.
...
sequenceDiagram
    ...
    CC->>RP: invoke once with -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>]
    RP-->>CC: allocate canonical pass directory <review-task-id>/<perspective>/pass-NN;<br/>seed input.md from templates/review-input.md
    CC->>CC: overwrite pass input.md<br/>with the actual review request<br/>(templates/review-input.md as shape)
    CC->>RR: invoke once with -ReviewTaskId <id> -Perspective <viewpoint> -Pass <pass-NN>
    RR->>RIV: spawn -InputPath input.md
    RIV-->>RR: PASS or FAIL
    alt input verify FAIL
        RR-->>CC: FAIL — write-once: allocate new pass-NN<br/>under same review-task-id/perspective<br/>(rerun review-prepare)
        ...
        alt check FAIL
            ...
            RR-->>CC: PASS, <review-task-id>/<perspective>/pass-NN, verdict
...
|---|---|---|
| `현재 진행한 작업 코덱스 리뷰 진행해` | Mode A — changed files | git status / diff 확인 → 변경된 tracked 파일을 target 으로 결정 → 이 작업의 `<review-task-id>` 결정 → `review-prepare.ps1 -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>]` 호출 (canonical pass directory 발급 + input.md seed) → pass `input.md` 본문 직접 작성 (`templates/review-input.md` 기준) → `review-run.ps1 -ReviewTaskId <id> -Perspective <viewpoint> -Pass <pass-NN>` 호출 (verify + Codex 1 회 실행) → pass `result.md` 의 `## Verdict` 확인 → review-task path / final pass / verdict / corrective loop count 보고. 변경 파일이 0 개면 중단 (Mode B 로 자동 전환하지 않음). |
...
> **Note — procedure home + source snippet alignment.** Batch 2C-2 이후 BF save / explicit Brief restore **절차 자체** 는 `snippets/claude-skills/ai-harness-brief/SKILL.md` (ai-harness-brief skill) 가 소유한다; source `snippets/CLAUDE_SNIPPET.md` / `snippets/AGENTS_SNIPPET.md` 에서 `## BF save / checkpoint protocol` 섹션이 제거되고 `## Brief` 섹션은 BF-lv3 non-claim 만 남았으며, 이후 routing-pointer-removal batch 로 snippet 의 skill-routing pointer 까지 제거되었고, 다시 **Batch 3** 에서 `## Brief` 섹션 자체가 제거되었다 (`docs/systems/skills/STATUS.md` SK-05) — snippet 은 skill 로 routing 하지 않으며 discovery 는 각 skill 의 `description` 가 담당한다. 본 절의 description 과 그 skill 본문은 모두 `docs/contracts/brief/BRIEF_CONTRACT.md` 의 현행 (3차 reconciliation) framing 을 따른다 — canonical Brief 는 `<ProjectRoot>/log/brief/BRIEF.md` 한 자리이고 root `<ProjectRoot>/brief/` 는 rejected, user-home operator-local runtime root 도 rejected, target persistent footprint 는 `<ProjectRoot>/log/` only 다. Brief 가 유일한 restore source 다. 운영자가 이전 라운드에 destination `CLAUDE.md` / `AGENTS.md` 의 managed block 에 적용한 본문은 그 시점의 snippet 본문을 그대로 가지고 있으며, source snippet 본문이 갱신된 뒤에도 자동으로 refresh 되지 않는다 — destination managed-block refresh 는 사용자 명시 승인을 요구하는 별도 managed-block replacement step 이다 (`docs/decisions/GLOBAL_ADOPTION_DECISION.md` §6). 본 가이드 / contract doc / source snippet 본문과 destination managed block 의 framing 이 충돌하면, refresh 가 적용될 때까지 **현행 contract doc (`docs/contracts/brief/BRIEF_CONTRACT.md`) 의 framing 이 우선** 한다.
...
- 명시적 복원 요청 시 canonical Brief 가 없으면 부재를 보고하고 다음 행동을 묻는다 — Brief 가 유일한 restore source 다.
...
위 7b 본문의 UX 는 현행 (3차 reconciliation) contract doc (`docs/contracts/brief/BRIEF_CONTRACT.md`) 의 framing 을 따른다. source `snippets/CLAUDE_SNIPPET.md` / `snippets/AGENTS_SNIPPET.md` 의 `## Brief` 섹션은 Batch 3 에서 제거되었으므로 (위 line 191 참조), 그 framing 은 이제 contract doc 에만 존재하고 snippet 은 더 이상 Brief framing 을 restate 하지 않는다. 운영자가 이전 라운드에 destination `CLAUDE.md` / `AGENTS.md` 의 managed block 에 적용한 본문은 그 시점의 snippet 을 가지고 있으며, source snippet 본문이 갱신된 뒤에도 자동으로 refresh 되지 않는다 — destination managed-block refresh 는 사용자 명시 승인이 필요한 별도 managed-block replacement step 이다 (위 절두 note 참조). framing 충돌 시 현행 contract doc 가 우선이다.
...
본 절의 명령은 일상 운용용이 아니라, §7-§8 의 자연어 UX 가 사용 불가능하거나 인자 조합을 직접 디버그해야 할 때를 위한 fallback / reference 다. 일상 운용은 §7 의 자연어 의도가 정식 entrypoint 이며, 진입 script 두 개를 두 단계로 호출한다 — (a) `scripts/review-prepare.ps1 -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>]` 로 canonical pass directory 발급 + input.md seed → (b) AI 가 pass `input.md` 본문 작성 → (c) `scripts/review-run.ps1 -ReviewTaskId <id> -Perspective <viewpoint> -Pass <pass-NN>` 로 verify + Codex 1 회 실행.

canonical artifact 는 `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/input.md` 와 `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/result.md` 두 파일뿐이다 (`<perspective>` 는 required; canonical layout 은 three-level). 다른 sidecar 파일은 contract 의 일부가 아니다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md`).
...
```powershell
# 1. log/ 하위 디렉터리 (log/brief/, log/evidence/, log/review/) 는 거기에 쓰는 primitive
#    (brief-init.ps1 -> log/brief/, review-prepare.ps1 -> log/review/<...>/) 가 필요 시 자동 생성한다.
#    별도 log tree 초기화 단계는 필요 없다.

# 2. <project-root>/.gitignore 에 log/ 포함 확인 (사용자 책임)

# 3. review 진입 (canonical 두 단계 흐름; 운영자 / AI 가 <review-task-id>, <perspective>(필수), pass-NN 을 미리 결정):
#    (a) review-prepare.ps1 -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>] -Stage <stage> -Purpose <line>
#        호출 -- canonical three-level pass directory <review-task-id>/<perspective>/pass-NN/ 를 발급하고
#        input.md 를 templates/review-input.md 로부터 seed. -Perspective 는 필수
#        (미지정 / 빈 값이면 fail-fast; two-level fallback 없음).
#        -Pass 가 생략되면 같은 <task>/<perspective>/ directory 의 기존 pass 를 스캔해
#        다음 번호를 자동 할당 (per-perspective).
#    (b) AI 가 pass input.md 본문을 review request 로 직접 작성.
#    (c) review-run.ps1 -ReviewTaskId <id> -Perspective <viewpoint> -Pass <pass-NN> 호출 -- review-input-verify.ps1
...
```powershell
# log/ 하위 디렉터리는 primitive 가 필요 시 자동 생성한다 (별도 log tree 초기화 단계 없음).
# 위 shared / global 모드의 review 진입 두 단계 흐름과 동일하게:
#   (a) -File scripts/review-prepare.ps1 -ToolRoot (Get-Location).Path `
#       -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>] -Stage <stage> -Purpose <line>
#       (canonical three-level <review-task-id>/<perspective>/pass-NN/ pass directory 가 만들어진다.)
#   (b) AI 가 log/review/<review-task-id>/<perspective>/pass-NN/input.md 본문 직접 작성
...
| `no` | review scope 안 blocking finding 존재 | `## Blocking findings` 본문 각 항목을 batch / `/goal` approved scope 안 / 밖으로 분류. scope 안 finding 은 corrective patch + validation + 같은 `<review-task-id>/<perspective>/` 아래 새 `pass-NN/` 로 corrected-state re-review. scope 밖 finding 은 stop / report 후 별도 scoped 승인 요청. `no` verdict 만으로 batch closure 금지. |
...
## 12b. Acceptance scenario 분리 — review 와 BF 는 별도 평가 축
...
- Pure review-loop acceptance 는 BF artifact 의 존재를 요구하지 않는다.
- BF artifact 는 BF save / closeout / handoff / explicit Brief restore intent 가 명시적으로 scope 안에 있을 때에만 기대된다.
- pure review test repo 에서 BF artifact 가 없는 것은 review subsystem 의 실패가 아니다.
- dogfooding target repo 라도, 채택한 snippet 또는 사용자 발화에 BF save / closeout 의도가 포함된 경우에만 BF save protocol 이 `<ProjectRoot>/log/brief/BRIEF.md` (canonical Brief — project-local runtime artifact, gitignored under `log/`) 자리에 manual save 를 수행한다. root `<ProjectRoot>/brief/` 는 만들지 않는다 (rejected).
- review artifact 와 BF artifact 는 서로 다른 평가 축이다. 한 축의 부재 또는 fail 로 다른 축을 fail 처리하지 않는다.

시나리오별 기대 artifact:

| Scenario | 기대 artifact |
|---|---|
| Pure review-loop acceptance | `log/review/<review-task-id>/<perspective>/pass-NN/input.md`, `log/review/<review-task-id>/<perspective>/pass-NN/result.md` (canonical three-level layout; `-Perspective` required) |
| BF save / closeout acceptance | `<ProjectRoot>/log/brief/BRIEF.md` (canonical Brief — project-local runtime artifact, gitignored under `log/`). BF save protocol 의 갱신 대상도 이 자리이며, root `<ProjectRoot>/brief/` 는 만들지 않는다 (rejected). Brief 가 유일한 restore source 다. |
...
[ ] target project 안에서 brief-init.ps1 / review-prepare.ps1 실행 후 log/{brief,review}/ 가 자동 생성된다.
[ ] AI 가 사용자의 자연어 의도로부터 review-prepare.ps1 -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>] 호출 -> pass input.md 본문 직접 작성 -> review-run.ps1 -ReviewTaskId <id> -Perspective <viewpoint> -Pass <pass-NN> 호출의 두 단계로 한 pass review 를 닫는다.
...
[ ] 각 pass directory <review-task-id>/<perspective>/pass-NN 아래 canonical artifact 두 파일 (input.md, result.md) 이 모두 존재한다.
...
[ ] target project 의 .gitignore 에 log/ 가 포함되어 있다.
[ ] .ai-harness/scripts/brief-init.ps1 / review-prepare.ps1 실행 후 log/{brief,review}/ 가 자동 생성된다.
[ ] .ai-harness/scripts/review-prepare.ps1 -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>] 호출 -> AI 가 pass input.md 본문 직접 작성 -> .ai-harness/scripts/review-run.ps1 -ReviewTaskId <id> -Perspective <viewpoint> -Pass <pass-NN> 호출의 두 단계로 한 pass review 가 닫힌다.
[ ] 각 pass directory <review-task-id>/<perspective>/pass-NN 아래 canonical artifact 두 파일 (input.md, result.md) 이 모두 존재한다.
...
4. log/ 하위 디렉터리는 이후 primitive (brief-init.ps1 / review-prepare.ps1) 실행 시 자동 생성되므로 별도 초기화 단계는 필요 없다.
5. test repo 안의 임의 파일 1개에 사소한 변경을 만든다 (예: README.md 한 줄 추가).
6. Claude Code 에 자연어로 review 의도를 표현한다 (`현재 진행한 작업 코덱스 리뷰 진행해`). Claude Code 는 이 작업의 `<review-task-id>` (+ 필수 `<perspective>`) 와 `pass-NN` 을 결정한 뒤 `review-prepare.ps1 -ReviewTaskId <id> -Perspective <viewpoint> [-Pass <pass-NN>]` 호출 → pass `input.md` 본문 직접 작성 → `review-run.ps1 -ReviewTaskId <id> -Perspective <viewpoint> -Pass <pass-NN>` 호출의 두 단계 흐름으로 한 pass review 를 닫는다.
...
- `no` — `## Blocking findings` 본문을 보고 각 finding 을 사용자가 사전 승인한 batch / `/goal` approved scope 안 / 밖으로 분류한다. **scope 안 finding**: corrective patch + repo-local validation + 같은 `<review-task-id>/<perspective>/` 아래 새 `pass-NN/` 로 corrected-state re-review 진행 (사용자의 기존 batch 승인 안에서). **scope 밖 finding**: stop / report 후 별도 scoped 승인 요청 — 본 batch 안에서 silently 흡수 금지. `no` verdict 만으로 batch closure 금지 — concrete re-review 결과 (`yes` 또는 사용자 명시 risk-acceptance 가 적용된 `yes with risk`) 까지 진행해야 batch 가 닫힌다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md` §6a 와 동일).
...
verdict 의 source-of-truth 는 같은 review task 의 final pass directory `log/review/<review-task-id>/<perspective>/pass-NN/result.md` (`<perspective>` 는 required; canonical layout 은 three-level) 의 `## Verdict` 다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md`). 같은 `result.md` 안의 4 required disclosure H2 (`## Blocking findings` / `## Non-blocking concerns` / `## Review limitations` / `## Assumptions relied on`) 본문은 verdict 의 합리적 후속 판단 (commit / push / risk acceptance / corrective pass) 의 input 으로 함께 읽는다.

...
위 7b 본문의 UX 는 현행 (3차 reconciliation) contract docs (`docs/contracts/brief/BRIEF_CONTRACT.md`, `docs/contracts/chatlog/CHATLOG_CONTRACT.md`) 의 framing 을 따른다. source `snippets/CLAUDE_SNIPPET.md` / `snippets/AGENTS_SNIPPET.md` 의 `## Brief` / `## Chatlog` 섹션은 Batch 3 에서 제거되었으므로 (위 line 191 참조), 그 framing 은 이제 contract docs 에만 존재하고 snippet 은 더 이상 Brief/Chatlog framing 을 restate 하지 않는다. 운영자가 이전 라운드에 destination `CLAUDE.md` / `AGENTS.md` 의 managed block 에 적용한 본문은 그 시점의 snippet 을 가지고 있으며, source snippet 본문이 갱신된 뒤에도 자동으로 refresh 되지 않는다 — destination managed-block refresh 는 사용자 명시 승인이 필요한 별도 managed-block replacement step 이다 (위 절두 note 참조). framing 충돌 시 현행 contract docs 가 우선이다.
...
- BF / Chatlog artifact 는 BF save / closeout / handoff / explicit Brief restore intent 가 명시적으로 scope 안에 있을 때에만 기대된다.

...
- 결과 record 는 `<project-root>/log/review/<review-task-id>/pass-NN/` (또는 perspective 지정 시 `<project-root>/log/review/<review-task-id>/<perspective>/pass-NN/`) 의 canonical pair (`input.md`, `result.md`) 두 파일로 닫힌다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md`). `<review-task-id>` 는 하나의 `/goal` 작업 또는 하나의 review gate 단위이며 Claude Code chat / session id 가 아니다. `<perspective>` (optional) 는 review viewpoint 를 별도 path segment 로 분리하는 operator-named 값이며 미지정이 default (two-level) 다. `pass-NN` 는 같은 task (perspective 지정 시 같은 perspective) 의 corrective loop 안에서의 각 Codex review attempt 다.
...
└── review/    # review record (docs/contracts/review/REVIEW_RESULT_CONTRACT.md)
    └── <review-task-id>/
        ├── pass-01/                  # default two-level layout (no perspective)
        ...
        │   ├── input.md
        │   └── result.md
        └── <perspective>/            # optional three-level layout (-Perspective <viewpoint>)
...
- `<perspective>` (optional) 는 review viewpoint (예: `local-correctness` / `system-coherence`) 다. `-Perspective <viewpoint>` 로 명시하면 `<review-task-id>` 와 `pass-NN` 사이에 별도 path segment 로 들어가 three-level layout 이 되고, 미지정이면 two-level default 다. operator 가 명시하며 자동 추론하지 않고, single path segment (`..` / `/` / `\` / `pass-NN` 형태 금지, safe charset/length) 여야 한다.
- `pass-NN` 은 corrective loop attempt 다. 첫 attempt `pass-01`, 다음 `pass-02`, ... 로 증가한다. `review-prepare.ps1` 가 같은 pass-parent directory (perspective 미지정 시 task directory, 지정 시 `<task>/<perspective>/`) 의 기존 pass 를 스캔해 다음 번호를 자동 할당하거나, `-Pass <pass-NN>` 로 명시한다 — perspective 가 있으면 pass 번호는 per-perspective 다.
...
- canonical review artifact 는 `log/review/<review-task-id>[/<perspective>]/pass-NN/input.md` 와 `log/review/<review-task-id>[/<perspective>]/pass-NN/result.md` 두 파일뿐이다 (`<perspective>` 는 optional; 미지정 시 two-level). 다른 sidecar 파일은 contract 의 일부가 아니다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md`).
...
canonical artifact 는 `<ProjectRoot>/log/review/<review-task-id>[/<perspective>]/pass-NN/input.md` 와 `<ProjectRoot>/log/review/<review-task-id>[/<perspective>]/pass-NN/result.md` 두 파일뿐이다 (`<perspective>` 는 optional; 미지정 시 two-level). 다른 sidecar 파일은 contract 의 일부가 아니다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md`).
...
# 3. review 진입 (canonical 두 단계 흐름; 운영자 / AI 가 <review-task-id>, (optional) <perspective>, pass-NN 을 미리 결정):
#    (a) review-prepare.ps1 -ReviewTaskId <id> [-Perspective <viewpoint>] [-Pass <pass-NN>] -Stage <stage> -Purpose <line>
#        호출 -- canonical pass directory (-Perspective 미지정 시 <review-task-id>/pass-NN/,
#        지정 시 <review-task-id>/<perspective>/pass-NN/) 를 발급하고
#        input.md 를 templates/review-input.md 로부터 seed.
#        -Pass 가 생략되면 같은 pass-parent directory (perspective 미지정 시 task directory,
#        지정 시 <task>/<perspective>/) 의 기존 pass 를 스캔해 다음 번호를 자동 할당 (per-perspective).
...
#       (canonical <review-task-id>/pass-NN/ 또는 <review-task-id>/<perspective>/pass-NN/ pass directory 가 만들어진다.)
#   (b) AI 가 log/review/<review-task-id>/pass-NN/input.md (perspective 지정 시
#       log/review/<review-task-id>/<perspective>/pass-NN/input.md) 본문 직접 작성
...
|---|---|
| Pure review-loop acceptance | `log/review/<review-task-id>/pass-NN/input.md`, `log/review/<review-task-id>/pass-NN/result.md` (canonical layout; with `-Perspective`, the optional three-level `log/review/<review-task-id>/<perspective>/pass-NN/`) |
...
verdict 의 source-of-truth 는 같은 review task 의 final pass directory `log/review/<review-task-id>[/<perspective>]/pass-NN/result.md` (`<perspective>` 는 optional; 미지정 시 two-level) 의 `## Verdict` 다 (`docs/contracts/review/REVIEW_RESULT_CONTRACT.md`). 같은 `result.md` 안의 4 required disclosure H2 (`## Blocking findings` / `## Non-blocking concerns` / `## Review limitations` / `## Assumptions relied on`) 본문은 verdict 의 합리적 후속 판단 (commit / push / risk acceptance / corrective pass) 의 input 으로 함께 읽는다.

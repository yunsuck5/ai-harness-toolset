...
source-of-truth commit: `595d35d1041caba2ca483e093a8ef66dc23964c2` (commit `Record file-backed review request backlog`).
...
- **`<SourceRepoRoot>`** — `ai-harness-toolset` source repo 의 working tree (source-of-truth commit `595d35d`). 본 path 는 모든 SC 에서 정의되며 read-only invariant 의 대상 (SC7) 이다. channel resolution 결과와 무관하게 본 path 의 상태는 변화하지 않아야 한다.
...
- **A5.** `<ProjectRoot>/brief/BRIEF.md` 가 존재하지 않는다.
- **A6.** `<ProjectRoot>/.gitignore` 가 `log/` 를 무시한다. `brief/` 는 default 로 tracked 후보다 (gitignore 에 포함하지 않음).
...
- **A8.** Source repo (`<SourceRepoRoot>`) 의 working tree 가 source-of-truth commit 이며 `git status --porcelain=v1` 결과가 빈 문자열. smoke 도중 `<SourceRepoRoot>` 안에 새 / 변경 파일이 발생하면 SC7 fail.
...
- SC 간 inter-case state coupling 이 없으므로, 한 SC 의 산출물 (예: SC2 가 seed 한 untracked `brief/BRIEF.md`) 이 다음 SC (예: SC5 의 untracked-clean 요구) 를 오염시키지 않는다.
- evidence 수집 비용이 증가하지만, 진단성과 재현성에서 우위.

**Alternative: sequenced fixture with explicit reset / commit steps.**
...
- SC2 → SC3: SC3 의 action 이 brief-init 이면, SC2 가 seed 한 `<ProjectRoot>/brief/BRIEF.md` 가 이미 존재해 "already exists" 로 refuse 된다. 그 경우 다음 중 하나를 적용한다.
  1. `git -C <ProjectRoot> add brief/BRIEF.md && git -C <ProjectRoot> commit -m 'smoke: seed brief'` 후 SC3 직전에 `Remove-Item <ProjectRoot>\brief\BRIEF.md` 로 파일만 제거 (commit 은 evidence 에 남는다).
  2. SC3 만 fresh fixture 로 분리.
- SC3 → SC4: 추가 절차 없음.
- **SC2 또는 SC3 → SC5 (review-cycle).** SC2 / SC3 가 seed 한 `brief/BRIEF.md` 는 untracked 이고, review-cycle 의 untracked 가드는 `brief/` 를 제외하지 않는다 (D7 의 의도된 정책). SC5 호출 전 다음 중 하나를 명시한다.
  1. `git -C <ProjectRoot> add brief/BRIEF.md && git -C <ProjectRoot> commit -m 'smoke: seed brief'` — tracked 로 전환.
  2. `git -C <ProjectRoot> rm -f --cached brief/BRIEF.md 2>$null; Remove-Item -Recurse -Force <ProjectRoot>\brief` — 제거.
  3. SC5 만 fresh fixture 로 분리.
...
- **Pass.**
  - exit 0.
  - `<ProjectRoot>/brief/BRIEF.md` 가 생성됨. SHA-256 이 `<source-repo>/templates/brief/BRIEF.md` 와 일치.
...
- **Pass.**
  - exit 0.
  - `<ProjectRoot>/brief/BRIEF.md` 가 생성되고 hash 일치.
...
- **Pass.**
  ...
  - `<ProjectRoot>/brief/` 가 생성되지 않음.
- **Fail.** exit 0, channel trace 누락 (stdout + stderr 결합 transcript 기준), BRIEF 디렉터리 생성.
...
| `BriefRoot` | `<ProjectRoot>/brief`. brief-init 이 한 번 seed. `<ToolRoot>/templates/brief/BRIEF.md` 의 byte-equal copy. | SC2, SC3 (SHA-256). |
...
3. case-별 산출물:
   ...
   - SC2 / SC3. seed 된 `<ProjectRoot>/brief/BRIEF.md` 와 `<ToolRoot>/templates/brief/BRIEF.md` 의 SHA-256.
...
4. test-level meta — `<run-id>` 매핑, UTC 실행 시각, 운영자, source-of-truth commit (`595d35d`), fixture lifecycle (fresh vs sequenced + 적용된 reset / commit step 의 verbatim 기록).
...
- `<SourceRepoRoot>` working tree 가 source-of-truth commit 과 다름.
...
- §Target 의 commit hash (`595d35d`) 는 본 criteria 가 binding 하는 source-of-truth 다. 후속 commit 으로 contract 가 변경되면 본 criteria 의 영향 범위를 별도 라운드에서 재평가한다.

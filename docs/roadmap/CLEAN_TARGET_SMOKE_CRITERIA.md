...
4. test-level meta — `<run-id>` 매핑, UTC 실행 시각, 운영자, **execution HEAD** (실행 시점 `<SourceRepoRoot>` 에서 관찰된 git HEAD; `git rev-parse HEAD` 의 full hash 와 `git status --porcelain=v1` 결과를 evidence 에 그대로 기록한다 — 본 criteria 가 특정 commit 으로 pin 하지 않으므로 evidence 가 유일한 HEAD 기록 권한이다), fixture lifecycle (fresh vs sequenced + 적용된 reset / commit step 의 verbatim 기록).

...
- 기존 guide / contract / audit 문서 안의 legacy review-cycle path 참조 정합화. canonical review task/pass topology 채택 이후, `docs/roadmap/REVIEW_EFFORT_GUIDE.md` 및 그 외 `docs/` / `docs/roadmap/` 의 historical / operational 문서들에 남아 있는 `scripts/review-cycle.ps1`, `meta.json`, `result.json`, `target-files.list`, `log/review-targets/`, `log/review-requests/`, `-TargetFilesPath`, `-ReviewRequestPath`, `templates/review-meta.json`, `templates/review-result.*` 참조는 현행 normal operator path / canonical record 와 일치하지 않는다. audit / decision / contract design / smoke criteria docs 의 historical record (`docs/roadmap/TOOLROOT_PROJECTROOT_AUDIT.md`, `docs/roadmap/SHARED_GLOBAL_INVOCATION_CONTRACT.md`, `docs/roadmap/CLEAN_TARGET_SMOKE_CRITERIA.md`, `docs/roadmap/GLOBAL_ADOPTION_DECISION.md`) 안의 review-cycle 언급은 각 문서가 작성된 시점의 historical record 로 그대로 보존하는 것이 자연스럽고, 실제 정합화 대상은 주로 운영 권고 / 사용자 가이드 류 문서 (예: `docs/roadmap/REVIEW_EFFORT_GUIDE.md` §4 / §6 / §7 / §9 / §11) 다. 본 정합화는 §2 의 maintenance scope 안의 contract clarification 으로 분류되며, 별도 scoped 승인을 거친다. 본 status 진술만으로 정합화 작업이 시작되지 않는다.

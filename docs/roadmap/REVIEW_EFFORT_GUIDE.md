...
`-TargetFiles` (단일 파일) 또는 `-TargetFilesPath` (다중 파일 list) 로 review 대상 파일을 명시적으로 지정한다. 자동 git status 기반 detection 은 fallback 이며, 정합성 있는 review 에는 명시적 지정을 권장한다 (`docs/OPERATOR_GUIDE_KR.md` §9).

...
> **Superseded (BRIEF posture reconciliation).** 위 §6 Layer 4 의 BRIEF 관련 wording — `<ProjectRoot>\brief\`
> 를 별도 footprint 항목으로 둔 것, "self-dogfooding ProjectRoot 의 `brief/`", 그리고 "`log/brief/` 디렉터리는
> 어느 경우에도 만들지 않는다" — 은 모두 §9.3 의 reconciliation note 로 superseded 되었다. 현재 BRIEF 의 canonical
> 위치는 `<ProjectRoot>/log/brief/BRIEF.md` 이며, `log/brief/` 는 만들어야 하는 BRIEF 의 자리다.
> root `<ProjectRoot>/brief/` 는 ai-harness 용도로 금지되고, project-local footprint 는 `log/` only 다.
> canonical source-of-truth 는 `docs/BRIEF_CONTRACT.md` 다. 위 본문은 historical model 기록으로 보존한다.
...
> **BRIEF wording note — §9 전체에 적용.** §9.1–§9.5 본문에 등장하는 `brief/` (= root `<ProjectRoot>/brief/`) 표현은 **historical model 기록**이며 current live artifact 가 아니다. **현행 reconciled 기준** (§9.3 의 "Reconciliation 완료" note 및 canonical source-of-truth `docs/BRIEF_CONTRACT.md` 참조): BRIEF 의 canonical 위치는 `<ProjectRoot>/log/brief/BRIEF.md` 이고, `log/` 아래 operator-local runtime state (기본 gitignored durable restore state) 다. root `<ProjectRoot>/brief/` 는 ai-harness 용도로 만들지 않는다. target persistent footprint 는 `log/` only 이며 BRIEF 는 그 `log/` 트리 안 `log/brief/` 아래에 산다. 따라서 §9 본문의 "`log/` + `brief/`" 또는 "project-local `brief/`" 표현은 모두 `log/` only (BRIEF 는 `log/brief/BRIEF.md`) 로 읽는다. 이 note 는 `log/chatlog/current/resume.md` · `summary.md` (BF Level 1/2 session restore artifact) 와 `log/brief/BRIEF.md` (BF Level 3 durable restore artifact) 의 역할 구분을 바꾸지 않는다.
...
- self-dogfooding ProjectRoot 로 동작할 때, `log/` 와 `brief/` 가 project-local state 로 존재해도 된다. (historical wording — 위 §9 BRIEF wording note 참조: 현행 기준은 `log/` only, BRIEF 는 `log/brief/BRIEF.md`.)
...
- **Reconciliation 완료 (BRIEF posture reconciliation) — 문서 전체 적용.** 위에서 예고한 별도 scoped work 가 수행되었다. 그 결과 BRIEF 의 canonical 위치는 `<ProjectRoot>/log/brief/BRIEF.md` 이고, BRIEF 는 `log/` 아래 operator-local runtime state 이며, root `<ProjectRoot>/brief/` 는 ai-harness 용도로 금지된다. target project 의 persistent footprint 는 `log/` only 다.
  - **본 superseding note 는 문서 전체에 적용된다 (섹션 enumeration 이 아님).** `GLOBAL_INSTALL_UPDATE_MODEL.md` 어느 절이든 — §1, §5, §6, §7, §8, §9.4, §10, §11 등 — 에 등장하는 다음 wording 은 모두 **stale** 이며 본 note 로 superseded 된다: target footprint 를 `log/` + `brief/` 로 적은 모든 표현, `targetFootprintPolicy: log-brief-only`, `<ProjectRoot>/brief/` 또는 `<ProjectRoot>\brief\` 를 별도 footprint / project-local state 항목으로 둔 모든 언급, "project-local log/ + brief/" 형태의 diagram node 와 본문. 이 모든 것을 `log/` only (BRIEF 는 그 `log/` 트리 안의 `log/brief/` 아래) 로 읽는다.
  - §6 Layer 4 절에는 그 자리에 별도 superseding note 도 두었다. 그 외 절들의 본문·list·diagram·metadata 예시는 historical model 기록으로 그대로 보존하며, rewrite 하지 않는다. 충돌이 보이면 본 §9.3 note 와 `docs/BRIEF_CONTRACT.md` 가 우선한다.
  - canonical source-of-truth 는 `docs/BRIEF_CONTRACT.md` 다. `scripts/brief-init.ps1` 의 `Test-IsSourceRepoRoot` refuse guard 도 본 reconciliation 에서 제거되었다 (BRIEF 가 gitignored `log/` 아래로 이동하면서 그 guard 의 근거가 사라졌다).
...
- runtime artifact 가 ProjectRoot-local (`log/`, `brief/`) 로만 생성되는지 확인한다. (`brief/` 표현은 위 §9 BRIEF wording note 참조 — 현행 기준은 `log/` only, BRIEF 는 `log/brief/BRIEF.md`.)
- target footprint = `log/` + `brief/` only 원칙이 외부 target 에 적용 가능한지 확인한다. (현행 기준은 `log/` only — 위 §9 BRIEF wording note 참조.)

...
> **Superseded — two-step reconciliation.** 위 §6 Layer 4 의 BRIEF 관련 wording — `<ProjectRoot>\brief\` 를
> 별도 footprint 항목으로 둔 것, "self-dogfooding ProjectRoot 의 `brief/`", 그리고 "`log/brief/` 디렉터리는
> 어느 경우에도 만들지 않는다" — 은 §9.3 의 reconciliation list 가 기록하는 두 단계의 변천을 거쳤다.
> **(1) 1차 reconciliation (historical):** BRIEF 의 canonical 위치를 `<ProjectRoot>/log/brief/BRIEF.md` 로 옮기고
> root `<ProjectRoot>/brief/` 를 forbidden 으로 둔 framing. project-local footprint 는 `log/` only.
> **(2) 2차 reconciliation (현행 기준):** 그 framing 자체가 후속 정정 라운드에서 정정되었다.
...
> canonical source-of-truth 는 `docs/BRIEF_CONTRACT.md` 와 `docs/CHATLOG_CONTRACT.md` 다. 위 §6 Layer 4 본문은
> historical model 기록으로 보존하며, 본 note 의 2차 framing 이 우선한다.
...
> **BRIEF wording note — §9 전체에 적용 (two-step reconciliation).** §9.1–§9.5 본문에 등장하는 `brief/` (= root `<ProjectRoot>/brief/`) 와 `log/brief/BRIEF.md` 표현은 **historical / intermediate model 기록**이다. 두 단계의 reconciliation 을 거쳤다. **(1) 1차 reconciliation** (당시 stage): canonical 을 `<ProjectRoot>/log/brief/BRIEF.md` 로 옮기고 root `<ProjectRoot>/brief/` 를 forbidden 으로 둔 framing. §9.3 의 "Reconciliation 완료" note 가 이 1차 단계를 기록한다. **(2) 2차 reconciliation (현행 기준)**: 그 framing 자체가 정정되었다. **target repo product canonical Brief 는 `<ProjectRoot>/brief/BRIEF.md`** 이며, `<ProjectRoot>/log/brief/BRIEF.md` 는 현재 `scripts/brief-init.ps1` 의 seed destination (operator-local runtime artifact, gitignored under `log/`) 이고 product canonical 로 승격되지 않는다. **root `<ProjectRoot>/brief/` 는 forbidden 이 아니다.** BF Level 은 path 가 아니라 save / restore capability maturity 다 (BF Level 1/2 = manual save/restore discipline; BF Level 3 = deterministic Brief maintenance / validation / stale warning / session-start guidance / restore-offer — 미구현 future scoped work). canonical source-of-truth 는 `docs/BRIEF_CONTRACT.md` 와 `docs/CHATLOG_CONTRACT.md` 다. 본 §9 어느 절의 wording 이라도 위 2차 기준과 충돌하면 본 note 와 contract docs 가 우선한다. `log/chatlog/current/resume.md` · `summary.md` 는 canonical BF Level 1/2 artifact 가 아니라 failed intermediate / legacy migration source / deprecation candidate 다 (`docs/CHATLOG_CONTRACT.md`).
...
- self-dogfooding ProjectRoot 로 동작할 때, `log/` 와 `brief/` 가 project-local state 로 존재해도 된다. (historical wording — 위 §9 BRIEF wording note 참조: 현행 기준은 target product canonical Brief 가 `<ProjectRoot>/brief/BRIEF.md` 이고, `<ProjectRoot>/log/brief/BRIEF.md` 는 source-side primitive seed destination 이다.)
...
즉 self repo 에서 `scripts/` `config/` `snippets/` `templates/` 는 **source payload** 이고, `log/` 와 `brief/` 는 **project-local state** 다. 이 둘은 문서상 명확히 분리된다 — self repo 가 ProjectRoot 로도 동작한다는 사실이, project-local state 를 source payload 로 승격시키지 않는다. (source payload vs project-local state 의 분리라는 decision 은 유효하나, project-local state 의 자리 표현은 위 §9 BRIEF wording note 참조 — 현행 기준은 target product canonical Brief 가 `<ProjectRoot>/brief/BRIEF.md` 이고 `<ProjectRoot>/log/brief/BRIEF.md` 는 source-side primitive seed destination 이다.)
...
- **Reconciliation 2차 (현행 기준) — 문서 전체 적용.** 1차 framing 자체가 후속 정정 라운드에서 정정되었다. **target repo product canonical Brief 는 `<ProjectRoot>/brief/BRIEF.md`** 이며, `<ProjectRoot>/log/brief/BRIEF.md` 는 현재 `scripts/brief-init.ps1` 의 seed destination — operator-local runtime artifact (gitignored under `log/`) — 이고 **product canonical 로 승격되지 않는다.** **root `<ProjectRoot>/brief/` 는 forbidden 이 아니다.** BF Level 은 path 가 아니라 save / restore capability maturity 다; BF Level 3 (deterministic Brief maintenance / validation / stale warning / session-start guidance / restore-offer) 는 미구현 future scoped work 다. `log/chatlog/current/resume.md` · `summary.md` 는 canonical BF Level 1/2 artifact 가 아니라 failed intermediate / legacy migration source / deprecation candidate 다. canonical source-of-truth 는 `docs/BRIEF_CONTRACT.md` 와 `docs/CHATLOG_CONTRACT.md` 다. **본 §9 어느 절의 wording (1차 framing 기준이든 그 이전 historical wording 이든) 이 2차 framing 과 충돌하면 본 2차 note 와 contract docs 가 우선한다.**
  - §6 Layer 4 절의 superseding note 는 1차 reconciliation 기준이다 — 동일하게 2차 framing 으로 읽는다. 그 외 절들의 본문·list·diagram·metadata 예시는 rewrite 하지 않는다.
  - `scripts/brief-init.ps1` 는 본 2차 framing 의 BF Level 3 deterministic capability 와 별개의 narrow source-side primitive 다. 그 writer destination 을 target canonical (`<ProjectRoot>/brief/BRIEF.md`) 로 routing 하는 작업은 future scoped work 다. 1차 reconciliation 에서 제거된 `Test-IsSourceRepoRoot` refuse guard 는 그대로 제거된 상태를 유지한다.
...
- runtime artifact 가 ProjectRoot-local (`log/`, `brief/`) 로만 생성되는지 확인한다. (`brief/` 표현은 위 §9 BRIEF wording note 참조 — 현행 기준은 target product canonical Brief 가 `<ProjectRoot>/brief/BRIEF.md` 이고 `<ProjectRoot>/log/brief/BRIEF.md` 는 source-side primitive seed destination 이다.)
- target footprint = `log/` + `brief/` only 원칙이 외부 target 에 적용 가능한지 확인한다. (현행 기준은 위 §9 BRIEF wording note 참조 — target product canonical Brief 는 `<ProjectRoot>/brief/BRIEF.md` 이며, source-side primitive seed destination `<ProjectRoot>/log/brief/BRIEF.md` 는 `log/` tree 아래 operator-local runtime artifact 다.)

...
- path handling audit: `docs/roadmap/TOOLROOT_PROJECTROOT_AUDIT.md`
...
`docs/roadmap/TOOLROOT_PROJECTROOT_AUDIT.md` §8 은 D1–D9 의 결정 없이는 shared / global mode 의 invocation contract 가 확정되지 않는다고 기록했다. 본 문서는 그 D1–D9 에 대한 design 단계의 제안 결정을 기록하고, 그 결정 위에 invocation contract 의 shape 를 정의한다.
...
- `docs/roadmap/TOOLROOT_PROJECTROOT_AUDIT.md` §6 의 gap G1–G10.
- 같은 audit §7 의 blocker B1–B5.
- 같은 audit §8 의 required decisions D1–D9.
- `docs/roadmap/GLOBAL_ADOPTION_DECISION.md` §1 의 layer decision, §4 의 adoption direction, §6 의 marker policy, §7 의 explicit-trigger discipline, §8 의 ToolRoot / ProjectRoot conceptual split, §10 의 non-goals.
- `docs/roadmap/GLOBAL_ADOPTION_PROCEDURE.md` §4–§7 의 Claude skill global / update / removal 절차.
- `scripts/lib/path.ps1` 의 현재 path 모델 (read-only inspection).
- 본 audit 가 식별한 lifecycle script 들의 현재 `-ProjectRoot` / `-ToolRoot` 사용 (`scripts/review-cycle.ps1`, `scripts/review-prepare.ps1`, `scripts/review-run.ps1`, `scripts/review-verify.ps1`, `scripts/log-init.ps1`, `scripts/brief-init.ps1`, `scripts/brief-check.ps1`).
...
본 design 은 `docs/roadmap/GLOBAL_ADOPTION_DECISION.md` §8 의 path model 을 그대로 사용한다.
...
- **Shared / global mode** — `ToolRoot` 가 ProjectRoot 와 독립적인 경로. preferred direction.
...
**Decision.** 명시 우선의 priority chain 을 채택한다.

1. CLI parameter `-ToolRoot <path>` (lifecycle script 의 직접 invocation).
2. env var `AI_HARNESS_TOOL_ROOT` (shell-level / session-level).
3. dogfooding detection (D3 의 multi-marker 가 ProjectRoot 에서 모두 발견된 경우, `ToolRoot = ProjectRoot`).
4. legacy `<ProjectRoot>/.ai-harness` (해당 디렉터리가 실제 존재할 때만; project-local copy mode 의 backward compat).
5. 위 어느 것도 충족되지 않으면 명시 error 후 exit non-zero. error message 는 시도된 channel 의 목록과 각 channel 이 왜 활성화되지 않았는지를 포함한다.

**Rationale.** 명시 > env > implicit. user-level config 파일 (예: `~/.ai-harness-toolset/config.json`) 은 본 단계에서 채택하지 않는다 — 새 state 파일을 도입하지 않는 운영 원칙과 정합한다 (`docs/roadmap/POST_MVP_PLAN.md` §5 의 BF Level 3 금지 항목 framing 과 동일한 결). snippet body 에 ToolRoot 절대경로를 기록하는 case (B1 의 후보 d) 도 채택하지 않는다 — snippet 은 cross-machine portable 해야 하며, machine-specific 절대경로를 적재하면 다른 운영자에게 깨지기 때문.

본 결정은 channel 의 도입이지, env var 의 자동 활성화나 user shell config 변경이 아니다. env var 는 사용자가 자기 환경에서 setting 한다.

### D2 — `Resolve-CycleScript` / `Resolve-RunScript` fallback 정책

**Decision.** 후보 (c) — fallback 시 명시적 warning 출력 후 진행. 단, ToolRoot 가 explicit (CLI param 또는 env var) 로 지정된 경우에는 fallback 없이 throw.

구체적으로 다음 분기를 따른다.

- ToolRoot 가 D1 의 channel 1 또는 channel 2 로 결정된 경우 (explicit).
  - `<ToolRoot>/<RelativePath>` 가 없으면 throw. `$PSScriptRoot` fallback 을 사용하지 않는다.
  - throw message 는 ToolRoot 경로와 missing relative path 를 모두 포함한다.
- ToolRoot 가 channel 3 (dogfooding) 또는 channel 4 (legacy `.ai-harness/`) 로 결정된 경우 (implicit).
...
**Rationale.** audit Gap G3 의 narrowed risk (mis-diagnosis 품질) 를 직접 다룬다. explicit ToolRoot 에 대한 fail-fast 는 misconfiguration 의 진단 메시지가 root cause 를 가리키게 한다. implicit ToolRoot (dogfooding / legacy) 에서는 fallback 이 운영적으로 유용하므로 유지하되, warning 으로 silent 성을 제거한다.
...
- 사용자가 자기 환경에서 invocation channel 을 어떻게 결정하는지는 본 design 문서 (`docs/roadmap/SHARED_GLOBAL_INVOCATION_CONTRACT.md`) 와 `docs/roadmap/GLOBAL_ADOPTION_PROCEDURE.md` 를 참조하도록 안내.
...
**Decision.** 후보 (a) — 새 mode 를 step 1 에 추가. 검사 순서는 다음과 같다.

1. env var `AI_HARNESS_TOOL_ROOT` 가 set 되어 있고 그 경로 아래에 `scripts/review-cycle.ps1` 이 존재하면, 그 경로를 ToolRoot 로 사용한다 (shared / global mode).
2. 그 외에는 ProjectRoot 가 D3 의 multi-marker 모두를 만족하면, ProjectRoot 를 ToolRoot 로 사용한다 (dogfooding mode).
3. 그 외에는 `<ProjectRoot>/.ai-harness/scripts/review-cycle.ps1` 가 존재하면 그 부모 디렉터리를 ToolRoot 로 사용한다 (project-local copy mode, legacy).
4. 위 어느 것도 충족되지 않으면 사용자에게 toolset payload 가 적용되지 않았다고 알리고 중단한다. skill 은 자동 install / copy 를 시도하지 않는다.

**Rationale.** D1 / D3 의 결정과 정합한다. SKILL.md 는 사용자에게 raw PowerShell args 를 묻지 않는다는 기존 contract 를 유지하며, ToolRoot 결정만 추가로 명시한다. 사용자 prompt 로 매번 ToolRoot 를 묻는 후보 (b) 는 마찰이 크므로 채택하지 않는다. shared/global 을 사용자 명시 trigger 로만 활성화하는 후보 (c) 는 D1 의 자동 channel 활용을 막아 채택하지 않는다.

본 결정은 design 단계의 합의이며, 실제 SKILL.md 수정은 별도 scoped batch 에서 수행한다 (§6 step 4).
...
- runtime 의 `Get-ToolRoot` 는 D1 의 priority chain 을 그대로 사용하되, verifier 안에서는 `-ToolRoot` 인자가 첫 번째 channel (channel 1) 의 입력이다. 즉, 호출자가 `-ToolRoot` 를 명시하면 그 값으로 channel 1 이 결정되고, 명시하지 않으면 channel 2 (env var) → channel 3 (dogfooding) → channel 4 (legacy `.ai-harness/`) 순으로 fallback 한다.
...
- 이로써 D1 channel 1 (explicit `-ToolRoot`) 로 prepare/run 된 packet 이 동일 invocation 안에서 verify 될 때, verifier 가 동일 ToolRoot context 를 재현한다. channel 2 (env var), 3 (dogfooding), 4 (legacy) 로 resolve 된 ToolRoot 도 동일하게 forward 된다.
- 사용자가 packet 생성 이후 별도 invocation 으로 `review-verify.ps1` 만 직접 실행하는 경우, `-ToolRoot` 를 명시하지 않으면 verifier 안에서 channel 2/3/4 가 그대로 동작한다. meta.json 의 `toolRoot` 와 runtime resolution 이 mismatch 면 FAIL.
...
**Rationale.** audit Gap G8 가 식별한 binding gap 을 닫는다. 단순히 verifier 안에서 `Get-ToolRoot` 만 재호출하면 D1 channel 1 (explicit `-ToolRoot`) 로 만든 packet 이 verifier 입장에서 channel 2/3/4 로 fallback 하여 false-fail 할 수 있다. 새 `-ToolRoot` parameter 와 cycle/run 단계의 forward 는 그 false-fail 을 차단한다.
...
본 enforcement 는 commit-time discipline 으로 운영되며, daemon / watcher / hook 자동 등록은 도입하지 않는다 (`docs/roadmap/POST_MVP_PLAN.md` §5 의 BF Level 3 forbidden 항목과 정합).
...
```
Get-ToolRoot(-ToolRoot $p1, -ProjectRoot $project)
  if $p1 is non-empty:
  ...
  if Test-IsSourceRepoRoot($project):   # D3 multi-marker
    return $project                                            # channel 3 (dogfooding)

  $legacy = Join-Path $project '.ai-harness'
  if Test-Path $legacy -PathType Container:
    return GetFullPath($legacy)                                # channel 4 (legacy)

  throw with channel-trace message                             # channel 5
```

channel 5 의 throw message 는 다음을 포함한다.

- 시도된 channel 의 list.
- 각 channel 이 활성화되지 않은 이유 (param empty / env empty / markers missing / legacy dir absent).
...
`-ToolRootSource` 는 D1 의 channel 1 또는 channel 2 일 때 `explicit`, channel 3 또는 channel 4 일 때 `implicit`. resolution context 를 caller 가 전달한다.
...
- SKILL.md 의 step 1 은 §4 의 D5 checklist (env var → dogfooding marker → legacy `.ai-harness/`) 를 그대로 사용한다.
- 두 곳 모두 본 design 문서 (`docs/roadmap/SHARED_GLOBAL_INVOCATION_CONTRACT.md`) 와 `docs/roadmap/GLOBAL_ADOPTION_PROCEDURE.md` 를 참조하도록 명시한다.

본 문서 작성 시점에는 snippet body / SKILL.md 가 아직 수정되지 않았다. 실제 본문 수정은 §6 의 분할 단위에서 수행한다.
...
- D1 channel 4 (legacy `<ProjectRoot>/.ai-harness/`) 의 유지. project-local copy mode 의 사용자는 본 design 적용 후에도 추가 작업 없이 그대로 동작한다.
...
- O1. env var 의 이름. 본 design 은 `AI_HARNESS_TOOL_ROOT` 를 가정하지만, 다른 prefix (`AI_HARNESS_TOOLSET_ROOT`) 도 후보다. implementation 시작 전에 한 번 더 합의한다.
- O2. D5 의 SKILL.md 분기 우선순위. shared/global (env var) 을 먼저 검사할지, dogfooding 을 먼저 검사할지의 순서는 본 design 에서는 env var 우선으로 두었다. 운영자가 source repo 안에서 작업할 때 env var 가 set 되어 있으면 dogfooding 이 가려진다. 이 효과가 실제 운영에서 마찰을 일으키는지 implementation 직후 확인이 필요하다.
...
- D1–D9 의 결정은 본 문서 안에서 명문화되며, `docs/roadmap/TOOLROOT_PROJECTROOT_AUDIT.md` §8 의 같은 질문을 닫는다.

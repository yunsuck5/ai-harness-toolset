...
**result.md (dual-authored).** `result.md` 의 verdict/disclosure 본문은 active reviewer adapter 가 작성하고, runner 는 invocation 성공 뒤 verdict와 4개 disclosure H2의 **기계적으로 완전한 candidate shape**가 성립한 경우에만 그 끝에 machine run-fact 인 provenance 블록을 append 한다 — 이 블록은 reviewer 의 판단이 아니고 새 parser gate 를 도입하지 않으며, 값의 출처는 runtime/config/adapter/self-report 이지 caller 선언이 아니다. provenance append가 실패하면 runner는 검증된 candidate 본문을 훼손하지 않은 채 `review-run: PASS`·candidate H1 token·exit 0을 유지하고 `provenance-persisted: FAILED`로 실패를 드러낸다. reviewer 호출 실패, `result.md` 미생성, verdict 또는 required disclosure shape를 갖추지 못한 결과는 각각 invocation unavailable / review result unavailable인 실패 상태이며 reviewer verdict가 아니다. 그 기계 실패 결과는 pass에 보존하되 provenance·PASS·H1 verdict token을 발명하지 않는다. shape-valid body가 operator intake에서 내부 모순 때문에 unusable로 판정될 수 있으며, 이때 이미 기록된 provenance는 실제 실행 사실로 남지만 body의 verdict token은 consumable reviewer verdict가 아니다. candidate verdict token은 `## Verdict` heading 1개 아래 첫 비어있지 않은 줄의 **lowercase 정확 일치** 로만 읽는다. 4개 disclosure H2(Blocking findings / Non-blocking concerns / Review limitations / Assumptions relied on)는 각각 정확히 1회 존재해야 한다(기계 gate). target artifact의 지속성이 severity나 closure에 영향을 주면 해당 finding prose에 이를 적고, transient라는 이유만으로 자동 nonblocking 처리하지 않으며 committed temporary artifact도 존재하는 동안 실제 target으로 본다 — 새 verdict/H2/parser field/tag는 만들지 않는다. `## Counter-argument` 는 verdict 에 대한 strongest case AGAINST 를 적는 **non-parser, strongly-recommended** 관례다 — substance 없는 round 는 짧은 literal 로 두고 ceremonial boilerplate 를 피하며, boilerplate 누적은 escalation 의 증거 입력이다. 정확한 shape 의 값은 active surface(result template + verifier)가 소유한다.

**Verdict 어휘와 next-action.** 기계적으로 완전한 candidate body에는 정확히 한 verdict token이 있고 그 어휘는 `yes`(blocking 없음 — 후속은 사용자 결정, 자동 진행 금지), `no`(blocking 있음 — 승인 scope 안 finding 은 corrective 후 같은 task·perspective 아래 새 pass 로 corrected-state re-review, scope 밖 finding 은 stop/report; `no` 만으로 batch 를 닫지 않는다), `yes with risk`(`yes` 의 동의어가 아니다 — risk substance 의 명시적 사용자 수용 또는 re-review 경로가 필요하다) 셋뿐이다. invocation/shape-result unavailable에는 token이 0개이고, shape-valid body도 operator가 내부 모순으로 unusable 판정하면 consumable reviewer verdict가 0개다. 어느 경우도 네 번째 verdict가 아니다. evidence가 구체 blocker나 bounded risk를 뒷받침하면 해당 verdict를 발행하지만, blocker 존재 여부 자체를 판단할 수 없으면 verdict를 제조하지 않고 실패로 닫는다. blocking 여부의 source-of-truth 는 `## Blocking findings` 본문이며, blocking/non-blocking 경계는 review scope 와 finding substance 가 함께 결정한다. 사용자 표가 있더라도 결과를 본 뒤 threshold를 낮추거나 blocking finding을 자동 비차단화하지 않고, false-positive 판정은 evidence와 사용자 결정을 요구하며, review 횟수 상한을 만들지 않는다. operator 는 verdict line 만으로 다음 행동을 정하지 않고 4 disclosure 본문을 함께 읽는다 — shape PASS와 runner H1 token은 commit fitness 또는 consumable judgment의 자동 보증이 아니다.
...
**Script 책임 (deterministic gate only).** review scripts 는 의미 판단을 하지 않는다. 기계 책임은: pass 경로의 review-root + task-root containment 검증 · input 의 required H2/본문/placeholder 기계 gate · reviewer preamble과 `input.md`를 caller가 조립한 strict UTF-8 no-BOM byte sequence 그대로 stdin에 전달 · reviewer CLI 의 정확히 1회 실행 · invocation 성공과 result 존재·verdict shape · 4 disclosure H2 의 1회 존재 · 종료 코드 전달 — 이것뿐이다. scripts 는 finding 의 의미·재검토 필요·후속 단계를 판단하지 않고, verdict 를 읽어 어떤 것도 자동 트리거하지 않으며, 본문을 sidecar 파일로 분산하지 않는다. runner 는 기계적으로 완전한 candidate shape가 성공한 경우에만 단일 invocation 의 **H1 run-fact**(candidate verdict token·model/model-source·effort 3종·category 2종·reviewer-safe posture·engine identity·adapter kind/version)를 stdout 으로 emit 하고 동일 실행 사실을 provenance 블록으로 result 에 append 한다. H1 token은 verifier와 operator semantic intake 전에 consumable reviewer verdict가 아니다.
...
| `scripts/review-run.ps1` | reviewer CLI adapter-local launcher 해소 + strict UTF-8 raw-byte stdin 1회 실행 · reviewer-safe posture 강제 · invocation/result-unavailable 실패 분리 · model/effort/category 해소(분기의 구체 값 포함) · canonical candidate shape 선검증 · H1 candidate token/run-fact emit · provenance append |
...
- lifecycle 문서: 없음 — temporary Design / Plan은 각 batch closeout에서 retire(삭제)되고 planning anchor commit이 기록을 git history에 보존한다; 이 lifecycle에서는 Work Packet을 만들지 않았다.
- spec ↔ implementation: **live** — strict UTF-8 raw-byte stdin과 usable-judgment-only verdict 계약이 active surface·tests와 동기화되어 있다.

# review Backlog

next ID: RV-B-22

| ID | item | reopen / start condition |
|---|---|---|
| RV-B-01 | Review 2-pass / profile — user-facing 텍스트용 2차 profiling pass 의 가치 판정 | 별도 scoped goal + profiling 가치 입증 |
| RV-B-03 | result.md wrapper/fence 렌더링 정규화 여부(on-disk artifact hygiene; verdict shape 변경 아님) | fence 로 인한 실독 사례 누적 시 별도 scoped goal |
| RV-B-04 | no-exec/no-write reviewer 의 잔여 role-boundary framing(R1 evidence convention 은 closed) | 별도 scoped goal |
| RV-B-09 | (수용된 잔여 위험) operator narrow-scope over-confidence — 좁힌 input 작성 경향; mitigation = 독립 Codex review + skill 의 target-accuracy 규율 + retraction 규율 | 현 패턴을 넘는 구체 신규 증거 |
| RV-B-10 | (수용된 잔여 위험) `;`-chain 마지막 비-0 종료가 review-verify 성공을 가리는 보고 관례 위험 | 현 패턴을 넘는 구체 신규 증거 |
| RV-B-11 | (idea-only — implementation backlog 아님) devil's-advocate pre-pass orchestration | 반복 false-negative 또는 model-bias 의 구체 증거 + 사용자 별도 결정 + 별도 scoped goal |
| RV-B-12 | (idea-only — implementation backlog 아님) multi-reviewer consensus / calibration loop | RV-B-11 과 동일 증거 + maintenance-mode exit 의 별도 승인(두 조건 동시) + 별도 scoped goal |
| RV-B-13 | (deferred 결정) review-polishing selective-capture vehicle 결정 — instruction vs skill (non-hook); "leave as caller-judgment instruction" 결론 가능. retired skills 서브시스템 plan(Batch 4; git history)에서 이관 | review-polishing 개선신호의 selective capture 필요가 구체화 + 사용자 결정 + 별도 scoped goal |
| RV-B-14 | (deferred 결정) reviewer-execution-boundary / validation-oracle 개선 — reviewer(Codex)가 review 중 `Invoke-Pester` / `verify-ps1.ps1` 를 직접 실행해 validation 을 재현(현행 verdict 는 operator 가 제공한 validation 텍스트에 의존). **reconcile**: 이 row 는 `review_spec.md` Durable boundary 의 "deterministic validation runner 미도입" 경계를 뒤집지 않는다 — 도입 결정이 아니라 open candidate 일 뿐이다 | reviewer-exec 경계 재검토를 정당화하는 구체 증거(operator-validation 의존이 unsound verdict 를 낳은 사례) + 사용자 별도 결정 + 별도 scoped goal |
| RV-B-15 | (milestone-tracking) adoption smoke test · actual reviewer workflow test · actual development workflow usage test 는 서로 분리된 milestone 이다(한 번에 닫히지 않는 별개 검증 단계) | 해당 milestone 중 하나를 실제 착수할 때 + 별도 scoped goal |
| RV-B-16 | consultation advisory material의 canonical review input/reporting 수용 interface 여부(review owner 판단; consultation의 output·artifact·status·synthesis 의미는 소유하지 않음) | canonical-linked consultation 사례 누적 + 사용자 결정 + 별도 scoped goal |
| RV-B-20 | (open observation) pre-change review engine 과 current workflow contract 의 version skew — 독립성(검토 대상 변경을 포함하지 않는 engine)과 protocol 호환성(prepare/input/result/verify 계약·runner argument)은 별개이며, 오래된 engine 이 자기 계약 아래 정상 result 를 내면 current protocol 과 다른 review 가 canonical 로 소비될 수 있다. 미결정 = compatibility 기준·owner(operator prose / runner capability / handshake)·packet 변환 허용 여부·불호환 시 stop 형태. 비후보 = 새 parser·compatibility registry·자동 checkout·translation layer. 기존 경계(in-development engine fallback 금지·pre-change checkout 자동 생성 금지·incompatibility 를 verdict 로 바꾸지 않음) 불변 | 독립성은 충족하나 engine contract 차이로 prepare/run/verify 가 실패하거나 의미가 달라진 실제 표본 발생 시, 또는 review-machinery 변경을 pre-change engine 으로 self-review 해야 할 때 + 사용자 별도 결정 + 별도 scoped goal |
| RV-B-21 | (deferred 결정) Pester 운용 순서 — affected-first → candidate 안정화 → final full 1회(동일 안정 후보의 정상 경로이지 task 전체 retry quota 아님)를 change-class validation guidance 에 compact 하게 둘지 결정. 현행 표면은 affected+full 요구만 있고 순서·안정화 판단·intermediate full 억제·post-full 변경 처리는 없다. owner 후보 = review skill validation 문단(+ input template evidence 위치·tests/README 용어). 비후보 = deterministic change→test mapper·scheduler·CI gate·final full 제거·full 미실행의 자동 완화 | 운용 실측 표본(affected/full 시간·final 소비 full 수·stale intermediate full 수·full-only finding) 추가 후 사용자 결정 + 별도 scoped goal |

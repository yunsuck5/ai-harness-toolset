...
> 는 ai-harness 용도로 금지된다. canonical source-of-truth 는 `docs/contracts/brief/BRIEF_CONTRACT.md` 다. 본문은 결정 기록으로 보존한다.
...
- review / evidence / chatlog / brief runtime artifact 는 모두 target-local `<ProjectRoot>/log/` 아래로 들어간다 (3차 reconciliation 기준; canonical Brief 는 `<ProjectRoot>/log/brief/BRIEF.md`, root `<ProjectRoot>/brief/` 는 rejected — `docs/contracts/brief/BRIEF_CONTRACT.md`). 이전 라운드의 "brief artifact 는 target-local `brief/` 아래" wording 은 historical/superseded.

### Explicit trigger / approval required

다음 행위는 default 가 아니라, 사용자 명시 trigger 와 scoped 승인을 거친 뒤에만 실행된다.

- review execution (`scripts/review-prepare.ps1` → `scripts/review-run.ps1` canonical two-step entry; per `docs/contracts/review/REVIEW_RESULT_CONTRACT.md`)
...
- `BriefRoot` — `<ProjectRoot>/log/brief` (3차 reconciliation 기준; root `<ProjectRoot>/brief` 는 rejected — `docs/contracts/brief/BRIEF_CONTRACT.md`. 이전 라운드의 `<ProjectRoot>/brief` 표기는 historical/superseded.)
...
본 형식은 contract 가 아니라 audit 단계의 검토 대상 예시였다. 현행 normal operator path 는 canonical task/pass topology 채택 (POST_MVP_PLAN §10 Completed `c81fe45`) 이후 두 단계 entry 다 — 예시 형식만 갱신하면 다음과 같다.

```
powershell -NoProfile -ExecutionPolicy Bypass `
  -File H:/Work/ai-harness-toolset/ai-harness-toolset/scripts/review-prepare.ps1 `
  -ProjectRoot . `
  -ReviewTaskId <id> `
...
powershell -NoProfile -ExecutionPolicy Bypass `
  -File H:/Work/ai-harness-toolset/ai-harness-toolset/scripts/review-run.ps1 `
  -ProjectRoot . `
  -ReviewTaskId <id> `
  -Pass <pass-NN>
```

...
- BF Level 1/2 / Level 3 의 운영 규칙. `docs/brief/brief_spec.md` 가 source-of-truth 다.
...
- `<ProjectRoot>/log/brief/BRIEF.md` (canonical Brief) 의 artifact 는 reviewer verdict 가 아니다. `brief-check.ps1` 의 PASS / FAIL 도 reviewer verdict 가 아니다 (`docs/brief/brief_spec.md`).
...
- cross-subsystem boundary 변경 (review ↔ evidence ↔ brief 사이의 책임 경계).
...
- 동작이 contract 문서에 의존한다면, 그 direct contract docs (예: `docs/contracts/review/REVIEW_RESULT_CONTRACT.md`, `docs/brief/brief_spec.md`) 를 함께 포함한다.
...
- `log/` 아래 runtime artifact (`log/review/<review-task-id>/<perspective>/pass-NN/...`, `log/evidence/...`, `log/brief/...`) 는 target 으로 지정하지 않는다. 이는 generated read-only record 이며 source-of-truth 가 아니다.
...
- `<ProjectRoot>/log/brief/BRIEF.md` artifact 와 `brief-check.ps1` PASS / FAIL 은 reviewer verdict 가 아니다. verdict 의 source-of-truth 는 같은 review task 의 final pass 의 `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/result.md` 의 `## Verdict` 다.
...
- 비고: 같은 batch 에 BRIEF / packaging 변경을 함께 포함하지 않는다.
...
상황: `docs/brief/brief_spec.md` 또는 `templates/brief/BRIEF.md` 에 영향을 주는 변경.

- 권장 effort: safe-default(xhigh); 변경이 명확히 단순한 local correctness 면 `-Effort` 로 downgrade 가능. cross-subsystem 영향 (예: brief ↔ review 사이의 책임 경계 재정의) 이 있으면 downgrade 하지 않는다.
...
- 권장 effort: safe-default(xhigh). design 단계의 명확히 단순한 packet 은 `-Effort` 로 downgrade 가능하나, `docs/project/AI_HARNESS_TOOLSET_SCOPE.md` / `docs/contracts/review/REVIEW_RESULT_CONTRACT.md` / `docs/brief/brief_spec.md` 의 source-vs-target 경계와 충돌 risk 가 있으면 downgrade 하지 않는다.
...
- `log/brief/BRIEF.md` 의 artifact 는 reviewer verdict 가 아니다.

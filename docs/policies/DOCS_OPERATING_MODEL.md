...
→ Operative procedure and the single-home rule: `rules/docs-working-model/docs-working-model.md` (*Top-down operating model*, *Doc-vs-doc precedence*).
...
→ The shape that new/revised STATUS content must conform to: `rules/docs-working-model/docs-working-model.md` (*Per-system STATUS.md shape / altitude*).

## 3. Role of each layer (orientation map)

Placement scope for each folder is defined in `docs/README.md` §5; the map below is a reading aid for what each docs layer is *for* in the change/closeout flow. The binding "must-not-absorb" constraints are the operative rule (→ `rules/docs-working-model/docs-working-model.md` *Layer shape constraints*).
...
→ Operative STATUS belongs/does-not-belong, the altitude ceiling, the durable-pointer prohibition, and the BACKLOG tombstone form: `rules/docs-working-model/docs-working-model.md` (*Per-system STATUS.md shape / altitude*, *BACKLOG.md closed-row tombstone*).
...
→ Operative tombstone form: `rules/docs-working-model/docs-working-model.md` (*BACKLOG.md closed-row tombstone*).
...
→ Operative briefing procedure (which surfaces to read, how to synthesize): `rules/docs-working-model/docs-working-model.md` (*On-demand status-briefing model*).
...
→ Operative gate (Level 1 / Level 2 checklists + the inspect-all / report-each rule): `rules/docs-working-model/docs-working-model.md` (*Closeout reconciliation — two-level gate*).
...
→ Operative scope + Codex-review-gate rule: `rules/docs-working-model/docs-working-model.md` (*Scope of an application + review gate*).

## 9. The complementary docs surfaces (orientation)

- Placement — orientation map `docs/README.md`; binding rule `rules/docs-working-model/docs-working-model.md` (*Docs placement*).
- Per-question read-first routing → `docs/current/REPO_READING_GUIDE.md`.
- Docs change/closeout process (operative rule) → `rules/docs-working-model/docs-working-model.md`; this file holds its rationale and record.

On overlap, the placement orientation map is `docs/README.md` (its binding rule and the change/closeout process live in `rules/docs-working-model/docs-working-model.md`), and read-first routing is `REPO_READING_GUIDE.md`.

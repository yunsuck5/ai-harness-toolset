...
`... 2>&1 | Out-String`, `... 2>&1 | Out-Null`, and downstream string joins such as `($combined | ForEach-Object { [string]$_ }) -join "`n"` or `((& <exe> 2>&1) -join "`n").Trim()` are **not** mitigations for the abort. They run downstream of the `& ... 2>&1` evaluation; under PS 5.1 + EAP=Stop the terminating error fires before any downstream pipeline element ever receives input. Treat them as result-normalization patterns (display, success-path stringification) rather than as error-handling patterns.

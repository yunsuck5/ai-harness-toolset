...
- **model / model-source** — runner 가 model 을 resolve 해 reviewer CLI 에 전달하고, **Batch D2 이후 `model:` + `model-source:` 를 run-fact 로 emit 한다** (model-source 는 추론이 아니라 실제 resolver branch — `explicit` / `config`; missing/empty model 은 fail-fast). report from run-fact when present. **concrete model version 을 본 doc / report 에 literal 로 박지 않는다** — `config/reviewer.json` 가 source-of-truth 이며 report 는 runtime resolve 값(runner stdout 의 `model:` 라인)을 인용한다.

...
layout 은 optional 한 perspective segment 를 갖는다. perspective (review viewpoint — 예: `local-correctness` / `system-coherence`) 를 명시하지 않으면 위의 two-level 이 default 이고, 명시하면 `<review-task-id>` 와 `pass-NN` 사이에 별도의 `<perspective>` segment 가 들어가는 three-level 이 된다:
...
두 layout 모두에서 pass 당 canonical artifact 는 같은 `pass-NN/` 안의 `input.md` + `result.md` 두 파일로 동일하다 — perspective 는 path segment 하나를 더할 뿐 pass directory 내부 구조와 2-file 규약 (§10 non-goal) 을 바꾸지 않는다. 두 layout 의 의미와 backward compatibility 는 §1 이 정의한다.
...
- `<perspective>` (optional) — review viewpoint (예: `local-correctness` / `system-coherence`). 명시되면 `<review-task-id>` 와 `pass-NN` 사이의 **별도 path segment** 로 들어가 review viewpoint 를 작업 식별자 및 corrective attempt 와 한 폴더 이름에 섞지 않고 분리한다. operator 가 명시하며 자동 추론하지 않는다. **미지정 또는 빈 문자열은 "perspective 없음" (= two-level default) 으로 취급된다** — 검증은 *명시된 non-empty* perspective 값에만 적용되고, 빈 값은 invalid 가 아니라 omitted 다. 명시된 non-empty perspective 는 **single path segment** 여야 하고 `..` / path separator (`/`, `\`) / `pass-NN` 형태를 담지 않으며 safe charset/length 를 따른다 (`Test-ValidReviewTaskId` 동형, `scripts/lib/path.ps1` `Test-ValidPerspective`). 자리: `<ProjectRoot>/log/review/<review-task-id>/<perspective>/`.
- `pass-NN` — corrective loop 의 각 Codex review attempt. 첫 review attempt 는 `pass-01`, 두 번째 attempt 는 `pass-02`, 이렇게 zero-padded 두 자리로 증가한다. perspective 가 없으면 같은 `<review-task-id>` 안에서, perspective 가 있으면 같은 `<perspective>` 안에서 매겨진다 (per-perspective corrective attempt — perspective 마다 자기 `pass-NN` 시퀀스를 갖는다). 자리: two-level `<ProjectRoot>/log/review/<review-task-id>/pass-NN/` 또는 three-level `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/`.
- **backward compatibility / migration 없음**: 기존 two-level artifact 와 호출은 그대로 동작하며, three-level 은 perspective 가 명시될 때 추가로 발급/검증된다. 과거 artifact 의 migration / rewrite 는 하지 않는다 (§9). perspective 미지정이 default (= 오늘과 동일한 two-level) 다.
- canonical artifact per pass: 같은 `pass-NN/` 디렉터리 안의 `input.md` + `result.md` 한 쌍 (two-level / three-level 동일). 한 pass 의 record 는 외부 staging folder, sidecar JSON, hash sidecar 어디로도 분산되지 않는다.
...
`result.md` 의 verdict/disclosure **body** 는 active reviewer adapter (현재 MVP adapter = codex) 가 `--output-last-message log/review/<review-task-id>[/<perspective>]/pass-NN/result.md` (perspective optional, §1) 로 작성한다. operator-role AI 또는 운영자가 손으로 작성하지 않는다. runner 는 그 body 를 보존한 채 끝에 provenance 블록을 append 한다 (아래 dual-authorship).
...
1. pass directory (`<ProjectRoot>/log/review/<review-task-id>/pass-NN/` 또는 perspective 지정 시 `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/`) 가 `<ProjectRoot>/log/review/` 내부인지 containment 검증. perspective 가 operator-supplied path segment 이므로, review-root containment 만으로는 같은 review-root 안 다른 task 로의 traversal 을 막지 못한다 — 추가로 생성/해석되는 pass dir 이 의도한 `<review-task-id>/` 하위인지 task-root containment 로 검증한다 (`scripts/lib/path.ps1` `Assert-InTaskRoot`).
...
- `scripts/review-prepare.ps1 -ReviewTaskId <id> [-Perspective <viewpoint>] [-Pass <pass-NN>] -Stage <stage> -Purpose <line>`
  - canonical pass directory 를 발급한다 — `-Perspective` 미지정 시 two-level `<ProjectRoot>/log/review/<review-task-id>/pass-NN/`, 지정 시 three-level `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/`.
  - 그 안에 `templates/review-input.md` 본문을 그대로 옮긴 `input.md` 를 seed 한다.
  - `-Pass` 가 생략되면 같은 pass-parent directory (perspective 미지정 시 task directory, 지정 시 `<task>/<perspective>/`) 안의 기존 `pass-NN` 을 스캔해 다음 번호를 할당한다 (`pass-01`, `pass-02`, ... — per-perspective).
  - pass directory 가 이미 존재하면 write-once 위반으로 거부한다.
- `scripts/review-run.ps1 -ReviewTaskId <id> [-Perspective <viewpoint>] -Pass <pass-NN>`
  - 해당 pass directory (perspective 지정 시 three-level) 의 `input.md` 를 `scripts/review-input-verify.ps1` 로 검증한 뒤 Codex CLI 를 정확히 1 회 실행해 같은 pass directory 의 `result.md` 를 작성한다.
...
- `scripts/review-verify.ps1 -ReviewTaskId <id> [-Perspective <viewpoint>] -Pass <pass-NN> [-RequireResult]`
  - `-Perspective` 미지정 → old two-level pass dir 을, 지정 → new three-level pass dir 을 해석한다 (operator 가 layout 을 명시; 자동 추론 없음). result shape 검증 semantics 는 두 layout 에서 동일하다.
...
운영자 / AI 는 `<review-task-id>` 를 사용자의 `/goal` 작업 또는 review gate 단위로 직접 결정하고, 필요하면 `<perspective>` 를 명시한다. Claude Code chat / session id 를 자동으로 받아 쓰지 않으며 perspective 도 자동 추론하지 않는다.
...
| 4 | artifact pass count | `artifact pass-NN` attempt 수. **perspective (three-level) layout 에서는 per-perspective 로 보고한다** — 각 perspective 가 자기 `pass-NN` 시퀀스를 가지므로 perspective 별 pass count + 각 perspective 의 final-pass path 를 분리해 적고 (또는 명시적 aggregate 와 함께), 서로 다른 perspective 의 pass 를 한 숫자로 conflate 하지 않는다. two-level layout (perspective 미지정) 에서는 task 단위 단일 시퀀스다. 해당 workflow 가 `artifact pass-NN` 을 만들지 않으면 `N/A — no artifact pass-NN` (발명 금지) |
...
validation scope 용어의 정의 home 은 testing convention home 인 `tests/README.md` 다 (suite topology terms). 본 §6c 는 그 정의를 인용하며 재정의하지 않는다 (single-home; `docs/policies/DOCS_OPERATING_MODEL.md` §1):
...
|---|---|---|---|
| planning/design doc only | `docs/systems/**` plan/spec | `git diff --check` (신규 파일은 §6c.4); 신규 `.md` LF/no-BOM 확인 | 아니오 |
...
mirror surface 의 wording 이 본 §6c 와 충돌하면 본 §6c 가 우선한다. 본 §6c 는 §3a / §3b / §3d / §6b 의 기존 wording 을 변경하지 않고 그 위에 operator-side closeout validation scope 를 더한다.
...
수정 후 동일 review task 안에서 동일 의미의 review 가 필요하면 같은 `<review-task-id>/` 아래에 (perspective layout 을 쓰는 경우 같은 `<perspective>/` 아래에) 새 `pass-NN/` 를 만든다 — 같은 task (및 같은 perspective) 의 corrective loop 의 다음 attempt 다. perspective 별로 `pass-NN` 시퀀스가 독립이므로, 한 perspective 의 corrective attempt 는 그 perspective 안에서 증가한다. 이전 pass directory 안의 파일을 손으로 보정하지 않는다.
...
- review record 는 항상 `<ProjectRoot>/log/review/<review-task-id>/pass-NN/` (또는 perspective 지정 시 `<ProjectRoot>/log/review/<review-task-id>/<perspective>/pass-NN/`) 아래에만 만든다. `<ProjectRoot>/log/` 는 gitignored runtime tree 다.

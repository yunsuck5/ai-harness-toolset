...
- `INSTALL.md` §10 의 Claude skill global / update / removal 절차 (apply: `scripts/activate-global.ps1`; removal: `scripts/uninstall-global.ps1`).
...
- 사용자가 자기 환경에서 invocation channel 을 어떻게 결정하는지는 본 design 문서 (`docs/contracts/global-invocation/SHARED_GLOBAL_INVOCATION_CONTRACT.md`) 와 `INSTALL.md` 를 참조하도록 안내.
...
- 두 곳 모두 본 design 문서 (`docs/contracts/global-invocation/SHARED_GLOBAL_INVOCATION_CONTRACT.md`) 와 `INSTALL.md` 를 참조하도록 명시한다.

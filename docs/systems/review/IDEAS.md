...
>
> 본 문서의 목적은 review subsystem 의 heavier-mechanism candidate 중 **deferred backlog 도 아니고 implementation 대상도 아닌** idea 들을 durable source-managed 문서에 기록하여 future operator turnover / long-term governance review 시점에서 누락되지 않도록 하는 것이다 (idea-only durability mitigation — backlog/implementation 으로 승격되지 않은 idea 가 사라지지 않게 하는 committed 기록).
...
- **Counter-argument H2 Option A (parser-required form) 는 본 IDEAS.md 의 항목이 아니다.** parser-required `## Counter-argument` 는 **도입되지 않았다** — `scripts/review-verify.ps1 -RequireResult` 는 4 disclosure H2 만 강제하고, `docs/contracts/review/REVIEW_RESULT_CONTRACT.md` §3c.5 가 Option A 를 §3c non-goal 로 기록한다. **committed source-managed escalation threshold 은 존재하지 않는다** — 향후 escalation 은 별도 user 승인 + repo docs/contracts 로의 commit 을 거쳐야 한다 ("deferred-until-evidence" class; 본 IDEAS.md 의 idea-only class 와 분리).
- **Vertical-vs-horizontal review mode 는 본 IDEAS.md 의 항목이 아니다.** **later candidate** class — 별도 future batch 의 implementation candidate. idea-only 와도 deferred-until-evidence backlog 와도 다른 class. 본 IDEAS.md 의 의의는 본 candidate 의 idea-only 강등이 아니라 본 candidate 와의 명시적 분리. 본 문서에서 설계 / 구현 / 계획 없음.
...
- Phase 1 historical context — 본 IDEAS.md 의 idea-only item 들은 Phase 1 Batch II close 시점에 "Phase 2 candidates remain deferred" 로 framed 되었다 (Phase 1 Batch II commit `e6027ca` 의 historical snapshot). current-state framing 의 source-of-truth 는 `docs/systems/review/STATUS.md` ledger (RV-05/06/07) + 본 IDEAS.md + commit history (`76033f4` Counter-argument Option B codification) 다.
...
**Reopen criteria** (evidence threshold — 다음 중 어느 하나 이상 충족 시):
...
**Reopen criteria.** 아래 evidence threshold + STATUS.md maintenance-mode exit decision 의 **두 조건 동시 충족** 시:

- **evidence threshold 중 어느 하나** — repeated false negatives 또는 model-bias evidence (Devil's advocate idea-only item 의 reopen criteria 와 동일 evidence path; 본 evidence 의 누적이 두 idea 의 어느 쪽으로 escalate 되는지의 user 결정은 evidence 누적 후의 별도 결정).

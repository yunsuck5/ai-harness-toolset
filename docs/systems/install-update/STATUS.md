...
- **D8 external test signal** — the `verify-ps1.Tests.ps1` AC-VPS1-D8-* fixture fragility (system git `core.autocrlf` + native-exe stderr promotion via `& git … 2>&1` in a setup helper, which under `$ErrorActionPreference = 'Stop'` can abort before the exit code is read) is **resolved**: the git fixtures were migrated to the `Invoke-NativeProcess` containment shim (`scripts/lib/native-process.ps1` — EAP=Continue + separate `1>`/`2>` capture, exit-code-driven), the mitigation endorsed by `docs/policies/POWERSHELL_POLICY.md` ("Native command invocation under `$ErrorActionPreference = 'Stop'`"); the full Pester suite is green on a `core.autocrlf=true` machine. **Narrowed residual (non-blocking):** the verify-ps1 Step F lint still *allows* the merged-stderr discard forms (`$null = & … 2>&1` and `& … 2>&1 | Out-Null`) repo-wide, which that policy's reasoning indicates are not themselves true mitigations under EAP=Stop; other fixtures (`install-pipeline.Tests.ps1`, `install-update.Tests.ps1`, `support/lifecycle-fixture.ps1`) still use `& … 2>&1 | Out-Null` for git setup. Several of those builders set `core.autocrlf false` in the fixture repo (reducing the autocrlf-warning exposure), but not all do — e.g. `install-pipeline.Tests.ps1` `AC-IP-DOG-ENTRY-1` runs `git add` without that guard — so the same fragile class persists repo-wide and was deliberately left outside this S1-scoped fix. Reopen only by a separate decision to tighten the lint / policy or migrate the remaining fixtures. (detail in git history)

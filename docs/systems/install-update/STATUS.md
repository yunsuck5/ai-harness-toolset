...
- **D8 external test signal** — `verify-ps1.Tests.ps1` AC-VPS1-D8-* is an environment-dependent issue (system git `core.autocrlf` + native-exe stderr promotion in a helper), not an install/update blocker and not masked; it does not always reproduce. (detail in git history)

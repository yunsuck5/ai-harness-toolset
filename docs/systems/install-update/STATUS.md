...
- Decision-rationale record: `docs/decisions/POST_MVP_PLAN.md` (§1–§9). **Detailed phase / dogfood / incident / closeout narrative** (provenance): `docs/archive/old-roadmaps/INSTALL_UPDATE_LIFECYCLE_NARRATIVE.md`; full per-commit completed text: `docs/archive/old-roadmaps/POST_MVP_COMPLETED_NARRATIVE.md`.
...
Compact ledger. "Detail" points to the authoritative source; full narrative is in `docs/archive/old-roadmaps/INSTALL_UPDATE_LIFECYCLE_NARRATIVE.md` (lifecycle phases / dogfood / incident / closeout) and `docs/archive/old-roadmaps/POST_MVP_COMPLETED_NARRATIVE.md` (per-commit completed text).
...
- **D8 external test signal** — `verify-ps1.Tests.ps1` AC-VPS1-D8-* is an environment-dependent issue (system git `core.autocrlf` + native-exe stderr promotion in a helper), not an install/update blocker and not masked; it does not always reproduce. Narrative §10.

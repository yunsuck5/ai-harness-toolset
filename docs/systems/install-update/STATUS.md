...
- A **read-only install/update operator entrypoint** `scripts/install-update.ps1` exists (post-MVP hardening Batch 1, committed `8ddb04c`). It provides `inspect` and `verify` modes that emit a fixed-vocabulary final status plus a stdout JSON report (a strict field-name subset of the `run.json` shape); it has no mutation flag and writes no file (stdout/stderr only). Activation drift is detected by byte-identity (Claude/Codex managed blocks via marker-bounded content equality, skill mirror via SHA-256), not mere path existence; the Codex managed-block surface honors `AGENTS.override.md` precedence. The final-status vocabulary and the `run.json` evidence contract are codified in `INSTALL.md` §13, and the deterministic-narrow-entrypoint allowance (vs the still-out-of-scope productization/framework class) is codified in `INSTALL.md` §11 (b). The same batch hardened the canonical verifier `Invoke-InstallPipelineVerify` to enforce the complete `INSTALL.md` §4/§5 mode-conditional metadata schema for both modes (git-url: `repoUrl` non-empty + `sourcePath`/`toolRoot` empty; local-clone: `sourcePath`/`toolRoot` non-empty + `repoUrl` empty). Actual `run.json` write to the global install log path, activation apply orchestration, and snippet routing to this entrypoint remain future scoped work (this batch is read-only + contract-only).

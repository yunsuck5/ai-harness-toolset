...
- BF Level 3 (deterministic Brief maintenance / validation / stale warning / session-start guidance) 의 Step 3 안 implementation. (무요청 session-start restore-offer automation 은 폐기되었다.)
...
- Claude skill SKILL.md install / update / removal 은 `INSTALL.md` §10 (install / update: `scripts/activate-global.ps1`; removal: `scripts/uninstall-global.ps1` + `scripts/lib/uninstall-target.ps1`) 이 source-of-truth 인 별도 절차다. install / update automation 본체가 자동으로 skill 을 refresh 하지 않는다.
...
- 자동 skill refresh trigger field — `INSTALL.md` §10 (`scripts/activate-global.ps1`) 의 별도 절차와 충돌.
...
- **3-6 managed block / skill replace boundary** (§6) — global instruction file managed-block apply, Claude skill SKILL.md install / update / removal. install / update automation core 본체 밖이며 `GLOBAL_ADOPTION_DECISION.md` §6 / `INSTALL.md` §10 의 explicit user-approved scope (§10.6 / §11.7 의 forbidden enumeration 과 정합).
...
- ~~**post-MVP closeout 결정**~~ — **closed.** post-MVP numbered roadmap (§11) 의 whole-project closeout 결정이 기록되었다; closeout routing 은 `docs/decisions/DECISIONS.md` "Post-MVP closeout" block + `POST_MVP_PLAN.md` §11 step 6 다. 본 항목은 §13.2 목록 안에 closeout 표기로 보존된다. closeout 은 어떤 global / user mutation 도 자동 승인하지 않는다.
...
install / update / restore automation core (§12 runtime pipeline)
  ≠
managed-block / skill replace apply (GLOBAL_ADOPTION_DECISION.md §6 / INSTALL.md §10)
```
...
- **boundary 의 governing source-of-truth 표기.** managed-block 정책의 source-of-truth 는 `GLOBAL_ADOPTION_DECISION.md` §6 (marker / detection algorithm / destination path table / update policy), Claude skill 절차의 source-of-truth 는 `INSTALL.md` §10 (skill 의 install / update: `scripts/activate-global.ps1`; removal: `scripts/uninstall-global.ps1` + `scripts/lib/uninstall-target.ps1`).
...
- **Claude skill asset 의 source / destination shape boundary 보존.** source 는 `<ToolRoot>/snippets/claude-skills/<skill-name>/SKILL.md`, destination 은 `%USERPROFILE%\.claude\skills\<skill-name>\SKILL.md` (`INSTALL.md` §10). skill SKILL.md 는 `current/` runtime payload 안에 두지 않는다 (§10.2 와 정합).
...
- **actual Claude skill install / update / removal** 의 실행. `%USERPROFILE%\.claude\skills\<skill-name>\` 의 어떤 생성 / 덮어쓰기 / 삭제도 본 §14 의 doc 변경으로 자동 승인되지 않는다. `INSTALL.md` §10 의 절차 (install / update: `scripts/activate-global.ps1`; removal: `scripts/uninstall-global.ps1` + `scripts/lib/uninstall-target.ps1`) 가 governing.
...
- **부모 root-level docs** (`GLOBAL_ADOPTION_DECISION.md`, `GLOBAL_INSTALL_UPDATE_MODEL.md`) 의 본문 mutation. 본 anchor 는 STEP3 guide subordinate scope 안에서 닫는다.
...
- Claude skill install / update / removal 의 actual writer script / surface (예: `scripts/skill-apply.ps1`) 의 도입. `INSTALL.md` §10 의 절차는 §19.4 의 AI-guided global apply 절차가 따르며, 그 외 별도 writer 의 도입은 작업 후보 아님.
...
- `INSTALL.md` §10 의 install / update / removal 절차의 재정의 또는 복제.
- §6 canonical decomposition 의 ordering / numbering 변경.
...
- 부모 root-level docs (`GLOBAL_INSTALL_UPDATE_MODEL.md`, `GLOBAL_ADOPTION_DECISION.md`) 본문 mutation. 본 anchor 는 STEP3 guide subordinate scope 안에서 닫는다.
...
- 부모 root-level docs (`GLOBAL_INSTALL_UPDATE_MODEL.md`, `GLOBAL_ADOPTION_DECISION.md`) 본문 mutation.
...
- Step 4 validation 시작.
- snapshot / payload manifest / payload marker 생성 (본 anchor 는 docs-only).
- 부모 root-level docs (`GLOBAL_INSTALL_UPDATE_MODEL.md`, `GLOBAL_ADOPTION_DECISION.md`, `SHARED_GLOBAL_INVOCATION_CONTRACT.md`) 본문 mutation.
- operations backlog 항목 (현행 `docs/systems/install-update/BACKLOG.md`) 의 기존 status 변경.
- §6 canonical decomposition (9 단계 ordering / numbering) 변경.
- commit / push / publish / merge / release / adoption.
...
글로벌 install / update / refresh / managed-block apply / Claude skill install / update / removal / global payload refresh 는 사용자 명시 결정 위에서 **AI operator** 가 다음 절차로 수행한다. 단계별로 사용하는 수단이 다르다 — `current/` payload refresh 는 install-pipeline (§12 의 4 action), managed-block apply 는 §19.3 의 A-2 managed-block tooling (`apply-managed-block.ps1` / `activate-global.ps1`), Claude skill 은 §10 / `INSTALL.md` §10 의 source 기준 whole-file byte overwrite + post-write SHA-256 verify (read-only dry-run preview 는 있으나 별도 backup / rollback / sidecar 없음; Phase 4a `activate-global.ps1` canonical-overwrite path) 이다.
...
1. **destination 보고** — AI operator 는 적용 destination path 를 사용자에게 명시 보고한다.
   ...
   - Claude skill destination — `%USERPROFILE%\.claude\skills\<skill-name>\SKILL.md` (`INSTALL.md` §10 와 정합).
...
3. **overwrite materialization** — AI operator 가 source 의 본문을 class 별 수단으로 deterministic 하게 target 에 반영한다.
   ...
   - Claude skill — §10 / `INSTALL.md` §10 의 source 기준 whole-file byte overwrite + post-write SHA-256 verify (사용자 수정 overwrite 사전 고지). skill 에는 read-only dry-run preview 는 있으나 (Phase 4a `activate-global.ps1` canonical-overwrite path) 별도 pre-write backup / rollback / sidecar 는 없다.
...
- 부모 root-level docs (`GLOBAL_INSTALL_UPDATE_MODEL.md`, `GLOBAL_ADOPTION_DECISION.md`) 본문 mutation. 본 절은 STEP3 guide subordinate scope 안에서 닫는다.
- §6 canonical decomposition 9 단계의 ordering / numbering 변경.

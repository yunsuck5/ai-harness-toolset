...
- target project 에는 `ai-harness-toolset` payload 를 설치하지 않는다. target persistent footprint 는 **`<ProjectRoot>/log/` only** 다 (§8 — 3차 reconciliation 기준). BRIEF / Evidence / Review 세 runtime artifact 트리는 모두 `log/` 아래 들어간다.
- `ai-harness-toolset` repo 는 canonical ToolRoot 이면서 self-dogfooding ProjectRoot 인 special case 다. ProjectRoot 로 동작할 때 `<ProjectRoot>/log/` 를 가질 수 있고, 그 안의 runtime artifact (BRIEF / Evidence / Review) 는 source payload 도 install payload 도 아니다 (§9). root `<ProjectRoot>/brief/` 는 self-dogfooding 에서도 만들지 않는다.
...
| `targetFootprintPolicy` | target footprint 정책. 현행 (3차 reconciliation) 값: `log-only` — BRIEF / Evidence / Review 모두 `<ProjectRoot>/log/` 아래에 들어간다. 이전 (2차) 값 `log-brief-only` 는 historical 로 superseded. |
...
- `<ProjectRoot>\log\`
- `<ProjectRoot>\brief\`
- review / evidence / brief 같은 project-local 산출물이 생성되는 위치다.
- `GLOBAL_ADOPTION_DECISION.md` §4 의 target-local state / result boundary (`log/brief/`, `log/evidence/`, `log/review/`) 와 정합한다. `log/` 는 그 하위 `brief/` `evidence/` `review/` 를 포함한다 (3차 reconciliation 기준).
...
- `log/brief/` 디렉터리는 어느 경우에도 만들지 않는다. `log/` 는 runtime artifact 트리이고 brief 의 자리가 아니다 (`POST_MVP_PLAN.md` §3 — 당시 인용; 본 줄 전체가 아래 superseded note 의 1차/2차 framing 기록이다).

> **Superseded (historical 1차/2차 framing) — full lineage single-home: §1 3rd-reconciliation note + §9.3 reconciliation list.** 위 §6 Layer 4 의 BRIEF / footprint wording (`<ProjectRoot>\brief\` 를 별도 footprint 항목으로 둔 것, "self-dogfooding ProjectRoot 의 `brief/`", "`log/brief/` 디렉터리는 어느 경우에도 만들지 않는다" 등) 은 1차 / 2차 framing 의 historical model 기록으로 보존된다. **현행 (3차 기준): canonical Brief = `<ProjectRoot>/log/brief/BRIEF.md`, root `<ProjectRoot>/brief/` 는 rejected, project-local footprint = `<ProjectRoot>/log/` only** (BRIEF / Evidence / Review 모두 `log/` 아래; canonical source-of-truth `docs/brief/brief_spec.md`). 본 3차 framing 이 위 §6 Layer 4 잔재 wording 보다 우선한다.
...
- `<ProjectRoot>/log/` — project-local, operator-local, source-control-excluded runtime root. BRIEF / Evidence / Review 의 세 runtime artifact 트리가 모두 이 한 자리 아래로 들어간다 (`<ProjectRoot>/log/brief/BRIEF.md`, `<ProjectRoot>/log/evidence/`, `<ProjectRoot>/log/review/`).
...
> **BRIEF wording note — §9 전체에 적용.** §9.1–§9.5 본문의 `brief/` (= root `<ProjectRoot>/brief/`) 와 `log/brief/BRIEF.md` 표현은 1차 / 2차 framing 의 **historical / intermediate model 기록**이다 (full 3-step lineage 의 single-home: §1 3rd-reconciliation note + §9.3 reconciliation list). **현행 (3차 — 문서 전체 적용): canonical Brief 는 `<ProjectRoot>/log/brief/BRIEF.md`** — project-local, operator-local, source-control-excluded runtime artifact under `<ProjectRoot>/log/` (gitignored); **root `<ProjectRoot>/brief/` 는 rejected**, user-home operator-local runtime root (예: `%USERPROFILE%\.ai-harness\projects\<project-key>\...`) 도 rejected, **target persistent footprint = `<ProjectRoot>/log/` only** (BRIEF / Evidence / Review 모두 `log/` 아래). BF Level 은 path 가 아니라 save / restore capability maturity 다 (BF Level 1/2 = manual save/restore discipline; BF Level 3 = deterministic Brief maintenance / validation / stale warning / session-start guidance — 미구현 future scoped work; 무요청 session-start restore-offer automation 은 폐기됨). canonical source-of-truth 는 `docs/brief/brief_spec.md` 다. 본 §9 어느 절의 wording (1차 / 2차 framing 기준) 이 3차 framing 과 충돌하면 본 note 와 brief spec 이 우선한다.
...
- self-dogfooding ProjectRoot 로 동작할 때, `log/` 가 project-local state 로 존재해도 된다 (3차 reconciliation 기준; BRIEF / Evidence / Review 모두 `<ProjectRoot>/log/` 아래). 위 §9 BRIEF wording note 참조 — 본 줄의 1차 / 2차 framing 잔재 wording 은 superseded. root `<ProjectRoot>/brief/` 는 self-dogfooding 에서도 만들지 않는다 (rejected).
...
- `scripts/brief-init.ps1` 의 `Test-IsSourceRepoRoot` refuse guard 는 1차 reconciliation 에서 제거되어 현재 존재하지 않는다(아래 3차 항목과 정합). 당시 이전 posture 를 반영하던 brief 문서 literal wording 의 정합화는 이후 별도 라운드들(3차 reconciliation 반영, 그리고 brief 도메인의 spec 이주 — 현행 single home: `docs/brief/brief_spec.md`)로 수행되었다. 본 문서는 그 reconciliation 들을 승인한 문서가 아니다.
- **Reconciliation 1차 (BRIEF posture reconciliation, historical).** 1차 reconciliation 에서 BRIEF 의 canonical 위치를 `<ProjectRoot>/log/brief/BRIEF.md` 로 옮기고 root `<ProjectRoot>/brief/` 를 forbidden 으로 둔 framing 이 채택되었다. target project 의 persistent footprint 는 `log/` only 로 정리되었다. 이 단계 자체는 이미 supersede 되었다 (아래 2차 항목 참조).
  - 위 1차 framing 하에서 작성된 본 문서의 prose wording — target footprint 를 `log/` + `brief/` 로 적은 표현, `targetFootprintPolicy: log-brief-only`, `<ProjectRoot>/brief/` 또는 `<ProjectRoot>\brief\` 를 별도 footprint / project-local state 항목으로 둔 모든 언급 — 는 historical model 기록으로 그대로 보존된다. (§10 의 footprint diagram node 는 3차 reconciliation 기준으로 정정되었다 — current footprint = `<ProjectRoot>/log/` only, canonical Brief = `<ProjectRoot>/log/brief/BRIEF.md`, root `<ProjectRoot>/brief/` 는 rejected.)
...
graph TD
    ...
    R1["Layer 4: project-local log/<br/>(self project state, not tool payload)"]
    R2["Layer 4: project-local log/"]
    R3["Layer 4: project-local log/"]
...
graph TD
    ...
    subgraph allowed["Allowed (ai-harness usage)"]
        A1["log/ (only persistent footprint)"]
        A2["log/brief/BRIEF.md , log/evidence/ , log/review/"]
    end
    subgraph forbidden["Forbidden — ai-harness install footprint"]
        F0["root brief/ (rejected — canonical Brief is log/brief/BRIEF.md)"]
        F1[".ai-harness/"]
        F2["scripts/ , config/"]
...
graph TD
    ...
    STATE["project-local state:<br/>log/ (BRIEF / Evidence / Review under log/)<br/>(NOT tool payload, NOT install payload,<br/>NOT copied to external targets)"]
...
- `brief/` posture reconciliation (§9.3) — 그 reconciliation 은 brief 도메인 문서의 spec 이주(`docs/brief/brief_spec.md`)로 별도 수행되었으며, 본 문서는 여전히 그것을 승인하는 문서가 아니다.

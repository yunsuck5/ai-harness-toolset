...
- clean target smoke criteria: `docs/archive/audits/CLEAN_TARGET_SMOKE_CRITERIA.md`
- 운영 backlog: `docs/archive/backlog/operations.md`
...
- 현재 `scripts/brief-init.ps1` 의 `Test-IsSourceRepoRoot` refuse, 그리고 `docs/contracts/brief/BRIEF_CONTRACT.md` / `docs/decisions/POST_MVP_PLAN.md` §3 / `docs/archive/audits/TOOLROOT_PROJECTROOT_AUDIT.md` §5.2 의 literal wording 은 이전 posture 를 반영한다. 이들을 본 §9 의 clarified model 과 정합화하는 작업 (script + contract docs reconciliation) 은 **별도 scoped work** 이며, 본 문서 작성 goal 의 범위가 아니다. 본 문서는 그 reconciliation 을 자동 승인하지 않는다.

...
- The promotion gate that authorized implementation was satisfied: the `BACKLOG.md` IU-B-08 entry, scoped `/goal`s per batch, and a Codex review gate on each batch. The real-environment **dogfood** (isolated non-primary machine) of `-Apply` + reinstall verification is now **cleared** (§13); the entrypoint stays narrow and single-purpose. The install/update lifecycle subsystem **LTS readiness is now CLOSED** (`STATUS.md` completed-ledger IU-14; narrative `docs/archive/old-roadmaps/INSTALL_UPDATE_LIFECYCLE_NARRATIVE.md` §7); the Phase 4b one-shot candidate is **RETIRED** (`BACKLOG.md` IU-B-07 [RETIRED]). Nothing remains for this uninstall entrypoint.
...
- `docs/systems/install-update/STATUS.md` — records IU-B-08 (batches 1–3 + isolated-machine dogfood cleared) as a completed-ledger row, and the install/update lifecycle subsystem LTS readiness (ledger IU-14); detailed narrative `docs/archive/old-roadmaps/INSTALL_UPDATE_LIFECYCLE_NARRATIVE.md` §3/§7.

...
- `docs/systems/install-update/STATUS.md` — install-update subsystem 의 current status (current state / completed ledger / accepted residual risks). Phase 4a 이후의 현행 posture 와 original Phase 4 one-shot goal 의 미완료 / RETIRE 상태는 STATUS.md 가 source-of-truth 다 — 본 IDEAS.md 는 그것을 재서술하지 않고 참조만 한다. 본 IDEAS.md 항목 1 은 그 "미완료로 남은 original objective" 의 idea-only 기록이다.

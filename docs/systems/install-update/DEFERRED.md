...
> **LTS-maintenance note (install/update lifecycle subsystem closeout, HEAD `69ef433`).** As of the install/update lifecycle LTS readiness closeout (`STATUS.md` completed-ledger IU-14; full narrative preserved in git history), the install/update-related deferred items below — notably IU-D-01 (git-url hardening residue), IU-D-02 (source-cut actual handling), IU-D-07 (packaging), and IU-D-08 (literal `?` encoding regression gate) — are carried as **accepted residual risks under LTS maintenance**, each retaining its existing reopen condition. They are explicitly **not** LTS-readiness blockers and are **not** silently dropped; the closeout neither resolves them nor changes their reopen conditions. IU-D-05 (Step 6 — *whole-project* post-MVP closeout decision) remains open: the subsystem closeout is a scoped portion of it, not the project-wide decision.
...
| IU-D-04 | Step 5 — `ai-harness-toolset` self-adoption — **Resolved** at resolved HEAD `8293878d` (apply 2026-05-25; STATUS IU-13) | originally: depends on the toolset operating itself via the global model; not started | reopen condition met — a scoped, explicit user-approved self-adoption goal was applied (`INSTALL.md` §2A AI-guided operational install; no productized wrapper) | `STATUS.md` completed-ledger IU-13; full narrative in git history; `INSTALL.md` §2A |
| IU-D-05 | Step 6 — post-MVP closeout decision | kept as a deliberate roadmap step | reached after self-adoption (step 5) | `docs/decisions/POST_MVP_PLAN.md` §11 step 6 |

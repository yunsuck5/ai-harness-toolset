...
- **CLI-only MVP** — closed (`docs/decisions/POST_MVP_PLAN.md` §1; `docs/decisions/DECISIONS.md` "MVP closeout").
- **Codex review subsystem** — operational, in maintenance mode; canonical review task/pass topology adopted (record = `<ProjectRoot>/log/review/<review-task-id>/pass-NN/{input.md,result.md}`). Contract: `docs/contracts/review/REVIEW_RESULT_CONTRACT.md`.
- **Brief narrow source-side primitive** — implemented (`scripts/brief-init.ps1`, `scripts/brief-check.ps1`, `scripts/brief-status.ps1`, `templates/brief/BRIEF.md`). This is not the full BF Level 3 capability. Contract: `docs/contracts/brief/BRIEF_CONTRACT.md`.
- **Global adoption operating layer** — decided; shared / global stable runtime ToolRoot (channel 3) is the current default adoption shape (`docs/decisions/GLOBAL_ADOPTION_DECISION.md`, `docs/contracts/global-invocation/SHARED_GLOBAL_INVOCATION_CONTRACT.md`).
...
- **Native process capture standardization** — **closed** at three source-repo `origin/main` commits: `80212c03b6063d817643f604c464d07dfb84f49c` (containment helper `scripts/lib/native-process.ps1` + its 12-axis Pester suite `tests/native-process.Tests.ps1` + 2 production-script call-site migrations + 9 latent test-helper migrations + Step E `tests/install-pipeline.Tests.ps1` + `tests/resolve-script.Tests.ps1` `Out-String` / Width-8192 cleanup), `80de884325084dd8696b2cabf9401c553b1aeafa` (pre-existing `apply-managed-block` test `AC-AMB-DRYRUN-OK` Korean-console em-dash assertion stabilization), and `47e707295541ab017a6d87e87fd377955d35aa36` (`scripts/verify-ps1.ps1` Step F raw-native-stderr-capture lint expansion). Closed scope does NOT include the deferred reviewer-execution-boundary improvement, review-input governance follow-on, or global installed content refresh — those remain separate scoped /goals. Detail: `docs/decisions/DECISIONS.md` "native process invocation that captures stdout / stderr ..." entry (durable rule + commit pointers + deferred-item list).

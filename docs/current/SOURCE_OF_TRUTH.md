...
- **Implementation:** `docs/systems/skills/STATUS.md` — per-system status + completed ledger + not-yet-done batches. Batch 1 (snippet review-procedure de-duplication) has landed in the working tree (`snippets/CLAUDE_SNIPPET.md` / `AGENTS_SNIPPET.md` `## Review flow` reduced to routing + pointer and the `## Execution discipline` section removed — review-run discipline owned by the `ai-harness-review` skill); Batches 2–4 (plan §8) are not approved. Each migration batch remains a separate scoped goal + Codex review gate + explicit user approval.

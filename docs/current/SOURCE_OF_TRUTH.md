...
- **Implementation:** `docs/systems/skills/STATUS.md` — per-system status + completed ledger + not-yet-done batches. Batch 1 (snippet review-procedure de-duplication) has landed in the working tree (`snippets/CLAUDE_SNIPPET.md` / `AGENTS_SNIPPET.md` `## Review flow` reduced — Batch 1 left a routing pointer that the later routing-pointer-removal batch removed, leaving only the review-record hard boundary — and the `## Execution discipline` section removed; review-run discipline owned by the `ai-harness-review` skill); Batch 2 is **partially** landed (2A/2B session-start restore-offer removal + 2C-0 generic deployed-extension mirror + 2C-1 `ai-harness-brief` skeleton + 2C-2 Brief/BF procedure extraction + snippet minimization; only the 2D closeout remains), and Batches 3–4 (plan §8) are not yet approved. Each migration batch remains a separate scoped goal + Codex review gate + explicit user approval.

...
- **Secondary:** `snippets/CLAUDE_SNIPPET.md` / `snippets/AGENTS_SNIPPET.md` (current always-loaded payload), `snippets/claude-skills/ai-harness-review/SKILL.md` (review-only function-level skill) and `snippets/claude-skills/ai-harness-brief/SKILL.md` (Batch 2C-1 skeleton — created + lifecycle-mirrored, but its snippet routing / procedure extraction is still Batch 2C-2), `docs/README.md` §4 (the high bar for always-on content), `docs/policies/DOCS_OPERATING_MODEL.md` §1 (single-home-plus-pointers).
- **Implementation:** `docs/systems/skills/STATUS.md` — per-system status + completed ledger + not-yet-done batches. Batch 1 (snippet review-procedure de-duplication) has landed in the working tree (`snippets/CLAUDE_SNIPPET.md` / `AGENTS_SNIPPET.md` `## Review flow` reduced to routing + pointer and the `## Execution discipline` section removed — review-run discipline owned by the `ai-harness-review` skill); Batch 2 is **partially** landed (2A/2B session-start restore-offer removal + 2C-0 generic deployed-extension mirror + 2C-1 `ai-harness-brief` skeleton; 2C-2 procedure extraction + 2D closeout remain), and Batches 3–4 (plan §8) are not yet approved. Each migration batch remains a separate scoped goal + Codex review gate + explicit user approval.

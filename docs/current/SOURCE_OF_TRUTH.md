...
- **Implementation:** n/a — design-stage plan; it approves no batch and has no code implementation. The implementing tracks (repo-local instruction creation, snippet residual relocation/reduction, ToolRoot relocation) are each a separate scoped goal + Codex review + explicit user approval.
- **Historical:** n/a (new scope).

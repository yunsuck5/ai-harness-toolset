...
| IU-D-11 | `path.Tests.ps1` NOFB-2 fixture 강화(현행 = 비결함: Get-ToolRoot/Test-IsSourceRepoRoot 가 ProjectRoot 자식 미탐색 + NOFB-1 이 full-payload child 음성 케이스 커버 — NOFB-2 child 는 single-marker 라 payload-like 강도가 약하나 동작 회귀 아님) | NOFB-2 child fixture 를 full source-repo shape(3-marker)로 강화하기로 결정 |

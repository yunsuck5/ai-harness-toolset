...
- git-url `inspect` / `update-source`는 호출자가 exact `-Ref` 또는 recorded `-Branch` 중 target selector를 정확히 하나 명시한다. exact Ref는 inspect/preflight 시점에 remote의 `refs/heads/*`가 현재 광고하는 branch-tip의 40-hex SHA만 받으며 tag·symbolic/short ref·non-tip historical commit은 받지 않는다. apply는 fresh clone에서 그 preflight SHA가 available하고 동일한지를 확인해 같은 SHA를 사용한다. apply 시점의 tip 재광고는 요구하지 않으며 unavailable/mismatch면 canonical output mutation 전에 실패한다.
- selector는 source identity migration이 아니다. exact Ref는 `install.json.branch`·`remote`를 빈 값까지 보존하고, differing Branch/Remote는 source-cut이다. fresh clone은 recorded remote가 non-empty이면 그 alias, empty이면 `origin`을 사용하되 metadata는 바꾸지 않는다. recorded branch가 empty이면 remote symbolic/default HEAD의 advertised branch-tip SHA를 exact Ref로만 선택하며 branch를 추정·persist하지 않는다. selector 생략·동시 지정·git-url `-SourcePath`는 source resolve failure로 닫고 persistent install을 바꾸지 않는다.
- git-url의 D3 source validity와 archive는 같은 resolved SHA tree에 결박되고 default checkout은 의미가 없다. name-based bootstrap도 verified resolved-SHA checkout의 contract/script만 소비한다(구체 command·순서는 `INSTALL.md` 소유). metadata-valid inspect는 payload·root README·source·activation 진단군을 실제 평가한 뒤 payload > source > activation > clean precedence로 하나의 status를 고르며, unresolved git-url target은 status와 무관하게 mutation 전에 차단한다.
...
- **lifecycle scripts** — `scripts/install-global.ps1`(fresh install + first-time managed-block insertion bootstrap + git-url clone remote alias + existing-install selector 안내) · `scripts/install-update.ps1`(inspect/verify/update-source + git-url target-selector·전체 진단·preflight/apply SHA mutation guard) · `scripts/update-global.ps1`(name-based update 재진입; selector를 canonical implementation으로 그대로 전달하고 delegate의 `activation_pending`을 INCOMPLETE로 보존하며 `-Json` stdout을 machine JSON으로 한정) · `scripts/activate-global.ps1`(activation orchestration·canonical-overwrite path) · `scripts/apply-managed-block.ps1`(managed-block replace/insert/remove IO) · `scripts/uninstall-global.ps1` · `scripts/uninstall-finalizer.ps1`.
- **lib** — `scripts/lib/path.ps1`(ToolRoot 5-channel 해소·payload completeness 판정·source-repo 판정·ProjectRoot 해소) · `scripts/lib/resolve-script.ps1`(component script 해소·fallback 정책) · `scripts/lib/activation-surface.ps1`(activation surface plan 의 single home — source→destination map·mutation class) · `scripts/lib/managed-block.ps1`(managed-block primitive — set/add/remove) · `scripts/lib/uninstall-target.ps1`(read-only uninstall plan resolver) · `scripts/lib/install-pipeline-core.ps1`(selected resolved-SHA tree D3 검증 + same-SHA archive를 소유하는 pipeline core; temp-only entry는 `tests/support/install-pipeline-fixture.ps1`).

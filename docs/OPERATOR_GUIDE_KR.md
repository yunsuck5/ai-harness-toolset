...
- 결과 record 는 `<project-root>/log/review/<review-task-id>/pass-NN/` 의 canonical pair (`input.md`, `result.md`) 두 파일로 닫힌다 (`docs/REVIEW_RESULT_CONTRACT.md`). `<review-task-id>` 는 하나의 `/goal` 작업 또는 하나의 review gate 단위이며 Claude Code chat / session id 가 아니다. `pass-NN` 는 같은 task 의 corrective loop 안에서의 각 Codex review attempt 다.
...
- canonical review artifact 는 `log/review/<review-task-id>/pass-NN/input.md` 와 `log/review/<review-task-id>/pass-NN/result.md` 두 파일뿐이다. 다른 sidecar 파일은 contract 의 일부가 아니다 (`docs/REVIEW_RESULT_CONTRACT.md`).
...
canonical artifact 는 `<ProjectRoot>/log/review/<review-task-id>/pass-NN/input.md` 와 `<ProjectRoot>/log/review/<review-task-id>/pass-NN/result.md` 두 파일뿐이다. 다른 sidecar 파일은 contract 의 일부가 아니다 (`docs/REVIEW_RESULT_CONTRACT.md`).
...
전체 contract 는 `docs/REVIEW_RESULT_CONTRACT.md` 가 source-of-truth 다. removed-legacy artifact shape 의 historical reference 는 `docs/backlog/review.md` 와 `docs/backlog/operations.md` 에 격리되어 있으며 operator path 가 아니다.

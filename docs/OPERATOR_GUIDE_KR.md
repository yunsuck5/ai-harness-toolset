...
- 한 번의 review pass 는 두 단계 entry script 호출로 닫힌다: `review-prepare.ps1` 가 새 pass directory 를 발급하고 `input.md` 를 seed → AI 가 `input.md` 본문을 직접 작성 → `review-run.ps1` 가 `review-input-verify.ps1` 로 heading shape 를 검증한 뒤 Codex CLI 를 1 회 실행해 같은 pass directory 에 `result.md` 를 작성한다.
- 결과 record 는 `<project-root>/log/review/<review-task-id>/pass-NN/` 의 canonical pair (`input.md`, `result.md`) 두 파일로 닫힌다 (`docs/REVIEW_RESULT_CONTRACT.md`). `<review-task-id>` 는 하나의 `/goal` 작업 또는 하나의 review gate 단위이며 Claude Code chat / session id 가 아니다. `pass-NN` 는 같은 task 의 corrective loop 안에서의 각 Codex review attempt 다. 현재 script 는 canonical 두 단계 layout 대신 flat 한 `log/review/<script-allocated-id>/` 한 단계 디렉터리를 발급하며, 이 transitional divergence 는 `docs/REVIEW_RESULT_CONTRACT.md` §4a 에 격리되어 있다.
...
└── review/    # review record (docs/REVIEW_RESULT_CONTRACT.md)
    └── <review-task-id>/
        ├── pass-01/
...
- `pass-NN` 은 같은 review task 안의 corrective loop attempt 다. 첫 attempt `pass-01`, 다음 `pass-02`, ... 로 증가한다.
- review record 의 retention 단위는 `<review-task-id>/` 디렉터리 전체 또는 그 안의 개별 `pass-NN/` 디렉터리다. 사용자가 손으로 지운다. 자동 prune / rotate 는 없다.
- 현재 script 는 canonical 두 단계 layout 대신 flat 한 `log/review/<script-allocated-id>/` 한 단계 디렉터리를 발급한다 (`docs/REVIEW_RESULT_CONTRACT.md` §4a transitional divergence). 본 layout 도식의 `<review-task-id>/pass-NN/` 는 canonical 표기이며, 운영자 / AI 는 각 script-allocated-id 를 한 review task 의 한 pass 로 매핑해 보고한다.
...
flowchart TD
    ...
    B -- yes --> P[scripts/review-prepare.ps1<br/>allocate new pass directory<br/>(canonical: &lt;review-task-id&gt;/pass-NN;<br/>current script: flat &lt;script-allocated-id&gt;),<br/>seed input.md from templates/review-input.md]
    P --> C[Claude Code overwrites<br/>pass input.md<br/>with the actual review request]
    C --> R[scripts/review-run.ps1 -RunId &lt;script-allocated-id&gt;]
...
- canonical review artifact 는 `log/review/<review-task-id>/pass-NN/input.md` 와 `log/review/<review-task-id>/pass-NN/result.md` 두 파일뿐이다 (canonical 두 단계 layout). 다른 sidecar 파일은 contract 의 일부가 아니다 (`docs/REVIEW_RESULT_CONTRACT.md`). 현재 script 는 flat `<script-allocated-id>/` 단일 디렉터리를 발급하는 transitional divergence 가 있다 (§4a 참조).
...
sequenceDiagram
    ...
    CC->>CC: scope / target files / context 결정
    CC->>RP: invoke once
    RP-->>CC: allocate new pass directory; seed input.md<br/>(canonical: <review-task-id>/pass-NN;<br/>current script: flat <script-allocated-id>)
    CC->>CC: overwrite pass input.md<br/>with the actual review request<br/>(templates/review-input.md as shape)
    CC->>RR: invoke once with -RunId <script-allocated-id>
...
|---|---|---|
| `현재 진행한 작업 코덱스 리뷰 진행해` | Mode A — changed files | git status / diff 확인 → 변경된 tracked 파일을 target 으로 결정 → 이 작업의 `<review-task-id>` 결정 → `review-prepare.ps1` 로 새 pass directory 발급 + input.md seed → pass `input.md` 본문 직접 작성 (`templates/review-input.md` 기준) → `review-run.ps1 -RunId <script-allocated-id>` 호출 (verify + Codex 1 회 실행) → pass `result.md` 의 `## Verdict` 확인 → review-task path / final pass / verdict / corrective loop count 보고. 변경 파일이 0 개면 중단 (Mode B 로 자동 전환하지 않음). |
...
3. `<project-root>/log/brief/BRIEF.md` (canonical Brief — project-local runtime artifact, gitignored under `log/`) 를 manual save 로 직접 갱신. `docs/BRIEF_CONTRACT.md` 의 canonical heading set 을 그대로 사용. root `<project-root>/brief/` 는 만들지 않는다 (rejected).
...
본 절의 명령은 일상 운용용이 아니라, §7-§8 의 자연어 UX 가 사용 불가능하거나 인자 조합을 직접 디버그해야 할 때를 위한 fallback / reference 다. 일상 운용은 §7 의 자연어 의도가 정식 entrypoint 이며, 진입 script 두 개를 두 단계로 호출한다 — (a) `scripts/review-prepare.ps1` 로 새 pass directory 발급 + input.md seed → (b) AI 가 pass `input.md` 본문 작성 → (c) `scripts/review-run.ps1 -RunId <script-allocated-id>` 로 verify + Codex 1 회 실행.

canonical artifact 는 `<ProjectRoot>/log/review/<review-task-id>/pass-NN/input.md` 와 `<ProjectRoot>/log/review/<review-task-id>/pass-NN/result.md` 두 파일뿐이다 (canonical 두 단계 layout). 다른 sidecar 파일은 contract 의 일부가 아니다 (`docs/REVIEW_RESULT_CONTRACT.md`). 현재 script 는 flat `<script-allocated-id>/` 디렉터리를 발급하며, 운영자 / AI 는 그 disk path 를 한 review task 의 한 pass 로 매핑해 보고한다 (`docs/REVIEW_RESULT_CONTRACT.md` §4a).
...
#    (a) review-prepare.ps1 호출 -- 새 pass directory 를 발급하고
#        input.md 를 templates/review-input.md 로부터 seed.
#        현재 script 는 canonical <review-task-id>/pass-NN/ 두 단계 layout 대신
#        flat <script-allocated-id>/ 한 단계 디렉터리를 만든다 (REVIEW_RESULT_CONTRACT §4a).
#    (b) AI 가 pass input.md 본문을 review request 로 직접 작성.
#    (c) review-run.ps1 -RunId <script-allocated-id> 호출 -- review-input-verify.ps1 로
#        heading shape 를 검증한 뒤 Codex CLI 를 1 회 실행, 같은 pass directory 에
#        result.md 를 작성.
#    그 외 sidecar artifact 는 canonical contract 가 보장하지 않는다.
...
#       (canonical <review-task-id>/pass-NN/ 가 아니라 flat <script-allocated-id>/ 가 만들어진다.)
#   (b) AI 가 log/review/<script-allocated-id>/input.md 본문 직접 작성
#   (c) -File scripts/review-run.ps1 -ToolRoot (Get-Location).Path -RunId <script-allocated-id>
...
`log/review/<review-task-id>/pass-NN/` 의 canonical artifact 는 두 파일이다. 다른 sidecar 파일은 contract 의 일부가 아니다.

| 파일 | 작성자 | 역할 |
|---|---|---|
| `input.md` | Claude Code (operator-role AI) | reviewer 에게 전달되는 모든 입력. `## Context`, `## Required inspection paths`, `## Review questions`, `## Constraints`, `## Final verdict` 5 개의 required heading + 권장 informational heading (`## Stage`, `## Purpose`, `## Target files`) 으로 구성. `templates/review-input.md` 가 shape 기준. |
| `result.md` | Codex CLI (`--output-last-message`) | reviewer 의 응답. 정확히 1 개의 `## Verdict` heading 과 그 다음 첫 비어있지 않은 줄에 `yes` / `no` / `yes with risk` 중 하나. `## Findings` / `## Risks` / `## Notes` section 자유 추가. `templates/review-result.md` 가 shape 기준. |

전체 contract 는 `docs/REVIEW_RESULT_CONTRACT.md` 가 source-of-truth 다. removed legacy artifact (`meta.json`, `target-files.list`, `result.json`, 외부 staging folder) 의 historical reason 은 `docs/backlog/review.md` 와 `docs/backlog/operations.md` 에 격리되어 있으며 operator path 가 아니다.

---

## 11. verdict handling

`result.md` 의 `## Verdict` 다음 첫 줄은 정확히 다음 세 값 중 하나여야 한다.

| verdict | 의미 | 사용자 행동 가이드 |
|---|---|---|
| `yes` | 검토 범위 내에서 진행 가능 | 다음 작업 단계로 이동. commit / push / release 는 여전히 별도 승인. |
| `yes with risk` | 진행 가능하나 명시된 risk 동반 | result.md 의 `## Risks` 항목을 읽고 사용자 본인이 risk 수용 여부를 판단. 자동 게이트 아님. |
| `no` | 검토 범위 내에서 진행 불가 | result.md 의 `## Findings` / required changes 를 사용자가 읽고 보정한 뒤, 같은 `<review-task-id>` 아래에 새 `pass-NN` 으로 다시 review 실행. |

`Verdict: yes`, `Final verdict: yes`, prose 안의 verdict 등 inline 형태는 거부된다. lowercase 정확 매치만 통과한다.
...
toolset 은 verdict 를 읽어서 git 동작이나 release 동작을 트리거하는 wrapper 를 제공하지 않는다 (`docs/REVIEW_RESULT_CONTRACT.md` §10 non-goals).
...
|---|---|
| Pure review-loop acceptance | `log/review/<review-task-id>/pass-NN/input.md`, `log/review/<review-task-id>/pass-NN/result.md` (canonical 두 단계 layout; 현재 script 는 flat `log/review/<script-allocated-id>/` 발급, §4a transitional divergence) |
| BF save / closeout acceptance | `<ProjectRoot>/log/brief/BRIEF.md` (canonical Brief — project-local runtime artifact, gitignored under `log/`). BF save protocol 의 갱신 대상도 이 자리이며, root `<ProjectRoot>/brief/` 는 만들지 않는다 (rejected). `log/chatlog/current/resume.md` / `summary.md` 는 갱신 대상이 아니다 (legacy / deprecation candidate, `docs/CHATLOG_CONTRACT.md`). |
...
[ ] AI 가 사용자의 자연어 의도로부터 review-prepare.ps1 호출 -> pass input.md 본문 직접 작성 -> review-run.ps1 -RunId <script-allocated-id> 호출의 두 단계로 한 pass review 를 닫는다.
[ ] 두 단계 모두 channel 3 global stable install 로 자동 resolve 되거나 명시적 -ToolRoot 로 결정된다.
[ ] 운영자 / AI 가 <review-task-id> (한 /goal 작업 또는 한 review gate 단위) 와 pass-NN (corrective loop attempt) 을 결정해 보고한다. 현재 script 의 flat <script-allocated-id> 는 한 review task 의 한 pass 로 매핑된다 (REVIEW_RESULT_CONTRACT §4a).
[ ] runtime artifact 가 ProjectRoot 의 log/ 아래에만 생성되고, ToolRoot (channel 3 payload) 는 변경되지 않는다.
[ ] 각 pass directory (canonical: <review-task-id>/pass-NN; 현재 script: flat <script-allocated-id>) 아래 canonical artifact 두 파일 (input.md, result.md) 이 모두 존재한다.
...
[ ] .ai-harness/scripts/review-prepare.ps1 호출 -> AI 가 pass input.md 본문 직접 작성 -> .ai-harness/scripts/review-run.ps1 -RunId <script-allocated-id> 호출의 두 단계로 한 pass review 가 닫힌다.
[ ] 각 pass directory (canonical: <review-task-id>/pass-NN; 현재 script: flat <script-allocated-id>) 아래 canonical artifact 두 파일 (input.md, result.md) 이 모두 존재한다.
...
6. Claude Code 에 자연어로 review 의도를 표현한다 (`현재 진행한 작업 코덱스 리뷰 진행해`). Claude Code 는 이 작업의 `<review-task-id>` 와 `pass-NN` 을 결정한 뒤 `review-prepare.ps1` 호출 → pass `input.md` 본문 직접 작성 → `review-run.ps1 -RunId <script-allocated-id>` 호출의 두 단계 흐름으로 한 pass review 를 닫는다.
...
- `<project-root>/log/brief/BRIEF.md` (`docs/BRIEF_CONTRACT.md`, canonical Brief — project-local, operator-local, source-control-excluded runtime artifact under `<project-root>/log/`, gitignored by default and not a commit / push target). root `<project-root>/brief/` 와 user-home operator-local runtime root 는 Brief 자리가 아니다 (rejected).
...
verdict 의 source-of-truth 는 같은 review task 의 final pass directory `log/review/<review-task-id>/pass-NN/result.md` 의 `## Verdict` 다 (`docs/REVIEW_RESULT_CONTRACT.md`).

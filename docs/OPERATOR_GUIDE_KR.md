...
> **Note — snippet drift.** 본 절은 현재 채택된 `snippets/CLAUDE_SNIPPET.md` / `snippets/AGENTS_SNIPPET.md` 의 BF save / restore-offer protocol 동작을 fidelity 로 기술한다. 단 두 snippet 본문은 본 가이드 및 `docs/BRIEF_CONTRACT.md` / `docs/CHATLOG_CONTRACT.md` 가 정정한 framing 보다 앞선 시점의 모델 — "`log/brief/BRIEF.md` 가 canonical BRIEF (BF Level 3)", "`log/chatlog/current/resume.md` / `summary.md` 가 canonical BF Level 1/2", "root `<ProjectRoot>/brief/` 는 forbidden" — 으로 작성되어 있어 현재 contract 와 drift 상태다. 두 snippet 의 본문 갱신은 별도 scoped 승인이 필요한 future scoped work 이며 본 라운드의 변경 scope 밖이다. 따라서 채택한 snippet 을 그대로 따르면 두 자리에 갱신이 발생할 수 있으나, 그 자리는 **canonical 자리가 아니라** failed intermediate / legacy migration source / deprecation candidate 다 (`docs/CHATLOG_CONTRACT.md`). target repo 의 product canonical Brief 자리는 `<ProjectRoot>/brief/BRIEF.md` 이며, BF Level 1/2 의 manual save / restore discipline 이 향하는 자리도 그쪽이다 (`docs/BRIEF_CONTRACT.md`). 현재 snippet protocol 의 writer destination 을 `brief/BRIEF.md` 로 routing 하고 BF Level 3 의 deterministic 자동화 (validation / stale warning / restore-offer) 를 갖추는 작업은 미구현 future scoped work 다. 본 절의 protocol description 은 현재 동작의 fidelity 기록이며, 그 destination 을 canonical 자리로 해석하지 않는다. 본 가이드 / contract docs 와 snippet 본문 사이에 framing 이 충돌하면 본 라운드 기준 **현행 contract docs (`docs/BRIEF_CONTRACT.md`, `docs/CHATLOG_CONTRACT.md`) 의 framing 이 우선** 한다.

`ai-harness-toolset` 의 일상 운용에서 사용자는 raw PowerShell 명령을 직접 입력하지 않고, Claude Code 안에서 자연어 의도를 표현한다. 채택된 snippet 이 활성화되어 있으면 Claude Code 는 현재 protocol 에 따라 BF artifact 를 갱신한다.
...
이 의도가 감지되면 Claude Code 는 현재 채택된 snippet protocol 의 절차를 따른다. 아래 3 / 4 단계의 writer destination 은 현재 snippet protocol 동작상 그렇게 되어 있다는 fidelity 기록이며, 그 두 자리를 canonical Brief 자리로 해석하지 않는다 (`docs/CHATLOG_CONTRACT.md` 의 deprecation candidate 분류).
...
3. 현재 snippet protocol 동작상 `log/chatlog/current/resume.md` 자리에 갱신. 그 자리는 legacy / deprecation candidate 이며 target product canonical (`<project-root>/brief/BRIEF.md`) 은 별개다.
4. 현재 snippet protocol 동작상 `log/chatlog/current/summary.md` 자리에 갱신. 동일하게 legacy / deprecation candidate.
5. 관련 review / evidence / Chatlog artifact 는 path / link 로만 참조 — 본문 인라인 금지.
6. BRIEF 는 짧게 유지. 상세 내용이 필요하면 path 만 가리킨다.
...
### 새 Claude Code 세션 진입 시 — restore-offer (현재 snippet protocol)

사용자가 같은 프로젝트에서 새 Claude Code 세션을 열면 현재 snippet protocol 의 manual discipline 동작은 다음과 같다. 본 흐름은 채택된 snippet 에 의존하며, deterministic restore-offer automation (BF Level 3) 은 미구현이다.

1. Claude Code 가 snippet 의 read order 에 따라 BRIEF 존재 여부를 확인. target repo 의 product canonical 자리는 `<project-root>/brief/BRIEF.md` 다 (`docs/BRIEF_CONTRACT.md`). 현재 snippet 본문이 추가로 `log/chatlog/current/resume.md` / `summary.md` 를 fallback 으로 가리키는 경우, 그 자리는 legacy / deprecation candidate 임을 인지하고 사용한다.
...
읽을 수 있는 BRIEF 가 어느 자리에도 없으면 raw transcript / 누적 Chatlog 본문을 읽어 BRIEF 를 임의로 재구성하지 않고, BRIEF 가 비어있거나 미흡함을 사용자에게 보고한 뒤 다음 행동을 묻는다.
...
위 7b 본문의 UX 는 채택된 `snippets/CLAUDE_SNIPPET.md` / `snippets/AGENTS_SNIPPET.md` 의 protocol 동작 형태 (자연어 trigger → BF save / restore-offer 흐름) 를 fidelity 로 기술한 것이다. 그러나 두 snippet 본문의 path 표현, "canonical" / "forbidden" 라벨링, BF Level 정의는 현행 contract docs (`docs/BRIEF_CONTRACT.md`, `docs/CHATLOG_CONTRACT.md`) 와 drift 상태다 (위 절두 note 참조). 따라서 본 가이드는 두 자리의 substance 일치를 hard rule 로 강제하지 않는다 — drift 가 해소되기 전까지 framing 의 source-of-truth 는 contract docs 다. snippet 본문 갱신은 별도 scoped 승인이 필요한 future scoped work 다.
...
- dogfooding target repo 라도, 채택한 snippet 또는 사용자 발화에 BF save / closeout 의도가 포함된 경우에만 현재 snippet protocol 이 `log/chatlog/current/` 자리에 파일을 갱신한다. 그 자리는 canonical 자리가 아니라 legacy / deprecation candidate 다 (`docs/CHATLOG_CONTRACT.md`).
...
| BF save / closeout acceptance (현재 snippet protocol) | 현재 snippet 동작상 `log/chatlog/current/resume.md`, `log/chatlog/current/summary.md` 자리에 갱신. 두 파일은 legacy / deprecation candidate 이며 canonical 자리가 아니다 (`docs/CHATLOG_CONTRACT.md`). target canonical 인 `<ProjectRoot>/brief/BRIEF.md` 로의 routing 은 future scoped work. |
...
따라서 13 절의 checklist 는 review artifact 축만 점검한다. 같은 repo 에서 BF save / closeout 의도까지 dogfooding 하는 경우에만 현재 snippet 의 writer destination 갱신을 별도 축으로 점검한다. 두 축의 fail 은 분리해서 보고한다.

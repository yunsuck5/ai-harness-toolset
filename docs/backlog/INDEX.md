...
> **Routing (access-pattern restructure, 2026-05-23; review domain migrated since).** Open backlog work lives in the per-domain/per-system backlog files: review domain → `docs/review/review_backlog.md`; install/update/operational → `docs/systems/install-update/BACKLOG.md`. Each is a self-contained triage entrypoint. The original mixed bodies were superseded by these triage entrypoints; their full historical text is preserved in git history — not current authority and not the open-work entrypoint. Closed items: install-update is authoritative in its `STATUS.md` completed-ledger; migrated domains (brief, review) preserve closed-item detail in git history.

## 탐색 규칙

- open work → per-domain/per-system backlog (`docs/review/review_backlog.md`, `docs/systems/install-update/BACKLOG.md`) 를 본다. 이들이 self-contained 진입점이다.
- closed item → install-update 는 `STATUS.md` completed-ledger 가 authoritative; 이주된 도메인(brief·review)은 git history 가 보존.
...
- review 도메인의 open work → `docs/review/review_backlog.md`; install-update system 의 open work → `docs/systems/install-update/BACKLOG.md` (self-contained entry).

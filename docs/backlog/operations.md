...
- Review: `<ProjectRoot>/log/review/<review-task-id>/pass-NN/` (canonical 두 단계 layout per `docs/REVIEW_RESULT_CONTRACT.md`; 현재 script 는 flat `<ProjectRoot>/log/review/<script-allocated-id>/` 를 emit 하는 transitional divergence — REVIEW_RESULT_CONTRACT §4a).
...
본 라운드의 boundary 상 다음 항목은 변경하지 않았다. 본 backlog 항목은 어느 것도 자동 승인하지 않는다.
...
- `snippets/CLAUDE_SNIPPET.md`, `snippets/AGENTS_SNIPPET.md`. (2차 reconciliation framing 그대로 남아 있다 — refresh 는 사용자 명시 승인이 필요한 별도 managed-block replacement step, `docs/roadmap/GLOBAL_ADOPTION_DECISION.md` §6.)
- `templates/brief/BRIEF.md` preamble 의 BF Level 3 라벨 / canonical 자리 표현 등.
- destination `CLAUDE.md` / `AGENTS.md` 의 managed block refresh (project-root / user-global). user-approved 별도 작업.
- `docs/DECISIONS.md`, `docs/roadmap/CLEAN_TARGET_SMOKE_CRITERIA.md`, `docs/roadmap/GLOBAL_ADOPTION_DECISION.md`, `docs/roadmap/TOOLROOT_PROJECTROOT_AUDIT.md`, `docs/roadmap/REVIEW_EFFORT_GUIDE.md` 등 본 라운드 inspect 목록 외 docs 의 BRIEF / footprint wording (현행 contract docs 와 충돌 시 contract docs 우선).

...
- **Status**: 부분 완료 + Brief / BF Level contract 재정합. global stable runtime ToolRoot 모델로의 docs wording 정합화 (`docs/OPERATOR_GUIDE_KR.md`, `README.md`, `docs/AI_HARNESS_TOOLSET_SCOPE.md`, `docs/roadmap/POST_MVP_PLAN.md` 의 channel 3 / channel 5 framing) 자체는 closeout 이다. 그러나 그 라운드에서 함께 도입되었던 "BRIEF posture reconciliation" — `log/brief/BRIEF.md` 를 canonical 자리로 격상하고 root `brief/` 를 forbidden 으로 둔 framing — 은 별도 round 에서 정정되었다. 현재 contract 의 framing 은 다음과 같다 (자세한 내용은 `docs/BRIEF_CONTRACT.md`, `docs/CHATLOG_CONTRACT.md`): (a) BF Level 은 path 가 아니라 save / restore capability maturity 다. (b) target repo 의 product canonical restore source 는 `<ProjectRoot>/brief/BRIEF.md` 다. (c) `scripts/brief-init.ps1` / `scripts/brief-check.ps1` 가 seed / 검증하는 `<ProjectRoot>/log/brief/BRIEF.md` 는 narrow source-side primitive 의 현재 destination 이며, ai-harness-toolset 내부 runtime artifact 로 존재할 수 있으나 target repo product canonical 로 승격되지 않는다. (d) `log/chatlog/current/resume.md` / `summary.md` 는 canonical 자리가 아니라 failed intermediate / legacy migration source / deprecation candidate 다. (e) BF Level 3 의 full capability — deterministic Brief maintenance / validation / stale warning / session-start guidance / restore-offer / writer routing 의 target canonical 정합화 — 은 미구현 future scoped work 다. 본 backlog 항목은 그 framing 정합화 자체를 자동 승인하지 않는다 — wording 갱신은 별도 scoped goal 의 결과로 contract docs 에 반영된다.

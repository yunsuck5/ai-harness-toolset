...
- **Status**: 부분 완료 + Brief / BF Level contract 재정합 + source snippet 정합화 완료. global stable runtime ToolRoot 모델로의 docs wording 정합화 (`docs/OPERATOR_GUIDE_KR.md`, `README.md`, `docs/AI_HARNESS_TOOLSET_SCOPE.md`, `docs/roadmap/POST_MVP_PLAN.md` 의 channel 3 / channel 5 framing) 자체는 closeout 이다. 그 라운드에서 함께 도입되었던 "BRIEF posture reconciliation" — `log/brief/BRIEF.md` 를 canonical 자리로 격상하고 root `brief/` 를 forbidden 으로 둔 framing — 은 별도 round 에서 정정되었고, 그 corrected framing 에 맞춰 source `snippets/CLAUDE_SNIPPET.md` / `snippets/AGENTS_SNIPPET.md` 도 정합화 완료 (별도 round). 현재 contract 의 framing 은 다음과 같다 (자세한 내용은 `docs/BRIEF_CONTRACT.md`, `docs/CHATLOG_CONTRACT.md`): (a) BF Level 은 path 가 아니라 save / restore capability maturity 다. (b) target repo 의 product canonical restore source 는 `<ProjectRoot>/brief/BRIEF.md` 다. (c) `scripts/brief-init.ps1` / `scripts/brief-check.ps1` 가 seed / 검증하는 `<ProjectRoot>/log/brief/BRIEF.md` 는 narrow source-side primitive 의 현재 destination 이며, ai-harness-toolset 내부 runtime artifact 로 존재할 수 있으나 target repo product canonical 로 승격되지 않는다. (d) `log/chatlog/current/resume.md` / `summary.md` 는 canonical 자리가 아니라 failed intermediate / legacy migration source / deprecation candidate 다. (e) BF Level 3 의 full capability — deterministic Brief maintenance / validation / stale warning / session-start guidance / restore-offer / writer routing 의 target canonical 정합화 — 은 미구현 future scoped work 다. 운영자가 이전 라운드에 destination `CLAUDE.md` / `AGENTS.md` 의 managed block 에 적용한 본문은 explicit refresh 단계 (`docs/roadmap/GLOBAL_ADOPTION_DECISION.md` §6) 를 거쳐야 옛 모델에서 갱신된다 — 그 refresh 자체는 사용자 명시 승인이 필요한 별도 작업이며 본 backlog 항목이 자동 승인하지 않는다.
...
- `brief/` vs `log/brief/BRIEF.md` 관련 framing (contract docs) — **재정합 완료.** 본 backlog 항목의 이전 라운드에서 contract docs 가 `log/brief/BRIEF.md` 를 canonical 자리로 격상하고 root `brief/` 를 forbidden 으로 둔 framing 은 이후 별도 scoped goal 로 정정되었다. 현재 `docs/BRIEF_CONTRACT.md` 는 BF Level 을 path 가 아니라 save / restore capability maturity 로 정의하고, target repo product canonical 을 `<ProjectRoot>/brief/BRIEF.md` 로, `<ProjectRoot>/log/brief/BRIEF.md` 를 narrow source-side primitive 의 현재 destination 으로 분리한다. `docs/CHATLOG_CONTRACT.md` 는 `log/chatlog/current/resume.md` / `summary.md` 를 canonical 자리에서 demote 하여 failed intermediate / legacy migration source / deprecation candidate 로 분류했다. 두 contract 의 정합 자체는 closeout 이지만, 본 backlog 항목 status 줄의 (a)–(e) framing 도 그 갱신과 같은 정신이다.
- `brief/` historical wording (roadmap docs) — **완료 (two-step reconciliation annotated).** `docs/roadmap/GLOBAL_INSTALL_UPDATE_MODEL.md` §9 BRIEF wording note 와 §9.3 Reconciliation 절, `docs/roadmap/SHARED_GLOBAL_INVOCATION_CONTRACT.md` 의 D7 Decision 줄 / Superseded note / BriefRoot 정의, `docs/roadmap/CLEAN_TARGET_SMOKE_CRITERIA.md` §"BRIEF posture reconciliation — applied" 본문, `docs/DECISIONS.md` 의 BRIEF 한 줄 entry 가 모두 2-step reconciliation framing 으로 정정되었다 — 즉 1차 reconciliation (`log/brief/BRIEF.md` canonical + root `brief/` forbidden) 은 historical 로 남기되, 2차 (현행) reconciliation (target product canonical = `<ProjectRoot>/brief/BRIEF.md`; `<ProjectRoot>/log/brief/BRIEF.md` = source-side primitive seed destination, not canonical) 이 우선임을 본문에서 명시. CLEAN_TARGET_SMOKE_CRITERIA.md 의 SC body assertion 자체는 변경되지 않았다 — primitive-behavior smoke 의 정의가 양쪽 reconciliation 모두에서 동일하기 때문이다. canonical source-of-truth 는 `docs/BRIEF_CONTRACT.md` 와 `docs/CHATLOG_CONTRACT.md` 다.

...
- **Status**: in progress — `docs/OPERATOR_GUIDE_KR.md` (이전 라운드), `README.md`, `docs/AI_HARNESS_TOOLSET_SCOPE.md`, `docs/roadmap/POST_MVP_PLAN.md` 의 wording 정합화 완료. `brief/` vs `log/brief/BRIEF.md` historical wording 중 contract docs (`docs/BRIEF_CONTRACT.md`, `docs/CHATLOG_CONTRACT.md`) 부분은 정리 완료. 잔여: roadmap docs 의 superseding-note 의존 historical wording, 그리고 실제 BF Level 3 기능 검증 (아래 마지막 bullet 묶음).
...
- `brief/` historical wording (roadmap docs) — **deferred.** `docs/roadmap/GLOBAL_INSTALL_UPDATE_MODEL.md` §9 / §9.3, `docs/roadmap/SHARED_GLOBAL_INVOCATION_CONTRACT.md` D7 의 superseded note 등에 아직 superseding-note 의존 historical wording 이 남아 있다. 이들은 roadmap / design-contract 문서이며 contract-docs 안정화 batch 의 allowed-file scope 밖이므로 별도 scoped pass 로 처리한다.
- BF Level 3 기능 검증 — **deferred.** `scripts/brief-init.ps1` / `scripts/brief-check.ps1` 의 실제 동작 (seed / no-overwrite / shape 검증) 검증은 docs 안정화 batch 의 대상이 아니다. 별도 scoped goal 을 거친다.

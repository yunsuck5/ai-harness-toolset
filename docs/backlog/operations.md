...
- **Status**: in progress — `docs/OPERATOR_GUIDE_KR.md` (이전 라운드), `README.md`, `docs/AI_HARNESS_TOOLSET_SCOPE.md`, `docs/roadmap/POST_MVP_PLAN.md` 의 wording 정합화 완료. 잔여: `brief/` vs `log/brief/BRIEF.md` historical wording 정리 (아래 마지막 bullet).
...
global stable runtime ToolRoot 모델로의 전환이 snippets / skill / resolution docs 에는 반영되었고, 아래 named docs 의 정합화 진행 상태는 각 bullet 에 기록한다 — `docs/OPERATOR_GUIDE_KR.md`, `README.md`, `docs/AI_HARNESS_TOOLSET_SCOPE.md`, `docs/roadmap/POST_MVP_PLAN.md` 는 정합화 완료이고, 잔여 추적 대상은 `brief/` 관련 historical wording 한 항목이다.

...
- `brief/` historical wording (roadmap docs) — **부분 정정.** `docs/roadmap/GLOBAL_INSTALL_UPDATE_MODEL.md` §9 와 `docs/roadmap/SHARED_GLOBAL_INVOCATION_CONTRACT.md` D7 의 superseding-note 정리는 이전 라운드에서 이루어졌다. 그러나 그 라운드의 reconciliation 결론 자체 (canonical = `log/brief/BRIEF.md`) 는 이후 정정되어 현재 contract framing 과 다르다 (status 줄 참조). 두 roadmap doc 의 본문 갱신은 본 항목의 자동 승인 대상이 아니며, 별도 scoped goal 로 처리한다.

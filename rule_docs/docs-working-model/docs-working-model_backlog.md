...
| DWM-B-12 | live domain repeal·retire·rename의 전체 절차 — affected reference, domain backlog/identity, Spec path, glossary 처분, implementation/routing, terminal/resume. Whole terminal-rule 처분은 active DWM rule이 소유하고 candidate identity/kind transition은 DWM-B-13이 소유한다. | 실제 live domain 처분 또는 scoped domain-lifecycle revision이 이 절차를 요구할 때 |

...
| DWM-B-13 | candidate identity/kind transition — two-candidate merge·rehome(rule↔domain kind-change·id/category 이동)·split·rename-before-promotion 의 lineage(candidate 정체성과 전환 뒤 유지하기로 한 current-bearing ideas) 보존 경로(현 유일 경로 = discard+re-incubate 로 lineage 유실) | 실제 merge/rehome 또는 promotion 전 identity change가 필요할 때. Live domain identity transition은 active DWM lifecycle이 소유하고 이 row는 pre-promotion candidate identity/lineage transition 질문만 소유한다. DWM-B-05(cross-folder E3 lineage)와 중첩 |
...
| DWM-B-18 | Terminal rule 전체 retire/replacement 계약의 첫 실제 owner-local pilot | 안전한 실제 terminal rule 처분이 사용자 승인되고 replacement 유무와 retained meaning 처분(없음 포함)을 특정할 수 있을 때 |
| DWM-B-19 | Distributed terminal rule 처분의 source→installed payload→activation 잔여와 old route 부재를 첫 실제 runtime에서 검증 | 실제 distributed rule 처분과 별도 global update/activation 범위가 사용자 승인될 때 |
| DWM-B-20 | Live domain retire/repeal/supersede 계약의 첫 실제 owner-local source pilot | exact live domain 처분, successor 유무와 retained meaning/future work disposition, source mutation이 사용자 승인될 때 |
| DWM-B-21 | Deployed live domain 처분의 source→installed payload→activation 잔여와 old route 부재를 첫 실제 runtime에서 검증 | deployed surface가 있는 실제 domain 처분과 별도 global update/activation 범위가 사용자 승인될 때 |
| DWM-B-22 | Late-finding selector·minimum-affected rewind/resume·forward-correction 계약의 첫 실제 owner-local pilot | 실제 inactive/late policy 또는 명시적 user target change가 current work에 발생해 controlling owner, affected dependency, invalidated/preserved material과 resume decision을 기록할 수 있을 때 |
| DWM-B-23 | Cross-owner handoff 계약의 첫 실제 source/target disposition·shared-interface dependency pilot | 실제 foreign-owner finding이 발생해 target actual disposition/status와 source independence 또는 shared-interface coupling·commit order를 판정할 수 있을 때 |

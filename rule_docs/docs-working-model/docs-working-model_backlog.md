...
| DWM-B-02 | TERM-RESERVE 후속 — `owner-pending` entry field-schema·`pending`→`owner-pending` 전이·finalization monotonicity 기계화(bound candidate `pending`의 explicit field-list / complete-legacy provenance와 그 범위의 structural/common-pointer validation은 현 checker 소유; ambiguous prose·candidate provenance 누락·path-vs-concept 잔여는 false stop 방지를 위해 manual) | 첫 실제 `owner-pending` 등록/전이가 발생하거나 finalization-owner close에서 carry-forward decision이 필요할 때 |

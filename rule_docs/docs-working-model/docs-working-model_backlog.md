...
| DWM-B-17 | repo 내부 `rules/**`의 한국어 본문+고정 영어 기술어 authoring policy를 교체·완화할지 재검토 | 한국어 규범 문면의 해석 분산 사고가 반복 관측되거나, 외부 협업자/타 벤더 agent의 repo 내부 `rules/**` 직접 소비에서 언어 해석 차이로 인한 정정·운용 실패가 실제로 관측될 때 |

...
| `rules/terminology-glossary.md` | candidate id와 두 분류값에 실제 프로젝트 공용 용어 trigger가 없는지 확인. 없으면 owner-local로 두고 entry를 만들지 않음 |
| `scripts/docs-working-model-check.ps1` | terminology-glossary revision이 소유하는 `TERM-RESERVE` 제거와 `GLOSSARY-POINTER` 미도입 결과를 interface 입력으로만 대조. 이 G Work Packet은 checker를 수정하거나 처분을 close하지 않고 G 변경이 제거·불채택 진단을 재도입하지 않는지만 확인 |
| `tests/docs-working-model-check.Tests.ps1` | terminology-glossary revision의 처분 결과를 reference로만 소비. 이 G Work Packet은 전용 tests 제거/미도입을 자기 구현 scope로 소유하지 않음 |
| `rule_docs/docs-working-model/docs-working-model_backlog.md` | provisional terminology 후속 row를 삭제하고 durable-pointer 기계화는 반복 관측+결정 가능한 predicate가 있을 때만 재검토 |

Pre-transition tracked reference 조사에서 candidate id와 내부 분류값의 canonical sibling mention은 없었고, candidate 문서 외부의 durable reference도 없었다. 따라서 promotion sibling sweep의 변경 대상은 별도로 발견되지 않았으며, transition 자체가 만드는 Design/Plan/WP의 owner-local usage는 promoted 상태에 맞는 새 usage로 분류한다.
...
| `rules/terminology-glossary.md` | terminal landing에서 실제 프로젝트 공용 용어 trigger가 새로 생겼는지 재확인하고, 생긴 경우에만 채택/기각 |
...
기존 `snippets/rules/repository-change-safety.md`, `snippets/rules/no-background-or-hidden-state.md`, `snippets/rules/global-file-mutation-boundary.md`는 G-1의 interface 대조 입력이지 그 terminal rule의 수정 target이 아니다. 이 중 global-file boundary의 실제 충돌은 별도 owner-local revision lifecycle이 소유하며 G-1 범위 확장으로 흡수하지 않는다.
...
- **Structural:** active-lifecycle snapshot, atomic incubation removal, rule-folder purity, E2/E3, glossary trigger/no-provisional-state, sibling mention sweep.
...
- glossary trigger가 없는데 owner-local classification token을 선등록하지 않았는가; trigger가 생겼다면 한 줄 의미만 소유하고 full procedure를 복제하지 않는가.

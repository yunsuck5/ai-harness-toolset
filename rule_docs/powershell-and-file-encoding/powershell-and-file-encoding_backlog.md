...
| PFE-B-04 | helper-local byte-stdin observable contract와 terminal rule의 descriptive realization 요약 사이 최종 의미 소유 surface·single-home 경계를 확정 | `Invoke-NativeProcess`에 다음 public parameter/realization이 추가되거나 두 표면이 drift할 때 |
| PFE-B-05 | `.md`/`.json`/`.txt`의 UTF-8 no-BOM+LF 문면을 binding convention으로 유지할지 advisory로 낮출지와, binding 유지 시 enforcement 범위를 심사 | 별도 scoped PFE goal이 승인되거나 non-PS1 인코딩 false-pass·운용 incident가 관측될 때 |

...
| GFM-B-05 | project-local instruction의 separate scoped approval이 scope correction만 허용하는지 safety-weakening exception도 허용하는지 심사하고, 후자를 허용할 경우 named changeset·지속 범위·종료 조건을 결정 | 구체 exception/bypass 요청이 발생하거나 별도 scoped GFM goal이 승인될 때 |

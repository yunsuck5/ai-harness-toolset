...
이 문서는 용어집이 아닙니다. 프로젝트 공용으로 채택·기각된 용어의 의미는 `rules/terminology-glossary.md`를 기준으로 하고, owner-local 용어의 상세 의미는 해당 active owner를 따릅니다. 아래 표는 처음 읽는 사람이 용어를 주로 어디서 만나는지만 안내하며, 모든 항목이 glossary entry라는 뜻은 아닙니다.
...
프로젝트 공용 의미는 `rules/terminology-glossary.md`에서, 그 밖의 상세 의미는 해당 active owner에서 확인하십시오.

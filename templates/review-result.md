...
이 파일은 reviewer가 발행하는 **기계적으로 완전한 candidate body**의 `log/review/<review-task-id>/<perspective>/pass-NN/result.md` 형식 기준이다. active reviewer adapter (현재 MVP adapter = codex) 가 `--output-last-message` 로 같은 pass directory 에 result.md 의 verdict/disclosure **body** 를 작성하고, `scripts/review-run.ps1` 가 그 complete shape를 확인한 뒤 `## Reviewer run provenance` 블록을 append 한다 (result.md 는 dual-authored — 아래 note 참조). invocation 실패·result 미생성·shape invalid pass 에는 H1 token과 provenance를 만들지 않는다. shape-valid body도 operator intake에서 내부 모순으로 unusable 판정될 수 있으며, 이때 provenance는 run fact로 남지만 candidate token은 consumable reviewer verdict가 아니다. operator-role AI 가 본문을 읽고 finding / risk / required change 의 의미를 판단한다.
...
target artifact의 지속성이 finding severity나 closure에 영향을 주면 그 사실을 해당 finding prose에 적는다. transient라는 이유만으로 자동 nonblocking 처리하지 않으며, committed temporary artifact도 존재하는 동안 실제 review target이다. 이 정보는 finding prose의 판단 근거이지 새 verdict/H2/parser field/tag가 아니다.

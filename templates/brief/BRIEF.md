...
`<ProjectRoot>/brief/BRIEF.md` 의 시작 template 이다. 이 파일은 새로운 사람이나 새로운 AI agent 가 이 project 를 처음 잡을 때 가장 먼저 읽는 **durable project restore file** 이다 (BF Level 3).

이 template 은 source repo (`ai-harness-toolset`) 의 `templates/brief/BRIEF.md` 다. 실제 BRIEF artifact 는 target project 의 `<ProjectRoot>/brief/BRIEF.md` 에 둔다. 두 자리는 다르며, 본 파일을 source repo 의 `brief/` 아래에 commit 하지 않는다 (`docs/BRIEF_CONTRACT.md` 의 source-vs-target 경계 절 참조).

`scripts/brief-init.ps1` 가 이 template 을 target project 의 `<ProjectRoot>/brief/BRIEF.md` 로 한 번 seed 한다. 이후 갱신은 사람이 손으로 한다. tooling 은 본문을 자동으로 채우거나 보정하지 않는다.
...
> 새 collaborator / 새 agent 가 가장 먼저 읽을 path. 경로 위주로 짧게.

...
`## Reference sweep` informational section 은 이름 / 위치 / 식별자 / 구조를 바꾸는 변경 (파일 rename · 이동 · **삭제 / 제거**, 식별자 집합 / range 변경, 폴더 역할 재정의, 용어 교체, fixed-line / fixed-token cleanup) 이 있는 round 에서 operator 가 review 호출 전에 수행한 reference sweep 의 *증거* 를 기록하는 자리다 (네 class 의 정의는 아래에 inline). `swept: yes` 같은 주장만 적지 말고 최소한 — (1) sweep 의 trigger (어떤 rename / move / delete / identifier / structure / fixed-token 변경), (2) 검색한 pattern, (3) 검색한 path 범위, (4) 네 class (filename·path / bare-token·ID / folder-as-bucket / semantic-phrasing) 중 점검한 것과 미점검(unswept) class, (5) grep 으로 잡히지 않는 semantic phrasing class 를 의미 기준으로 점검했는지 — 가 드러나게 적는다. **삭제 / 제거** 변경에서는 다음 deletion granular technique 를 적용한다 — case-insensitive 검색, 철자 / 대소문자 변형(variant), 식별자 없이 섹션만 가리키는 bare-section 참조 (`§N`, `narrative §N`). 이름 / 위치 / 식별자 / 구조 변경이 없는 round 에서는 `N/A — no name/path/identifier/structure change this round.` 같은 짧은 명시를 둔다. 본 section 도 verifier 의 strict shape gate 대상이 아니며, sweep 의 완전성은 기계적으로 판정되지 않는다 (operator evidence 기록이지 sweep lint 가 아니다).
...
(informational) 이 review 에 쓴 `-Perspective <viewpoint>` (예: `local-correctness` / `system-coherence`) 를 여기에 적어 input.md 가 자기 layout 을 self-document 하게 한다. perspective 는 entrypoint(`review-prepare/run/verify`) 에서 **필수** 이며 layout 은 always three-level (`log/review/<review-task-id>/<perspective>/pass-NN/`) 이다. 본 section 은 informational 이며 `scripts/review-input-verify.ps1` 의 shape gate 대상이 아니다 (parser 강제 없음; active placeholder 도 두지 않는다).
...
operator-role AI 가 verdict 와 4 disclosure section 본문을 읽은 뒤 수행할 next-action mapping (`yes` / `no` / `yes with risk` 각각의 다음 단계) 는 `ai-harness-review` skill 의 step 7 (Verdict → next-action mapping) 이 정의한다. verdict line 만으로 다음 단계를 결정하지 않으며, 4 disclosure section 본문을 함께 읽는다.
